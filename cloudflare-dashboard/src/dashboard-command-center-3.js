...
function characterConnectionState(row){return String(row&&row.connectionState||row&&row.status&&row.status.connectionState||'offline').toLowerCase()}
function charactersOnlineFirst(rows){return (Array.isArray(rows)?rows:[]).map((row,index)=>({row,index,offline:characterConnectionState(row)==='offline'?1:0})).sort((a,b)=>a.offline-b.offline||a.index-b.index).map(entry=>entry.row)}
function renderOverview(){if(!overview)return;const chars=overview.characters||[],displayChars=charactersOnlineFirst(chars);$('overviewTime').textContent='Stand '+new Date(overview.now||Date.now()).toLocaleTimeString('de-DE');$('characterCards').innerHTML=displayChars.length?displayChars.map(charCard).join(''):'<div class="empty">Noch keine v3 Runtime-Snapshots.</div>';renderQuotaDeck();renderAutomation();renderEconomy(chars)}
