...
.al-panel{overflow:auto;border:4px solid #777;background:#000;padding:7px;box-shadow:0 8px 24px #0009}
...
.al-equipment-grid{display:grid;grid-template-columns:repeat(4,52px);gap:2px;width:max-content;margin:auto}
.al-inventory-grid{display:grid;grid-template-columns:repeat(7,52px);gap:2px;width:max-content;margin:auto}
.al-slot{position:relative;width:52px;height:52px;overflow:hidden;border:2px solid #666;background:#000;box-sizing:border-box;image-rendering:pixelated}
.al-slot.empty{border-color:#292929}.al-slot.filled:hover{border-color:#bcbcbc;box-shadow:0 0 0 1px #ffffff30 inset}
.al-sprite-frame{position:absolute;left:4px;top:4px;width:40px;height:40px;overflow:hidden;pointer-events:none}
...
.al-qty{right:1px;bottom:0;padding:0 2px;background:#050505dc;color:#fff;font-size:13px;line-height:15px}
...
@media(max-width:760px){.al-panel{padding:5px}.al-slot{width:48px;height:48px}.al-sprite-frame{left:2px;top:2px}.al-equipment-grid{grid-template-columns:repeat(4,48px)}.al-inventory-grid{grid-template-columns:repeat(7,48px)}.event{border-left-width:7px}.event .sev{min-width:70px;padding:6px 7px}}
`;
...
function installCharacterPresenceDurations(source) {
  ...
  const loadoutHelpers = "const gearSlotRows=[['earring1','helmet','earring2','amulet'],['mainhand','chest','offhand','cape'],['ring1','pants','ring2','orb'],['belt','shoes','gloves','elixir']];function alSprite(it,sprites){const sp=it&&sprites&&sprites[it.name];if(!sp)return '<span class=\"al-missing\">?</span>';const cell=40,cols=Math.max(1,Number(sp.columns)||1),rows=Math.max(1,Number(sp.rows)||1),x=Math.max(0,Number(sp.x)||0),y=Math.max(0,Number(sp.y)||0);return '<span class=\"al-sprite-frame\"><img loading=\"lazy\" decoding=\"async\" referrerpolicy=\"no-referrer\" src=\"'+esc(sp.file)+'\" style=\"width:'+cols*cell+'px;height:'+rows*cell+'px;left:'+(-x*cell)+'px;top:'+(-y*cell)+'px\"></span>';}function alLevel(level){const n=Number(level)||0;return n===10?'X':n===11?'Y':n===12?'Z':String(n)}function alSlot(it,sprites,label){const title=label+(it&&it.name?' · '+it.name:' · Leer'),q=it?Number(it.q)||1:1,level=it?Number(it.level)||0:0;return '<div class=\"al-slot '+(it?'filled':'empty')+'\" title=\"'+esc(title)+'\">'+(it?alSprite(it,sprites):'<span class=\"al-missing\">?</span>')+(q>1?'<span class=\"al-qty\">'+esc(fmt(q))+'</span>':'')+(level>0?'<span class=\"al-level\">'+esc(alLevel(level))+'</span>':'')+(it&&it.locked?'<span class=\"al-lock\">X</span>':'')+(it&&it.special?'<span class=\"al-special\">✦</span>':'')+'<span class=\"al-slot-name\">'+esc(it&&it.name||label)+'</span></div>';}function equipmentHtml(gear,sprites){const g=gear&&typeof gear==='object'?gear:{},labels={earring1:'Ohrring 1',helmet:'Helm',earring2:'Ohrring 2',amulet:'Amulett',mainhand:'Haupthand',chest:'Rüstung',offhand:'Nebenhand',cape:'Umhang',ring1:'Ring 1',pants:'Hose',ring2:'Ring 2',orb:'Orb',belt:'Gürtel',shoes:'Schuhe',gloves:'Handschuhe',elixir:'Elixier'};return '<section class=\"al-panel\"><div class=\"al-panel-head\"><b>GETRAGENE AUSRÜSTUNG</b><span>Adventure-Land-Layout</span></div><div class=\"al-equipment-grid\">'+gearSlotRows.flat().map(slot=>alSlot(g[slot],sprites,labels[slot]||slot)).join('')+'</div></section>';}function inventoryHtml(items,sprites,size){const list=Array.isArray(items)?items.filter(Boolean):[],byIndex=new Map();let highest=-1;list.forEach((it,i)=>{const idx=Number.isFinite(Number(it.index))?Math.max(0,Math.floor(Number(it.index))):i;byIndex.set(idx,it);highest=Math.max(highest,idx)});const requested=Number.isFinite(Number(size))?Math.max(0,Math.floor(Number(size))):0,resolved=Math.min(160,Math.max(42,requested,highest+1)),slots=Math.ceil(resolved/7)*7;return '<section class=\"al-panel\"><div class=\"al-panel-head\"><b>INVENTAR</b><span>'+list.length+' / '+resolved+' belegt</span></div><div class=\"al-inventory-grid\">'+Array.from({length:slots},(_,idx)=>alSlot(byIndex.get(idx),sprites,'Slot '+(idx+1))).join('')+'</div></section>';};";
  ...
  const footerReplacement = "<div class=\"line\"><span>Brain</span><b>'+esc(brainS.quality&&brainS.quality.state||'–')+'</b></div><div class=\"char-details\">'+equipmentHtml(gear,itemSprites)+inventoryHtml(inventory,itemSprites,inventorySize)+'</div></article>'";
