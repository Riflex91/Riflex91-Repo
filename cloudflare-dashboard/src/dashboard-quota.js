export const QUOTA_ROWS_FUNCTION = `function quotaRows(){
  ...
  return [
    ...
    {icon:'▦',title:'D1',note:d1Blocked?'Tageslimit erreicht':(d1Worst?('Reads '+humanCount(read.remaining)+' frei · Writes '+humanCount(write.remaining)+' frei'):'Row-Zugriffe · Bot-Zähler startet mit diesem Release'),remaining:d1Blocked?0:(d1Worst&&d1Worst.remaining),limit:d1Blocked?(Number(read.limit)||5000000):(d1Worst&&d1Worst.limit),unit:'count',source:d1Blocked?'D1 Quota-Circuit offen':(d1Worst?'engster Wert: '+d1Worst.name+(d1.approximate?' · Bot-Zähler':''):'Live-Wert fehlt')},
