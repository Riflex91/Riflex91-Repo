...
export const BANK_RETRIEVE_SOCKET_BUDGET_GEWICHT = 20;
export const BANK_RETRIEVE_SHADOW_INVARIANTEN = Object.freeze([
  "V5-ALT-002",
  "V5-ALT-003",
  "V5-INV-002",
  "V5-INV-004",
...
export const BANK_RETRIEVE_SHADOW_LIVE_VORAUSSETZUNGEN = Object.freeze([
  "character",
  "bank",
  "inventory",
] as const);

export interface BankRetrieveShadowSnapshot {
...
export interface BankRetrieveShadowAnforderung {
  readonly schemaVersion: 1;
  ...
  readonly snapshot: BankRetrieveShadowSnapshot;
  readonly authority: ProduktiveBankRetrieveEinmalAuthority;
}

export interface BankRetrieveShadowReleaseBeobachterPort {
  beobachte(
...
export interface BankRetrieveShadowAbhaengigkeiten {
  ...
  readonly releaseBeobachter: BankRetrieveShadowReleaseBeobachterPort;
  readonly vorabLeaseToken?: BankLeaseToken;
  readonly jetztMs: () => number;
}

export interface BankRetrieveShadowErgebnis {
  readonly schemaVersion: 1;
...
function validiereAnforderung(a: BankRetrieveShadowAnforderung): void {
  if (a.schemaVersion !== 1) {
    throw new Error("BANK_RETRIEVE_SHADOW_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [a.freigabeId, "BANK_RETRIEVE_SHADOW_FREIGABE_ID_UNGUELTIG"],
    [a.auftragId, "BANK_RETRIEVE_SHADOW_AUFTRAG_ID_UNGUELTIG"],
    [a.ablaufId, "BANK_RETRIEVE_SHADOW_ABLAUF_ID_UNGUELTIG"],
    [a.transaktionsId, "BANK_RETRIEVE_SHADOW_TX_ID_UNGUELTIG"],
    [a.accountId, "BANK_RETRIEVE_SHADOW_ACCOUNT_ID_UNGUELTIG"],
    [a.characterId, "BANK_RETRIEVE_SHADOW_CHARACTER_ID_UNGUELTIG"],
    [a.serverRegion, "BANK_RETRIEVE_SHADOW_SERVER_REGION_UNGUELTIG"],
    [a.serverIdentifier, "BANK_RETRIEVE_SHADOW_SERVER_ID_UNGUELTIG"],
    [a.snapshot.fingerprint, "BANK_RETRIEVE_SHADOW_SNAPSHOT_FP_UNGUELTIG"],
  ] as const) {
    pruefeText(wert, fehler);
  }
  pruefeZeit(
    a.ausgestelltAmMs,
    "BANK_RETRIEVE_SHADOW_AUSSTELLZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.gueltigBisMs,
    "BANK_RETRIEVE_SHADOW_GUELTIGKEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.externalFenceBeobachtetAmMs,
    "BANK_RETRIEVE_SHADOW_FENCE_ZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.snapshot.beobachtetAmMs,
    "BANK_RETRIEVE_SHADOW_SNAPSHOT_ZEIT_UNGUELTIG",
  );
  if (a.gueltigBisMs < a.ausgestelltAmMs
    ...
    throw new Error("BANK_RETRIEVE_SHADOW_ZEITFENSTER_UNGUELTIG");
  }
  if (a.externalFenceBeobachtetAmMs > a.ausgestelltAmMs
      || a.ausgestelltAmMs - a.externalFenceBeobachtetAmMs > 1_000) {
    throw new Error("BANK_RETRIEVE_SHADOW_EXTERNAL_FENCE_STALE");
  }
  if (a.snapshot.schemaVersion !== 1
    ...
    throw new Error("BANK_RETRIEVE_SHADOW_SNAPSHOT_BINDUNG_UNGUELTIG");
  }
  if (a.externalFence.konflikt
    ...
    throw new Error("BANK_RETRIEVE_SHADOW_EXTERNAL_FENCE_UNGUELTIG");
  }
}
...
function journalEintrag(
  a: BankRetrieveShadowAnforderung,
  sequenz: number,
...
export class ProduktiveBankRetrieveShadowAdmission {
  public async pruefe(
    a: BankRetrieveShadowAnforderung,
    d: BankRetrieveShadowAbhaengigkeiten,
    ...
    try {
      if (d.vorabLeaseToken !== undefined) {
        ...
        if (token.accountId !== a.accountId
          ...
          throw new Error("BANK_RETRIEVE_SHADOW_VORAB_LEASE_BINDUNG_UNGUELTIG");
        }
        ...
        if (sichtbar?.zustand !== "ACQUIRING") {
          throw new Error("BANK_RETRIEVE_SHADOW_VORAB_LEASE_NICHT_ACQUIRING");
        }
        leaseToken = token;
      } else {
      ...
      if (lease.zustand !== "ACTIVE") {
        throw new Error("BANK_RETRIEVE_SHADOW_LEASE_NICHT_ACTIVE");
      }
      ...
      if (!d.leaseController.validiereMutation(
        ...
        throw new Error("BANK_RETRIEVE_SHADOW_MUTATIONS_FENCE_UNGUELTIG");
      }
      if (!d.leaseController.validiereSnapshot(
        ...
        throw new Error("BANK_RETRIEVE_SHADOW_SNAPSHOT_NICHT_FREIGEGEBEN");
      }
      ...
      const freigabe = await ErteilteAusfuehrungsFreigabe.erteile({
        ...
        verifierId: BANK_RETRIEVE_VERIFIER_ID,
        invariantenKennungen: BANK_RETRIEVE_SHADOW_INVARIANTEN,
        voraussetzungsIds: BANK_RETRIEVE_SHADOW_LIVE_VORAUSSETZUNGEN,
