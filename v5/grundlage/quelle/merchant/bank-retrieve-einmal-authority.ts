...
export const BANK_RETRIEVE_ACTION_CONTRACT_ID = "AL-ACTION-BANK-SWAP";
export const BANK_RETRIEVE_RECOVERY_CONTRACT_ID = "AL-RECOVERY-BANK-SWAP";
export const BANK_RETRIEVE_VERIFIER_ID = "AL-VERIFIER-BANK-SWAP";
export const BANK_RETRIEVE_EINMAL_POLICY_ID = "BANK-SWAP-PRODUKTION-EINMAL-V1";
export const BANK_RETRIEVE_EINMAL_BESTAETIGUNG =
  "V5 BANK SWAP EINMAL AUSFUEHREN";

export interface V5BankRetrieveEinmalAuthorityAnforderung {
  readonly schemaVersion: 1;
  readonly aktivierungsId: string;
  readonly transaktionsId: string;
  readonly faehigkeitId: typeof MERCHANT_BANK_RETRIEVE_FAEHIGKEIT_ID;
  ...
  readonly actionContractId: typeof BANK_RETRIEVE_ACTION_CONTRACT_ID;
  readonly recoveryContractId: typeof BANK_RETRIEVE_RECOVERY_CONTRACT_ID;
  readonly verifierId: typeof BANK_RETRIEVE_VERIFIER_ID;
  readonly policyId: typeof BANK_RETRIEVE_EINMAL_POLICY_ID;
  readonly bestaetigungText: typeof BANK_RETRIEVE_EINMAL_BESTAETIGUNG;
...
export interface V5BankRetrieveEinmalAuthorityRevalidierungsErgebnis {
  readonly schemaVersion: 1;
...
export interface V5BankRetrieveEinmalAuthorityDurableIntent {
  readonly schemaVersion: 1;
  readonly aktivierungsId: string;
  readonly transaktionsId: string;
  readonly faehigkeitId: typeof MERCHANT_BANK_RETRIEVE_FAEHIGKEIT_ID;
  ...
  readonly gueltigBisMs: number;
  readonly art: "BANK_RETRIEVE_EINMAL_AUTHORITY_VOR_WIRKUNG";
...
export interface V5BankRetrieveEinmalAuthorityDurableBestaetigung {
...
export interface V5BankRetrieveEinmalAuthorityProtokollPort {
  schreibeDurable(
    intent: V5BankRetrieveEinmalAuthorityDurableIntent,
  ): Promise<V5BankRetrieveEinmalAuthorityDurableBestaetigung>;
}

export interface V5BankRetrieveEinmalAuthorityDaten {
  readonly schemaVersion: 1;
  readonly aktivierungsId: string;
  readonly transaktionsId: string;
  readonly faehigkeitId: typeof MERCHANT_BANK_RETRIEVE_FAEHIGKEIT_ID;
...
export class ProduktiveBankRetrieveEinmalAuthority
implements FaehigkeitsAutoritaetsPort {
  readonly #daten: V5BankRetrieveEinmalAuthorityDaten;
  #verbraucht = false;
  #widerrufen = false;

  public constructor(daten: V5BankRetrieveEinmalAuthorityDaten) {
  ...
  public daten(): V5BankRetrieveEinmalAuthorityDaten {
    return this.#daten;
  }
}

export interface V5BankRetrieveEinmalAuthorityErgebnis {
  readonly schemaVersion: 1;
  readonly erfolgreich: boolean;
  readonly grund: string;
  readonly authority: ProduktiveBankRetrieveEinmalAuthority | null;
  readonly evidenceIds: readonly string[];
  readonly maximaleVerwendungen: 1;
  readonly gameplayWriteAusgefuehrt: false;
  readonly rawWriteAutoritaet: false;
...
export async function erteileProduktiveBankRetrieveEinmalAuthority(
  anforderung: V5BankRetrieveEinmalAuthorityAnforderung,
  protokoll: V5BankRetrieveEinmalAuthorityProtokollPort | null,
  ...
  if (!anforderung || anforderung.schemaVersion !== 1
    ...
    return blockiert("V5_BANK_RETRIEVE_EINMAL_BINDUNG_UNGUELTIG");
  }
  if (!Number.isSafeInteger(anforderung.jetztMs)
    ...
    return blockiert("V5_BANK_RETRIEVE_EINMAL_ZEIT_ODER_GENERATION_UNGUELTIG");
  }
  const ids = evidenceIds(anforderung.healthEvidence, anforderung.jetztMs);
  if (ids === null) return blockiert("V5_BANK_RETRIEVE_EINMAL_HEALTH_EVIDENCE_UNGUELTIG");
  if (protokoll === null) {
    return blockiert("V5_BANK_RETRIEVE_EINMAL_DURABLE_PROTOKOLL_FEHLT", ids);
  }
  ...
  let ack: V5BankRetrieveEinmalAuthorityDurableBestaetigung;
  try {
    ack = await protokoll.schreibeDurable(intent);
  } catch {
    return blockiert("V5_BANK_RETRIEVE_EINMAL_AUDIT_NICHT_DURABLE", ids);
  }
  if (ack.durable !== true
      || ack.aktivierungsId !== anforderung.aktivierungsId
      || ack.transaktionsId !== anforderung.transaktionsId
      || !text(ack.bestaetigungsId)) {
    return blockiert("V5_BANK_RETRIEVE_EINMAL_DURABILITY_NICHT_BESTAETIGT", ids);
  }
