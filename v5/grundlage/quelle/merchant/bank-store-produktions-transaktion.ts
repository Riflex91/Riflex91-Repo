...
import {
  BANK_STORE_SHADOW_INVARIANTEN,
  BANK_STORE_SOCKET_BUDGET_GEWICHT,
} from "./bank-store-shadow-admission.js";

export const BANK_STORE_PRODUKTIV_LIVE_VORAUSSETZUNGEN = Object.freeze([
  "character",
  "bank",
  "inventory",
] as const);

export interface BankStoreExplizitAdapterAnfrage {
  readonly modus: "STORE";
...
export interface ProduktiverBankStoreBeobachterPort {
  beobachte(
    leaseEpoche: number,
    mountEpoche: number,
...
export interface ProduktiveBankStoreTransaktionsAnforderung {
...
export interface ProduktiveBankStoreTransaktionsAbhaengigkeiten<Ergebnis> {
  ...
  readonly adapter: AusfuehrungsAdapter<
    BankStoreExplizitAdapterAnfrage,
    Ergebnis,
    ErteilteAusfuehrungsFreigabe
  >;
  readonly bankBeobachter: ProduktiverBankStoreBeobachterPort;
...
export interface ProduktiveBankStoreTransaktionsErgebnis {
  readonly schemaVersion: 1;
  readonly transaktionsId: string;
  readonly status:
    | "COMMITTED"
...
function validiereAnforderung(
  a: ProduktiveBankStoreTransaktionsAnforderung,
): void {
  if (a.schemaVersion !== 1) {
    throw new Error("BANK_STORE_PROD_TX_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [a.freigabeId, "BANK_STORE_PROD_TX_FREIGABE_ID_UNGUELTIG"],
    [a.auftragId, "BANK_STORE_PROD_TX_AUFTRAG_ID_UNGUELTIG"],
    [a.ablaufId, "BANK_STORE_PROD_TX_ABLAUF_ID_UNGUELTIG"],
    [a.transaktionsId, "BANK_STORE_PROD_TX_ID_UNGUELTIG"],
    [a.accountId, "BANK_STORE_PROD_TX_ACCOUNT_ID_UNGUELTIG"],
    [a.characterId, "BANK_STORE_PROD_TX_CHARACTER_ID_UNGUELTIG"],
    [a.sessionId, "BANK_STORE_PROD_TX_SESSION_ID_UNGUELTIG"],
    [a.serverRegion, "BANK_STORE_PROD_TX_SERVER_REGION_UNGUELTIG"],
    [a.serverIdentifier, "BANK_STORE_PROD_TX_SERVER_ID_UNGUELTIG"],
    [a.configFingerprint, "BANK_STORE_PROD_TX_CONFIG_FP_UNGUELTIG"],
    [a.prestateFingerprint, "BANK_STORE_PROD_TX_PRESTATE_FP_UNGUELTIG"],
  ] as const) {
  ...
  if (!/^items[0-9]+$/.test(a.pack)
    ...
    throw new Error("BANK_STORE_PROD_TX_KANDIDAT_UNGUELTIG");
  }
  pruefeZeit(a.ausgestelltAmMs, "BANK_STORE_PROD_TX_ZEIT_UNGUELTIG");
  pruefeZeit(
    a.gueltigBisMs,
    "BANK_STORE_PROD_TX_ABLAUFZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.externalFenceBeobachtetAmMs,
    "BANK_STORE_PROD_TX_FENCE_ZEIT_UNGUELTIG",
  );
  ...
  if (v.characterId !== a.characterId
      ...
      || (v.bankSlotItem !== null
        ...
        || v.inventorySlotItem.fingerprint !== v.transferItem.fingerprint)
      || !Number.isSafeInteger(v.characterGold)
      || v.characterGold < 0
      || !Number.isSafeInteger(v.bankGold)
      || v.bankGold < 0) {
    throw new Error("BANK_STORE_PROD_TX_PRESTATE_BINDUNG_UNGUELTIG");
  }
  if (a.externalFence.konflikt
    ...
    throw new Error("BANK_STORE_PROD_TX_EXTERNAL_FENCE_UNGUELTIG");
  }
  if (!/^[0-9a-f]{40}$/i.test(a.wissensSnapshot.gitCommit)
    ...
    throw new Error("BANK_STORE_PROD_TX_WISSENSSNAPSHOT_UNGUELTIG");
  }
  const authority = a.authority.daten();
  if (authority.transaktionsId !== a.transaktionsId
      || authority.faehigkeitId !== MERCHANT_BANK_STORE_FAEHIGKEIT_ID
      || authority.anbieterModulId !== MERCHANT_BANK_CORE_MODUL_ID
      || authority.actionContractId !== BANK_STORE_ACTION_CONTRACT_ID
      || authority.recoveryContractId !== BANK_STORE_RECOVERY_CONTRACT_ID
      || authority.verifierId !== BANK_STORE_VERIFIER_ID
      || a.gueltigBisMs > authority.gueltigBisMs
      || !a.authority.gueltigFuer(a.ausgestelltAmMs)) {
    throw new Error("BANK_STORE_PROD_TX_AUTHORITY_BINDUNG_UNGUELTIG");
  }
}
...
function statusFuer(
  recovery: RecoveryAbschluss,
): ProduktiveBankStoreTransaktionsErgebnis["status"] {
  switch (recovery.art) {
...
function liveVoraussetzungen(
  a: ProduktiveBankStoreTransaktionsAnforderung,
  beobachter: ProduktiverBankStoreBeobachterPort,
) {
  return Object.freeze({
    async pruefe(ids: readonly string[], jetztMs: number) {
      if (ids.length !== BANK_STORE_PRODUKTIV_LIVE_VORAUSSETZUNGEN.length
          || ids.some(id =>
            !BANK_STORE_PRODUKTIV_LIVE_VORAUSSETZUNGEN.includes(
              id as typeof BANK_STORE_PRODUKTIV_LIVE_VORAUSSETZUNGEN[number],
            ))) {
        throw new Error("BANK_STORE_PROD_TX_LIVE_IDS_UNGUELTIG");
      }
      ...
      if (b.characterId !== a.vorher.characterId
        ...
        throw new Error("BANK_STORE_PROD_TX_LIVE_PRESTATE_DRIFT");
      }
...
export class ProduktiveBankStoreTransaktionsOrchestrierung {
  public async fuehreEinmalAus<Ergebnis>(
    a: ProduktiveBankStoreTransaktionsAnforderung,
    d: ProduktiveBankStoreTransaktionsAbhaengigkeiten<Ergebnis>,
  ): Promise<ProduktiveBankStoreTransaktionsErgebnis> {
    validiereAnforderung(a);
    if (d.adapter.actionContractId !== BANK_STORE_ACTION_CONTRACT_ID
        || d.adapter.recoveryContractId !== BANK_STORE_RECOVERY_CONTRACT_ID
        || d.adapter.verifierId !== BANK_STORE_VERIFIER_ID) {
      throw new Error("BANK_STORE_PROD_TX_ADAPTER_BINDUNG_UNGUELTIG");
    }
    ...
    try {
      if (d.vorabLeaseToken !== undefined) {
        ...
        if (leaseToken.accountId !== a.accountId
          ...
          throw new Error("BANK_STORE_PROD_TX_VORAB_LEASE_DRIFT");
        }
        ...
        if (sichtbar?.zustand !== "ACQUIRING") {
          throw new Error("BANK_STORE_PROD_TX_VORAB_LEASE_NICHT_ACQUIRING");
        }
      } else {
      ...
      if (lease.zustand !== "ACTIVE") {
        throw new Error("BANK_STORE_PROD_TX_LEASE_NICHT_ACTIVE");
      }
      ...
      if (ersterInventoryToken === undefined) {
        throw new Error("BANK_STORE_PROD_TX_INVENTORY_TOKEN_FEHLT");
      }
      ...
      if (!d.leaseController.validiereMutation(
        ...
        throw new Error("BANK_STORE_PROD_TX_MUTATIONS_FENCE_UNGUELTIG");
      }
      if (!d.leaseController.validiereSnapshot(
        ...
        throw new Error("BANK_STORE_PROD_TX_SNAPSHOT_NICHT_FREIGEGEBEN");
      }
      ...
      try {
        freigabe = await ErteilteAusfuehrungsFreigabe.erteile({
          ...
          verifierId: BANK_STORE_VERIFIER_ID,
          invariantenKennungen: BANK_STORE_SHADOW_INVARIANTEN,
          voraussetzungsIds: BANK_STORE_PRODUKTIV_LIVE_VORAUSSETZUNGEN,
