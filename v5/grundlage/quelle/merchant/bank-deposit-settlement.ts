export const BANK_DEPOSIT_ERSTER_BETRAG = 1;

export interface BankDepositBindung {
...
export interface BankDepositEinGoldBereitschaft {
  readonly schemaVersion: 1;
  ...
  readonly bindung: BankDepositBindung;
}

export interface BankDepositBereitschaftErgebnis {
  readonly schemaVersion: 1;
...
export interface BankDepositSettlementErgebnis {
  readonly schemaVersion: 1;
...
function pruefeBindung(bindung: BankDepositBindung): void {
  if (bindung.schemaVersion !== 1) {
    throw new Error("BANK_DEPOSIT_BINDUNG_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [bindung.characterId, "BANK_DEPOSIT_CHARACTER_ID_UNGUELTIG"],
    [bindung.sessionId, "BANK_DEPOSIT_SESSION_ID_UNGUELTIG"],
    [bindung.serverRegion, "BANK_DEPOSIT_SERVER_REGION_UNGUELTIG"],
    [bindung.serverKennung, "BANK_DEPOSIT_SERVER_KENNUNG_UNGUELTIG"],
  ] as const) {
    pruefeText(wert, fehler);
  }
  if (!/^[0-9a-f]{64}$/i.test(bindung.fingerprint)) {
    throw new Error("BANK_DEPOSIT_FINGERPRINT_UNGUELTIG");
  }
  pruefeGanzzahl(
    bindung.leaseEpoche,
    1,
    "BANK_DEPOSIT_LEASE_EPOCHE_UNGUELTIG",
  );
  pruefeGanzzahl(
    bindung.mountEpoche,
    1,
    "BANK_DEPOSIT_MOUNT_EPOCHE_UNGUELTIG",
  );
  pruefeGanzzahl(
    bindung.beobachtetAmMs,
    0,
    "BANK_DEPOSIT_ZEIT_UNGUELTIG",
  );
  pruefeGanzzahl(
    bindung.characterGold,
    0,
    "BANK_DEPOSIT_CHARACTER_GOLD_UNGUELTIG",
  );
  pruefeGanzzahl(
    bindung.bankGold,
    0,
    "BANK_DEPOSIT_BANK_GOLD_UNGUELTIG",
  );
}

function gleicheIdentitaet(
  vorher: BankDepositBindung,
  nachher: BankDepositBindung,
): boolean {
...
export function pruefeBankDepositEinGoldBereitschaft(
  eingabe: BankDepositEinGoldBereitschaft,
): BankDepositBereitschaftErgebnis {
  if (eingabe.schemaVersion !== 1) {
    throw new Error("BANK_DEPOSIT_BEREITSCHAFT_SCHEMA_UNGUELTIG");
  }
  ...
  return Object.freeze({
    ...
    betrag: BANK_DEPOSIT_ERSTER_BETRAG,
...
export function pruefeBankDepositEinGoldSettlement(
  vorher: BankDepositBindung,
  nachher: BankDepositBindung,
): BankDepositSettlementErgebnis {
  ...
  const ergebnis = (
    status: BankDepositSettlementErgebnis["status"],
    grund: string,
  ): BankDepositSettlementErgebnis => Object.freeze({
    schemaVersion: 1,
    status,
    betrag: BANK_DEPOSIT_ERSTER_BETRAG,
    grund,
  ...
  if (!gleicheIdentitaet(vorher, nachher)) {
    return ergebnis("DRIFT", "BANK_DEPOSIT_BINDUNG_DRIFT");
  }
  if (nachher.beobachtetAmMs <= vorher.beobachtetAmMs) {
    return ergebnis("DRIFT", "BANK_DEPOSIT_BEOBACHTUNG_NICHT_NEUER");
  }
  if (!neuerFingerprint) {
    return ergebnis("OFFEN", "BANK_DEPOSIT_KEIN_NEUER_FINGERPRINT");
  }
  if (characterGoldDelta === -BANK_DEPOSIT_ERSTER_BETRAG
      && bankGoldDelta === BANK_DEPOSIT_ERSTER_BETRAG) {
    return ergebnis("BESTAETIGT", "BANK_DEPOSIT_EXAKTES_GOLD_DELTA");
  }
  if (characterGoldDelta === 0 && bankGoldDelta === 0) {
    return ergebnis("OFFEN", "BANK_DEPOSIT_NOCH_KEIN_GOLD_DELTA");
  }
  return ergebnis("DRIFT", "BANK_DEPOSIT_GOLD_DELTA_WIDERSPRUCH");
}
