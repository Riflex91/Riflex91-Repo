...
import {
  ...
  type BankSwapBindung,
} from "./bank-swap-settlement.js";
import {
...
import {
  BANK_SWAP_SHADOW_INVARIANTEN,
  BANK_SWAP_SOCKET_BUDGET_GEWICHT,
} from "./bank-swap-shadow-admission.js";

export const BANK_SWAP_PRODUKTIV_LIVE_VORAUSSETZUNGEN = Object.freeze([
  "character",
  "bank",
  "inventory",
] as const);

export interface BankSwapZweiSlotAdapterAnfrage {
  ...
  readonly erwarteterInventoryFingerprint: string;
  readonly erwartetesCharacterGold: number;
  readonly erwartetesBankGold: number;
  readonly erwarteterFingerprint: string;
}

export interface ProduktiverBankSwapBeobachterPort {
  beobachte(
    leaseEpoche: number,
    mountEpoche: number,
  ): Promise<BankSwapBindung>;
}

export interface ProduktiveBankSwapTransaktionsAnforderung {
  ...
  readonly vorher: BankSwapBindung;
...
export interface ProduktiveBankSwapTransaktionsAbhaengigkeiten<Ergebnis> {
  ...
  readonly adapter: AusfuehrungsAdapter<
    BankSwapZweiSlotAdapterAnfrage,
    Ergebnis,
    ErteilteAusfuehrungsFreigabe
  >;
  readonly bankBeobachter: ProduktiverBankSwapBeobachterPort;
...
export interface ProduktiveBankSwapTransaktionsErgebnis {
  readonly schemaVersion: 1;
  readonly transaktionsId: string;
  readonly status:
    | "COMMITTED"
...
function validiereAnforderung(
  a: ProduktiveBankSwapTransaktionsAnforderung,
): void {
  if (a.schemaVersion !== 1) {
    throw new Error("BANK_SWAP_PROD_TX_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [a.freigabeId, "BANK_SWAP_PROD_TX_FREIGABE_ID_UNGUELTIG"],
    [a.auftragId, "BANK_SWAP_PROD_TX_AUFTRAG_ID_UNGUELTIG"],
    [a.ablaufId, "BANK_SWAP_PROD_TX_ABLAUF_ID_UNGUELTIG"],
    [a.transaktionsId, "BANK_SWAP_PROD_TX_ID_UNGUELTIG"],
    [a.accountId, "BANK_SWAP_PROD_TX_ACCOUNT_ID_UNGUELTIG"],
    [a.characterId, "BANK_SWAP_PROD_TX_CHARACTER_ID_UNGUELTIG"],
    [a.sessionId, "BANK_SWAP_PROD_TX_SESSION_ID_UNGUELTIG"],
    [a.serverRegion, "BANK_SWAP_PROD_TX_SERVER_REGION_UNGUELTIG"],
    [a.serverIdentifier, "BANK_SWAP_PROD_TX_SERVER_ID_UNGUELTIG"],
    [a.configFingerprint, "BANK_SWAP_PROD_TX_CONFIG_FP_UNGUELTIG"],
    [a.prestateFingerprint, "BANK_SWAP_PROD_TX_PRESTATE_FP_UNGUELTIG"],
  ] as const) {
    pruefeText(wert, fehler);
  }
  if (!/^items[0-9]+$/.test(a.pack)
      || !Number.isSafeInteger(a.a) || a.a < 0 || a.a > 41
      || !Number.isSafeInteger(a.b) || a.b < 0 || a.b > 41
      || a.a === a.b) {
    throw new Error("BANK_SWAP_PROD_TX_KANDIDAT_UNGUELTIG");
  }
  pruefeZeit(a.ausgestelltAmMs, "BANK_SWAP_PROD_TX_ZEIT_UNGUELTIG");
  pruefeZeit(
    a.gueltigBisMs,
    "BANK_SWAP_PROD_TX_ABLAUFZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.externalFenceBeobachtetAmMs,
    "BANK_SWAP_PROD_TX_FENCE_ZEIT_UNGUELTIG",
  );
  ...
  if (v.characterId !== a.characterId
      ...
      || v.bankGold < 0) {
    throw new Error("BANK_SWAP_PROD_TX_PRESTATE_BINDUNG_UNGUELTIG");
  }
  if (a.externalFence.konflikt
    ...
    throw new Error("BANK_SWAP_PROD_TX_EXTERNAL_FENCE_UNGUELTIG");
  }
  if (!/^[0-9a-f]{40}$/i.test(a.wissensSnapshot.gitCommit)
    ...
    throw new Error("BANK_SWAP_PROD_TX_WISSENSSNAPSHOT_UNGUELTIG");
  }
  const authority = a.authority.daten();
  if (authority.transaktionsId !== a.transaktionsId
      || authority.faehigkeitId !== MERCHANT_BANK_SWAP_FAEHIGKEIT_ID
      || authority.anbieterModulId !== MERCHANT_BANK_CORE_MODUL_ID
      || authority.actionContractId !== BANK_SWAP_ACTION_CONTRACT_ID
      || authority.recoveryContractId !== BANK_SWAP_RECOVERY_CONTRACT_ID
      || authority.verifierId !== BANK_SWAP_VERIFIER_ID
      || a.gueltigBisMs > authority.gueltigBisMs
      || !a.authority.gueltigFuer(a.ausgestelltAmMs)) {
    throw new Error("BANK_SWAP_PROD_TX_AUTHORITY_BINDUNG_UNGUELTIG");
  }
}
...
function statusFuer(
  recovery: RecoveryAbschluss,
): ProduktiveBankSwapTransaktionsErgebnis["status"] {
  switch (recovery.art) {
...
function liveVoraussetzungen(
  a: ProduktiveBankSwapTransaktionsAnforderung,
  beobachter: ProduktiverBankSwapBeobachterPort,
) {
  return Object.freeze({
    async pruefe(ids: readonly string[], jetztMs: number) {
      if (ids.length !== BANK_SWAP_PRODUKTIV_LIVE_VORAUSSETZUNGEN.length
          || ids.some(id =>
            !BANK_SWAP_PRODUKTIV_LIVE_VORAUSSETZUNGEN.includes(
              id as typeof BANK_SWAP_PRODUKTIV_LIVE_VORAUSSETZUNGEN[number],
            ))) {
        throw new Error("BANK_SWAP_PROD_TX_LIVE_IDS_UNGUELTIG");
      }
      ...
      if (b.characterId !== a.vorher.characterId
          ...
          || b.slotA !== a.vorher.slotA
          || b.slotB !== a.vorher.slotB
          || b.slotAItem.fingerprint !== a.vorher.slotAItem.fingerprint
          || b.slotBItem.fingerprint !== a.vorher.slotBItem.fingerprint
          || b.packRestFingerprint !== a.vorher.packRestFingerprint
          || b.inventoryFingerprint !== a.vorher.inventoryFingerprint
          || b.characterGold !== a.vorher.characterGold
          || b.bankGold !== a.vorher.bankGold
          || b.fingerprint !== a.vorher.fingerprint) {
        throw new Error("BANK_SWAP_PROD_TX_LIVE_PRESTATE_DRIFT");
      }
  ...
  return Object.freeze({
    async beobachte(
      ...
      try {
        ...
        const settlement = pruefeBankSwapSettlement(
          a.vorher,
          nachher,
        );
        let klassifikation:
...
export class ProduktiveBankSwapTransaktionsOrchestrierung {
  public async fuehreEinmalAus<Ergebnis>(
    a: ProduktiveBankSwapTransaktionsAnforderung,
    d: ProduktiveBankSwapTransaktionsAbhaengigkeiten<Ergebnis>,
  ): Promise<ProduktiveBankSwapTransaktionsErgebnis> {
    validiereAnforderung(a);
    if (d.adapter.actionContractId !== BANK_SWAP_ACTION_CONTRACT_ID
        || d.adapter.recoveryContractId !== BANK_SWAP_RECOVERY_CONTRACT_ID
        || d.adapter.verifierId !== BANK_SWAP_VERIFIER_ID) {
      throw new Error("BANK_SWAP_PROD_TX_ADAPTER_BINDUNG_UNGUELTIG");
    }
    ...
    try {
      if (d.vorabLeaseToken !== undefined) {
        ...
        if (leaseToken.accountId !== a.accountId
          ...
          throw new Error("BANK_SWAP_PROD_TX_VORAB_LEASE_DRIFT");
        }
        ...
        if (sichtbar?.zustand !== "ACQUIRING") {
          throw new Error("BANK_SWAP_PROD_TX_VORAB_LEASE_NICHT_ACQUIRING");
        }
      } else {
      ...
      if (lease.zustand !== "ACTIVE") {
        throw new Error("BANK_SWAP_PROD_TX_LEASE_NICHT_ACTIVE");
      }
      ...
      if (ersterInventoryToken === undefined) {
        throw new Error("BANK_SWAP_PROD_TX_INVENTORY_TOKEN_FEHLT");
      }
      ...
      if (!d.leaseController.validiereMutation(
        ...
        throw new Error("BANK_SWAP_PROD_TX_MUTATIONS_FENCE_UNGUELTIG");
      }
      if (!d.leaseController.validiereSnapshot(
        ...
        throw new Error("BANK_SWAP_PROD_TX_SNAPSHOT_NICHT_FREIGEGEBEN");
      }
      ...
      const intentEintrag = journalEintrag(
        ...
        a.ausgestelltAmMs,
        {
          ...
          slot_a: a.a,
          slot_b: a.b,
          item_a_fingerprint: a.vorher.slotAItem.fingerprint,
          item_b_fingerprint: a.vorher.slotBItem.fingerprint,
          pack_rest_fingerprint: a.vorher.packRestFingerprint,
          inventory_fingerprint: a.vorher.inventoryFingerprint,
      ...
      try {
        freigabe = await ErteilteAusfuehrungsFreigabe.erteile({
          ...
          verifierId: BANK_SWAP_VERIFIER_ID,
          invariantenKennungen: BANK_SWAP_SHADOW_INVARIANTEN,
          voraussetzungsIds: BANK_SWAP_PRODUKTIV_LIVE_VORAUSSETZUNGEN,
      ...
      try {
        transport = await d.ausfuehrung.fuehreAus(
          ...
          Object.freeze({
            ...
            erwarteterInventoryFingerprint: a.vorher.inventoryFingerprint,
