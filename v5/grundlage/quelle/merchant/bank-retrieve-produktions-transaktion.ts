...
import {
  ...
  type BankItemTransferBindung,
} from "./bank-item-transfer-settlement.js";
import {
...
import {
  BANK_RETRIEVE_SHADOW_INVARIANTEN,
  BANK_RETRIEVE_SOCKET_BUDGET_GEWICHT,
} from "./bank-retrieve-shadow-admission.js";

export const BANK_RETRIEVE_PRODUKTIV_LIVE_VORAUSSETZUNGEN = Object.freeze([
  "character",
  "bank",
  "inventory",
] as const);

export interface BankRetrieveExplizitAdapterAnfrage {
  readonly modus: "RETRIEVE";
  ...
  readonly erwartetesItemName: string;
  readonly erwartetesItemFingerprint: string;
  readonly erwarteterPackRestFingerprint: string;
  readonly erwarteterInventoryRestFingerprint: string;
  readonly erwartetesCharacterGold: number;
  readonly erwartetesBankGold: number;
  readonly erwarteterFingerprint: string;
}

export interface ProduktiverBankRetrieveBeobachterPort {
  beobachte(
    leaseEpoche: number,
    mountEpoche: number,
  ): Promise<BankItemTransferBindung>;
}

export interface ProduktiveBankRetrieveTransaktionsAnforderung {
  ...
  readonly vorher: BankItemTransferBindung;
...
export interface ProduktiveBankRetrieveTransaktionsAbhaengigkeiten<Ergebnis> {
  ...
  readonly adapter: AusfuehrungsAdapter<
    BankRetrieveExplizitAdapterAnfrage,
    Ergebnis,
    ErteilteAusfuehrungsFreigabe
  >;
  readonly bankBeobachter: ProduktiverBankRetrieveBeobachterPort;
...
export interface ProduktiveBankRetrieveTransaktionsErgebnis {
  readonly schemaVersion: 1;
  readonly transaktionsId: string;
  readonly status:
    | "COMMITTED"
...
function validiereAnforderung(
  a: ProduktiveBankRetrieveTransaktionsAnforderung,
): void {
  if (a.schemaVersion !== 1) {
    throw new Error("BANK_RETRIEVE_PROD_TX_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [a.freigabeId, "BANK_RETRIEVE_PROD_TX_FREIGABE_ID_UNGUELTIG"],
    [a.auftragId, "BANK_RETRIEVE_PROD_TX_AUFTRAG_ID_UNGUELTIG"],
    [a.ablaufId, "BANK_RETRIEVE_PROD_TX_ABLAUF_ID_UNGUELTIG"],
    [a.transaktionsId, "BANK_RETRIEVE_PROD_TX_ID_UNGUELTIG"],
    [a.accountId, "BANK_RETRIEVE_PROD_TX_ACCOUNT_ID_UNGUELTIG"],
    [a.characterId, "BANK_RETRIEVE_PROD_TX_CHARACTER_ID_UNGUELTIG"],
    [a.sessionId, "BANK_RETRIEVE_PROD_TX_SESSION_ID_UNGUELTIG"],
    [a.serverRegion, "BANK_RETRIEVE_PROD_TX_SERVER_REGION_UNGUELTIG"],
    [a.serverIdentifier, "BANK_RETRIEVE_PROD_TX_SERVER_ID_UNGUELTIG"],
    [a.configFingerprint, "BANK_RETRIEVE_PROD_TX_CONFIG_FP_UNGUELTIG"],
    [a.prestateFingerprint, "BANK_RETRIEVE_PROD_TX_PRESTATE_FP_UNGUELTIG"],
  ] as const) {
    pruefeText(wert, fehler);
  }
  if (!/^items[0-9]+$/.test(a.pack)
      || !Number.isSafeInteger(a.bankSlot) || a.bankSlot < 0 || a.bankSlot > 41
      || !Number.isSafeInteger(a.inventorySlot)
      || a.inventorySlot < 0 || a.inventorySlot > 63) {
    throw new Error("BANK_RETRIEVE_PROD_TX_KANDIDAT_UNGUELTIG");
  }
  pruefeZeit(a.ausgestelltAmMs, "BANK_RETRIEVE_PROD_TX_ZEIT_UNGUELTIG");
  pruefeZeit(
    a.gueltigBisMs,
    "BANK_RETRIEVE_PROD_TX_ABLAUFZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.externalFenceBeobachtetAmMs,
    "BANK_RETRIEVE_PROD_TX_FENCE_ZEIT_UNGUELTIG",
  );
  if (a.gueltigBisMs < a.ausgestelltAmMs
    ...
    throw new Error("BANK_RETRIEVE_PROD_TX_ZEITFENSTER_UNGUELTIG");
  }
  if (a.externalFenceBeobachtetAmMs > a.ausgestelltAmMs
      || a.ausgestelltAmMs - a.externalFenceBeobachtetAmMs > 1_000) {
    throw new Error("BANK_RETRIEVE_PROD_TX_FENCE_STALE");
  }
  const v = a.vorher;
  if (v.characterId !== a.characterId
      ...
      || v.richtung !== "RETRIEVE"
      || v.bankPack !== a.pack
      || v.bankSlot !== a.bankSlot
      || v.inventorySlot !== a.inventorySlot
      || v.inventoryCapacity <= a.inventorySlot
      || v.transferItem.fingerprint.trim().length === 0
      || (v.bankSlotItem === null
        || v.inventorySlotItem !== null
        || v.bankSlotItem.fingerprint !== v.transferItem.fingerprint)
      || !Number.isSafeInteger(v.characterGold)
      || v.characterGold < 0
      || !Number.isSafeInteger(v.bankGold)
      || v.bankGold < 0) {
    throw new Error("BANK_RETRIEVE_PROD_TX_PRESTATE_BINDUNG_UNGUELTIG");
  }
  if (a.externalFence.konflikt
    ...
    throw new Error("BANK_RETRIEVE_PROD_TX_EXTERNAL_FENCE_UNGUELTIG");
  }
  if (!/^[0-9a-f]{40}$/i.test(a.wissensSnapshot.gitCommit)
    ...
    throw new Error("BANK_RETRIEVE_PROD_TX_WISSENSSNAPSHOT_UNGUELTIG");
  }
  const authority = a.authority.daten();
  if (authority.transaktionsId !== a.transaktionsId
      || authority.faehigkeitId !== MERCHANT_BANK_RETRIEVE_FAEHIGKEIT_ID
      || authority.anbieterModulId !== MERCHANT_BANK_CORE_MODUL_ID
      || authority.actionContractId !== BANK_RETRIEVE_ACTION_CONTRACT_ID
      || authority.recoveryContractId !== BANK_RETRIEVE_RECOVERY_CONTRACT_ID
      || authority.verifierId !== BANK_RETRIEVE_VERIFIER_ID
      || a.gueltigBisMs > authority.gueltigBisMs
      || !a.authority.gueltigFuer(a.ausgestelltAmMs)) {
    throw new Error("BANK_RETRIEVE_PROD_TX_AUTHORITY_BINDUNG_UNGUELTIG");
  }
}
...
function actionVertrag() {
  return Object.freeze({
    pruefe(
      ...
      const exakt = actionContractId === BANK_RETRIEVE_ACTION_CONTRACT_ID
      ...
      return Object.freeze({
        ...
        invariantenKennungen: BANK_RETRIEVE_SHADOW_INVARIANTEN,
      });
    },
  });
}
...
function statusFuer(
  recovery: RecoveryAbschluss,
): ProduktiveBankRetrieveTransaktionsErgebnis["status"] {
  switch (recovery.art) {
...
function liveVoraussetzungen(
  a: ProduktiveBankRetrieveTransaktionsAnforderung,
  beobachter: ProduktiverBankRetrieveBeobachterPort,
) {
  return Object.freeze({
    async pruefe(ids: readonly string[], jetztMs: number) {
      if (ids.length !== BANK_RETRIEVE_PRODUKTIV_LIVE_VORAUSSETZUNGEN.length
          || ids.some(id =>
            !BANK_RETRIEVE_PRODUKTIV_LIVE_VORAUSSETZUNGEN.includes(
              id as typeof BANK_RETRIEVE_PRODUKTIV_LIVE_VORAUSSETZUNGEN[number],
            ))) {
        throw new Error("BANK_RETRIEVE_PROD_TX_LIVE_IDS_UNGUELTIG");
      }
      ...
      if (b.characterId !== a.vorher.characterId
          ...
          || b.richtung !== a.vorher.richtung
          || b.bankPack !== a.vorher.bankPack
          || b.bankSlot !== a.vorher.bankSlot
          || b.inventorySlot !== a.vorher.inventorySlot
          || b.inventoryCapacity !== a.vorher.inventoryCapacity
          || b.transferItem.fingerprint !== a.vorher.transferItem.fingerprint
          || b.packRestFingerprint !== a.vorher.packRestFingerprint
          || b.inventoryRestFingerprint !== a.vorher.inventoryRestFingerprint
          || b.characterGold !== a.vorher.characterGold
          || b.bankGold !== a.vorher.bankGold
          || b.fingerprint !== a.vorher.fingerprint) {
        throw new Error("BANK_RETRIEVE_PROD_TX_LIVE_PRESTATE_DRIFT");
      }
  ...
  return Object.freeze({
    async beobachte(
      ...
      try {
        ...
        const settlement = pruefeBankItemTransferSettlement(
          a.vorher,
          nachher,
        );
        let klassifikation:
        ...
        return Object.freeze({
          ...
          differenz: Object.freeze({
            schemaVersion: 1,
            erwarteteDomaenen: Object.freeze(["bank"]),
        ...
        return Object.freeze({
          ...
          differenz: Object.freeze({
            ...
            erwarteteDomaenen: Object.freeze(["bank"]),
            angewendeteDomaenen: Object.freeze([]),
            offeneDomaenen: Object.freeze(["bank"]),
            widerspruechlicheDomaenen: Object.freeze(
              finalerVersuch ? ["bank"] : [],
            ),
          }),
          evidenceFingerprints: Object.freeze([
            "BANK_RETRIEVE_BEOBACHTUNG_FEHLER:"
...
export class ProduktiveBankRetrieveTransaktionsOrchestrierung {
  public async fuehreEinmalAus<Ergebnis>(
    a: ProduktiveBankRetrieveTransaktionsAnforderung,
    d: ProduktiveBankRetrieveTransaktionsAbhaengigkeiten<Ergebnis>,
  ): Promise<ProduktiveBankRetrieveTransaktionsErgebnis> {
    validiereAnforderung(a);
    if (d.adapter.actionContractId !== BANK_RETRIEVE_ACTION_CONTRACT_ID
        || d.adapter.recoveryContractId !== BANK_RETRIEVE_RECOVERY_CONTRACT_ID
        || d.adapter.verifierId !== BANK_RETRIEVE_VERIFIER_ID) {
      throw new Error("BANK_RETRIEVE_PROD_TX_ADAPTER_BINDUNG_UNGUELTIG");
    }

    let leaseToken: BankLeaseToken | null = null;
    let kanal: MutationsKanalFreigabe | null = null;
    let inventoryToken: FencingToken | null = null;
    let sequenz = 1;
    ...
    try {
      if (d.vorabLeaseToken !== undefined) {
        ...
        if (leaseToken.accountId !== a.accountId
          ...
          throw new Error("BANK_RETRIEVE_PROD_TX_VORAB_LEASE_DRIFT");
        }
        ...
        if (sichtbar?.zustand !== "ACQUIRING") {
          throw new Error("BANK_RETRIEVE_PROD_TX_VORAB_LEASE_NICHT_ACQUIRING");
        }
      } else {
      ...
      if (lease.zustand !== "ACTIVE") {
        throw new Error("BANK_RETRIEVE_PROD_TX_LEASE_NICHT_ACTIVE");
      }

      const inventoryTokens = d.ressourcen.beanspruche(
        a.ablaufId,
        [{
      ...
      const ersterInventoryToken = inventoryTokens[0];
      if (ersterInventoryToken === undefined) {
        throw new Error("BANK_RETRIEVE_PROD_TX_INVENTORY_TOKEN_FEHLT");
      }
      inventoryToken = ersterInventoryToken;
      ...
      if (!d.leaseController.validiereMutation(
        ...
        throw new Error("BANK_RETRIEVE_PROD_TX_MUTATIONS_FENCE_UNGUELTIG");
      }
      if (!d.leaseController.validiereSnapshot(
        ...
        throw new Error("BANK_RETRIEVE_PROD_TX_SNAPSHOT_NICHT_FREIGEGEBEN");
      }
      ...
      const intentEintrag = journalEintrag(
        ...
        a.ausgestelltAmMs,
        {
          ...
          faehigkeit_id: MERCHANT_BANK_RETRIEVE_FAEHIGKEIT_ID,
          owner_id: MERCHANT_BANK_CORE_MODUL_ID,
          action_contract_id: BANK_RETRIEVE_ACTION_CONTRACT_ID,
          recovery_contract_id: BANK_RETRIEVE_RECOVERY_CONTRACT_ID,
          verifier_id: BANK_RETRIEVE_VERIFIER_ID,
          ...
          direction: "RETRIEVE",
          pack: a.pack,
          bank_slot: a.bankSlot,
          inventory_slot: a.inventorySlot,
          transfer_item_fingerprint: a.vorher.transferItem.fingerprint,
          pack_rest_fingerprint: a.vorher.packRestFingerprint,
          inventory_rest_fingerprint: a.vorher.inventoryRestFingerprint,
          ...
          resource_claims_and_fencing: Object.freeze([
            ...
            Object.freeze({
              ressourcenId: inventoryToken.ressourcenId,
              epoche: inventoryToken.epoche,
            }),
            Object.freeze({
      ...
      try {
        freigabe = await ErteilteAusfuehrungsFreigabe.erteile({
          ...
          verifierId: BANK_RETRIEVE_VERIFIER_ID,
          invariantenKennungen: BANK_RETRIEVE_SHADOW_INVARIANTEN,
          voraussetzungsIds: BANK_RETRIEVE_PRODUKTIV_LIVE_VORAUSSETZUNGEN,
      ...
      try {
        transport = await d.ausfuehrung.fuehreAus(
          ...
          Object.freeze({
            ...
            pack: a.pack,
            bankSlot: a.bankSlot,
            inventorySlot: a.inventorySlot,
            accountId: a.accountId,
            characterId: a.characterId,
            sessionId: a.sessionId,
            serverRegion: a.serverRegion,
            serverIdentifier: a.serverIdentifier,
            erwartetesItemName: a.vorher.transferItem.name,
            erwartetesItemFingerprint: a.vorher.transferItem.fingerprint,
            erwarteterPackRestFingerprint: a.vorher.packRestFingerprint,
            erwarteterInventoryRestFingerprint: a.vorher.inventoryRestFingerprint,
      ...
      if (inventoryToken !== null) {
        try {
          d.ressourcen.gibFrei(inventoryToken, d.jetztMs());
        } catch {
          // Fail-closed bei spaeterer Fencing-Pruefung.
        }
      }
