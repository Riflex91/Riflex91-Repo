...
export const BANK_STORE_ACTION_CONTRACT_ID = "AL-ACTION-BANK-SWAP";
export const BANK_STORE_RECOVERY_CONTRACT_ID = "AL-RECOVERY-BANK-SWAP";
export const BANK_STORE_VERIFIER_ID = "AL-VERIFIER-BANK-SWAP";
export const BANK_STORE_EINMAL_POLICY_ID = "BANK-SWAP-PRODUKTION-EINMAL-V1";
...
export async function erteileProduktiveBankStoreEinmalAuthority(
  anforderung: V5BankStoreEinmalAuthorityAnforderung,
  protokoll: V5BankStoreEinmalAuthorityProtokollPort | null,
  ...
  if (!anforderung || anforderung.schemaVersion !== 1
    ...
    return blockiert("V5_BANK_STORE_EINMAL_BINDUNG_UNGUELTIG");
  }
  if (!Number.isSafeInteger(anforderung.jetztMs)
    ...
    return blockiert("V5_BANK_STORE_EINMAL_ZEIT_ODER_GENERATION_UNGUELTIG");
  }
  const ids = evidenceIds(anforderung.healthEvidence, anforderung.jetztMs);
  if (ids === null) return blockiert("V5_BANK_STORE_EINMAL_HEALTH_EVIDENCE_UNGUELTIG");
  if (protokoll === null) {
    return blockiert("V5_BANK_STORE_EINMAL_DURABLE_PROTOKOLL_FEHLT", ids);
  }
  ...
  let ack: V5BankStoreEinmalAuthorityDurableBestaetigung;
  try {
    ack = await protokoll.schreibeDurable(intent);
  } catch {
    return blockiert("V5_BANK_STORE_EINMAL_AUDIT_NICHT_DURABLE", ids);
  }
  if (ack.durable !== true
      || ack.aktivierungsId !== anforderung.aktivierungsId
      || ack.transaktionsId !== anforderung.transaktionsId
      || !text(ack.bestaetigungsId)) {
    return blockiert("V5_BANK_STORE_EINMAL_DURABILITY_NICHT_BESTAETIGT", ids);
  }
