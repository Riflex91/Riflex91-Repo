...
export const BANK_WITHDRAW_SOCKET_BUDGET_GEWICHT = 20;
export const BANK_WITHDRAW_SHADOW_INVARIANTEN = Object.freeze([
  "V5-ALT-002",
  "V5-ALT-003",
  "V5-INV-002",
  "V5-INV-004",
...
export const BANK_WITHDRAW_SHADOW_LIVE_VORAUSSETZUNGEN = Object.freeze([
  "character",
  "bank",
  "inventory",
] as const);

export interface BankWithdrawShadowSnapshot {
...
export interface BankWithdrawShadowAnforderung {
  readonly schemaVersion: 1;
  ...
  readonly snapshot: BankWithdrawShadowSnapshot;
  readonly authority: ProduktiveBankWithdrawEinmalAuthority;
}

export interface BankWithdrawShadowReleaseBeobachterPort {
  beobachte(
...
export interface BankWithdrawShadowAbhaengigkeiten {
  ...
  readonly releaseBeobachter: BankWithdrawShadowReleaseBeobachterPort;
  readonly vorabLeaseToken?: BankLeaseToken;
  readonly jetztMs: () => number;
}

export interface BankWithdrawShadowErgebnis {
  readonly schemaVersion: 1;
...
function validiereAnforderung(a: BankWithdrawShadowAnforderung): void {
  if (a.schemaVersion !== 1) {
    throw new Error("BANK_WITHDRAW_SHADOW_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [a.freigabeId, "BANK_WITHDRAW_SHADOW_FREIGABE_ID_UNGUELTIG"],
    [a.auftragId, "BANK_WITHDRAW_SHADOW_AUFTRAG_ID_UNGUELTIG"],
    [a.ablaufId, "BANK_WITHDRAW_SHADOW_ABLAUF_ID_UNGUELTIG"],
    [a.transaktionsId, "BANK_WITHDRAW_SHADOW_TX_ID_UNGUELTIG"],
    [a.accountId, "BANK_WITHDRAW_SHADOW_ACCOUNT_ID_UNGUELTIG"],
    [a.characterId, "BANK_WITHDRAW_SHADOW_CHARACTER_ID_UNGUELTIG"],
    [a.serverRegion, "BANK_WITHDRAW_SHADOW_SERVER_REGION_UNGUELTIG"],
    [a.serverIdentifier, "BANK_WITHDRAW_SHADOW_SERVER_ID_UNGUELTIG"],
    [a.snapshot.fingerprint, "BANK_WITHDRAW_SHADOW_SNAPSHOT_FP_UNGUELTIG"],
  ] as const) {
    pruefeText(wert, fehler);
  }
  pruefeZeit(
    a.ausgestelltAmMs,
    "BANK_WITHDRAW_SHADOW_AUSSTELLZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.gueltigBisMs,
    "BANK_WITHDRAW_SHADOW_GUELTIGKEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.externalFenceBeobachtetAmMs,
    "BANK_WITHDRAW_SHADOW_FENCE_ZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.snapshot.beobachtetAmMs,
    "BANK_WITHDRAW_SHADOW_SNAPSHOT_ZEIT_UNGUELTIG",
  );
  if (a.gueltigBisMs < a.ausgestelltAmMs
    ...
    throw new Error("BANK_WITHDRAW_SHADOW_ZEITFENSTER_UNGUELTIG");
  }
  if (a.externalFenceBeobachtetAmMs > a.ausgestelltAmMs
      || a.ausgestelltAmMs - a.externalFenceBeobachtetAmMs > 1_000) {
    throw new Error("BANK_WITHDRAW_SHADOW_EXTERNAL_FENCE_STALE");
  }
  if (a.snapshot.schemaVersion !== 1
    ...
    throw new Error("BANK_WITHDRAW_SHADOW_SNAPSHOT_BINDUNG_UNGUELTIG");
  }
  if (a.externalFence.konflikt
    ...
    throw new Error("BANK_WITHDRAW_SHADOW_EXTERNAL_FENCE_UNGUELTIG");
  }
}
...
function journalEintrag(
  a: BankWithdrawShadowAnforderung,
  sequenz: number,
...
export class ProduktiveBankWithdrawShadowAdmission {
  public async pruefe(
    a: BankWithdrawShadowAnforderung,
    d: BankWithdrawShadowAbhaengigkeiten,
    ...
    try {
      if (d.vorabLeaseToken !== undefined) {
        ...
        if (token.accountId !== a.accountId
          ...
          throw new Error("BANK_WITHDRAW_SHADOW_VORAB_LEASE_BINDUNG_UNGUELTIG");
        }
        ...
        if (sichtbar?.zustand !== "ACQUIRING") {
          throw new Error("BANK_WITHDRAW_SHADOW_VORAB_LEASE_NICHT_ACQUIRING");
        }
        leaseToken = token;
      } else {
      ...
      if (lease.zustand !== "ACTIVE") {
        throw new Error("BANK_WITHDRAW_SHADOW_LEASE_NICHT_ACTIVE");
      }
      ...
      if (!d.leaseController.validiereMutation(
        ...
        throw new Error("BANK_WITHDRAW_SHADOW_MUTATIONS_FENCE_UNGUELTIG");
      }
      if (!d.leaseController.validiereSnapshot(
        ...
        throw new Error("BANK_WITHDRAW_SHADOW_SNAPSHOT_NICHT_FREIGEGEBEN");
      }
      ...
      const freigabe = await ErteilteAusfuehrungsFreigabe.erteile({
        ...
        verifierId: BANK_WITHDRAW_VERIFIER_ID,
        invariantenKennungen: BANK_WITHDRAW_SHADOW_INVARIANTEN,
        voraussetzungsIds: BANK_WITHDRAW_SHADOW_LIVE_VORAUSSETZUNGEN,
