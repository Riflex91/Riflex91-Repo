...
import {
  ...
  type BankWithdrawBindung,
} from "./bank-withdraw-settlement.js";
import {
...
import {
  BANK_WITHDRAW_SHADOW_INVARIANTEN,
  BANK_WITHDRAW_SOCKET_BUDGET_GEWICHT,
} from "./bank-withdraw-shadow-admission.js";

export const BANK_WITHDRAW_PRODUKTIV_LIVE_VORAUSSETZUNGEN = Object.freeze([
  "character",
  "bank",
  "inventory",
] as const);

export interface BankWithdrawEinGoldAdapterAnfrage {
  readonly betrag: 1;
...
export interface ProduktiverBankWithdrawBeobachterPort {
  beobachte(
    leaseEpoche: number,
    mountEpoche: number,
  ): Promise<BankWithdrawBindung>;
}

export interface ProduktiveBankWithdrawTransaktionsAnforderung {
  ...
  readonly vorher: BankWithdrawBindung;
...
export interface ProduktiveBankWithdrawTransaktionsAbhaengigkeiten<Ergebnis> {
  ...
  readonly adapter: AusfuehrungsAdapter<
    BankWithdrawEinGoldAdapterAnfrage,
    Ergebnis,
    ErteilteAusfuehrungsFreigabe
  >;
  readonly bankBeobachter: ProduktiverBankWithdrawBeobachterPort;
...
export interface ProduktiveBankWithdrawTransaktionsErgebnis {
  readonly schemaVersion: 1;
  readonly transaktionsId: string;
  readonly status:
    | "COMMITTED"
...
function validiereAnforderung(
  a: ProduktiveBankWithdrawTransaktionsAnforderung,
): void {
  if (a.schemaVersion !== 1) {
    throw new Error("BANK_WITHDRAW_PROD_TX_SCHEMA_UNGUELTIG");
  }
  for (const [wert, fehler] of [
    [a.freigabeId, "BANK_WITHDRAW_PROD_TX_FREIGABE_ID_UNGUELTIG"],
    [a.auftragId, "BANK_WITHDRAW_PROD_TX_AUFTRAG_ID_UNGUELTIG"],
    [a.ablaufId, "BANK_WITHDRAW_PROD_TX_ABLAUF_ID_UNGUELTIG"],
    [a.transaktionsId, "BANK_WITHDRAW_PROD_TX_ID_UNGUELTIG"],
    [a.accountId, "BANK_WITHDRAW_PROD_TX_ACCOUNT_ID_UNGUELTIG"],
    [a.characterId, "BANK_WITHDRAW_PROD_TX_CHARACTER_ID_UNGUELTIG"],
    [a.sessionId, "BANK_WITHDRAW_PROD_TX_SESSION_ID_UNGUELTIG"],
    [a.serverRegion, "BANK_WITHDRAW_PROD_TX_SERVER_REGION_UNGUELTIG"],
    [a.serverIdentifier, "BANK_WITHDRAW_PROD_TX_SERVER_ID_UNGUELTIG"],
    [a.configFingerprint, "BANK_WITHDRAW_PROD_TX_CONFIG_FP_UNGUELTIG"],
    [a.prestateFingerprint, "BANK_WITHDRAW_PROD_TX_PRESTATE_FP_UNGUELTIG"],
  ] as const) {
    pruefeText(wert, fehler);
  }
  if (a.betrag !== BANK_WITHDRAW_ERSTER_BETRAG) {
    throw new Error("BANK_WITHDRAW_PROD_TX_BETRAG_NICHT_EXAKT_EINS");
  }
  pruefeZeit(a.ausgestelltAmMs, "BANK_WITHDRAW_PROD_TX_ZEIT_UNGUELTIG");
  pruefeZeit(
    a.gueltigBisMs,
    "BANK_WITHDRAW_PROD_TX_ABLAUFZEIT_UNGUELTIG",
  );
  pruefeZeit(
    a.externalFenceBeobachtetAmMs,
    "BANK_WITHDRAW_PROD_TX_FENCE_ZEIT_UNGUELTIG",
  );
  if (a.gueltigBisMs < a.ausgestelltAmMs
    ...
    throw new Error("BANK_WITHDRAW_PROD_TX_ZEITFENSTER_UNGUELTIG");
  }
  if (a.externalFenceBeobachtetAmMs > a.ausgestelltAmMs
      || a.ausgestelltAmMs - a.externalFenceBeobachtetAmMs > 1_000) {
    throw new Error("BANK_WITHDRAW_PROD_TX_FENCE_STALE");
  }
  const v = a.vorher;
  if (v.characterId !== a.characterId
      ...
      || v.bankGold < 1) {
    throw new Error("BANK_WITHDRAW_PROD_TX_PRESTATE_BINDUNG_UNGUELTIG");
  }
  if (a.externalFence.konflikt
    ...
    throw new Error("BANK_WITHDRAW_PROD_TX_EXTERNAL_FENCE_UNGUELTIG");
  }
  if (!/^[0-9a-f]{40}$/i.test(a.wissensSnapshot.gitCommit)
    ...
    throw new Error("BANK_WITHDRAW_PROD_TX_WISSENSSNAPSHOT_UNGUELTIG");
  }
  const authority = a.authority.daten();
  if (authority.transaktionsId !== a.transaktionsId
      || authority.faehigkeitId !== MERCHANT_BANK_WITHDRAW_FAEHIGKEIT_ID
      || authority.anbieterModulId !== MERCHANT_BANK_CORE_MODUL_ID
      || authority.actionContractId !== BANK_WITHDRAW_ACTION_CONTRACT_ID
      || authority.recoveryContractId !== BANK_WITHDRAW_RECOVERY_CONTRACT_ID
      || authority.verifierId !== BANK_WITHDRAW_VERIFIER_ID
      || a.gueltigBisMs > authority.gueltigBisMs
      || !a.authority.gueltigFuer(a.ausgestelltAmMs)) {
    throw new Error("BANK_WITHDRAW_PROD_TX_AUTHORITY_BINDUNG_UNGUELTIG");
  }
}
...
function actionVertrag() {
  return Object.freeze({
    pruefe(
      ...
      const exakt = actionContractId === BANK_WITHDRAW_ACTION_CONTRACT_ID
      ...
      return Object.freeze({
        ...
        invariantenKennungen: BANK_WITHDRAW_SHADOW_INVARIANTEN,
      });
    },
  });
}
...
function statusFuer(
  recovery: RecoveryAbschluss,
): ProduktiveBankWithdrawTransaktionsErgebnis["status"] {
  switch (recovery.art) {
...
function liveVoraussetzungen(
  a: ProduktiveBankWithdrawTransaktionsAnforderung,
  beobachter: ProduktiverBankWithdrawBeobachterPort,
) {
  return Object.freeze({
    async pruefe(ids: readonly string[], jetztMs: number) {
      if (ids.length !== BANK_WITHDRAW_PRODUKTIV_LIVE_VORAUSSETZUNGEN.length
          || ids.some(id =>
            !BANK_WITHDRAW_PRODUKTIV_LIVE_VORAUSSETZUNGEN.includes(
              id as typeof BANK_WITHDRAW_PRODUKTIV_LIVE_VORAUSSETZUNGEN[number],
            ))) {
        throw new Error("BANK_WITHDRAW_PROD_TX_LIVE_IDS_UNGUELTIG");
      }
      ...
      if (b.characterId !== a.vorher.characterId
        ...
        throw new Error("BANK_WITHDRAW_PROD_TX_LIVE_PRESTATE_DRIFT");
      }
  ...
  return Object.freeze({
    async beobachte(
      ...
      try {
        ...
        const settlement = pruefeBankWithdrawEinGoldSettlement(
          a.vorher,
          nachher,
        );
        let klassifikation:
...
export class ProduktiveBankWithdrawTransaktionsOrchestrierung {
  public async fuehreEinmalAus<Ergebnis>(
    a: ProduktiveBankWithdrawTransaktionsAnforderung,
    d: ProduktiveBankWithdrawTransaktionsAbhaengigkeiten<Ergebnis>,
  ): Promise<ProduktiveBankWithdrawTransaktionsErgebnis> {
    validiereAnforderung(a);
    if (d.adapter.actionContractId !== BANK_WITHDRAW_ACTION_CONTRACT_ID
        || d.adapter.recoveryContractId !== BANK_WITHDRAW_RECOVERY_CONTRACT_ID
        || d.adapter.verifierId !== BANK_WITHDRAW_VERIFIER_ID) {
      throw new Error("BANK_WITHDRAW_PROD_TX_ADAPTER_BINDUNG_UNGUELTIG");
    }
    ...
    try {
      if (d.vorabLeaseToken !== undefined) {
        ...
        if (leaseToken.accountId !== a.accountId
          ...
          throw new Error("BANK_WITHDRAW_PROD_TX_VORAB_LEASE_DRIFT");
        }
        ...
        if (sichtbar?.zustand !== "ACQUIRING") {
          throw new Error("BANK_WITHDRAW_PROD_TX_VORAB_LEASE_NICHT_ACQUIRING");
        }
      } else {
      ...
      if (lease.zustand !== "ACTIVE") {
        throw new Error("BANK_WITHDRAW_PROD_TX_LEASE_NICHT_ACTIVE");
      }
      ...
      if (ersterGoldToken === undefined) {
        throw new Error("BANK_WITHDRAW_PROD_TX_GOLD_TOKEN_FEHLT");
      }
      ...
      if (!d.leaseController.validiereMutation(
        ...
        throw new Error("BANK_WITHDRAW_PROD_TX_MUTATIONS_FENCE_UNGUELTIG");
      }
      if (!d.leaseController.validiereSnapshot(
        ...
        throw new Error("BANK_WITHDRAW_PROD_TX_SNAPSHOT_NICHT_FREIGEGEBEN");
      }
      ...
      const intentEintrag = journalEintrag(
        ...
        a.ausgestelltAmMs,
        {
          ...
          faehigkeit_id: MERCHANT_BANK_WITHDRAW_FAEHIGKEIT_ID,
          owner_id: MERCHANT_BANK_CORE_MODUL_ID,
          action_contract_id: BANK_WITHDRAW_ACTION_CONTRACT_ID,
          recovery_contract_id: BANK_WITHDRAW_RECOVERY_CONTRACT_ID,
          verifier_id: BANK_WITHDRAW_VERIFIER_ID,
      ...
      try {
        freigabe = await ErteilteAusfuehrungsFreigabe.erteile({
          ...
          verifierId: BANK_WITHDRAW_VERIFIER_ID,
          invariantenKennungen: BANK_WITHDRAW_SHADOW_INVARIANTEN,
          voraussetzungsIds: BANK_WITHDRAW_PRODUKTIV_LIVE_VORAUSSETZUNGEN,
