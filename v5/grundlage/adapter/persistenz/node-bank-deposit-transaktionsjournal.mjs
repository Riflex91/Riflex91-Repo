...
const BASIS = "runtime/transactions/bank-deposit";
...
function validiereEintrag(e) {
  if (e === null || typeof e !== "object"
    ...
    throw new Error("BANK_DEPOSIT_TX_EINTRAG_UNGUELTIG");
  }
  text(e.journalId, 160, "BANK_DEPOSIT_TX_JOURNAL_ID_UNGUELTIG");
  text(e.transaktionsId, 128, "BANK_DEPOSIT_TX_ID_UNGUELTIG");
  if (e.sequenz === 1 && e.art !== "INTENT") {
    throw new Error("BANK_DEPOSIT_TX_INTENT_ERFORDERLICH");
  }
  if (e.sequenz > 1 && e.art === "INTENT") {
    throw new Error("BANK_DEPOSIT_TX_INTENT_POSITION_UNGUELTIG");
  }
...
function validiereState(s, txId) {
  if (s === null || typeof s !== "object"
    ...
    throw new Error("BANK_DEPOSIT_TX_STATE_UNGUELTIG");
  }
  let seq = 1;
  let terminal = false;
...
export class NodeBankDepositTransaktionsJournal {
  #fs;
  ...
  constructor(dateisystem) {
    if (!dateisystem
      ...
      throw new Error("BANK_DEPOSIT_TX_DATEISYSTEM_UNGUELTIG");
    }
    this.#fs = dateisystem;
  }
  ...
  async haengeDurableAn(eintragWert) {
    ...
    const txSafe = safe(e.transaktionsId, "BANK_DEPOSIT_TX_ID_UNGUELTIG");
    const statePath = BASIS + "/" + txSafe + "/state.json";
    const current = parse(
      await this.#fs.liesText(CURRENT),
      "BANK_DEPOSIT_TX_CURRENT_UNGUELTIG",
    );
    ...
    if (e.sequenz === 1) {
      ...
      if (current === undefined || current.status === "TERMINAL") {
        await this.#fs.schreibeAtomarDurable(
          ...
          "bank-deposit-current-" + txSafe,
        );
      }
    } else if (current?.status !== "OFFEN"
        || current.transaktionsId !== e.transaktionsId) {
      throw new Error("BANK_DEPOSIT_TX_CURRENT_BINDUNG_UNGUELTIG");
    }

    let state = parse(
      await this.#fs.liesText(statePath),
      "BANK_DEPOSIT_TX_STATE_UNGUELTIG",
    );
    if (state === undefined) {
      if (e.sequenz !== 1) throw new Error("BANK_DEPOSIT_TX_STATE_FEHLT");
      state = {
        schemaVersion: 1,
        transaktionsId: e.transaktionsId,
        status: "OFFEN",
    ...
    if (vorhanden !== undefined) {
      if (vorhanden.art !== e.art
        ...
        throw new Error("BANK_DEPOSIT_TX_JOURNAL_KOLLISION");
      }
      return this.#ack(e, name);
    }
    if (state.status === "TERMINAL") {
      throw new Error("BANK_DEPOSIT_TX_NACH_TERMINAL");
    }
    if (e.sequenz !== state.eintraege.length + 1) {
      throw new Error("BANK_DEPOSIT_TX_SEQUENZ_LUECKE");
    }
    ...
    await this.#fs.schreibeAtomarDurable(
      statePath,
      JSON.stringify(next) + "\n",
      "bank-deposit-state-" + txSafe + "-" + e.sequenz,
    );
    await this.#fs.schreibeAtomarDurable(
      CURRENT,
      JSON.stringify({
      ...
      "bank-deposit-current-" + txSafe + "-" + e.sequenz,
    );
    return this.#ack(e, name);
  }
  ...
  async pruefeStartBereit() {
    ...
    if (current.status !== "OFFEN"
      ...
      throw new Error("BANK_DEPOSIT_TX_CURRENT_UNGUELTIG");
    }
    return Object.freeze({
      bereit: false,
  ...
  async liesTransaktion(transaktionsId) {
    const txSafe = safe(transaktionsId, "BANK_DEPOSIT_TX_ID_UNGUELTIG");
    const state = parse(
    ...
    for (const meta of state.eintraege) {
      ...
      if (raw === undefined) throw new Error("BANK_DEPOSIT_TX_DATEI_FEHLT");
      const e = validiereEintrag(
        parse(raw, "BANK_DEPOSIT_TX_EINTRAG_UNGUELTIG"),
      );
  ...
  #ack(e, name) {
    return Object.freeze({
      durable: true,
      bestaetigungsId: "BANK-DEPOSIT-TX:" + name,
      journalId: e.journalId,
      transaktionsId: e.transaktionsId,
      sequenz: e.sequenz,
    });
