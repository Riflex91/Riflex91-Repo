...
const BASIS = "runtime/transactions/merchant-mluck";
...
function validateEntry(e) {
  if (e === null || typeof e !== "object"
    ...
    throw new Error("MLUCK_TX_JOURNAL_EINTRAG_UNGUELTIG");
  }
  text(e.journalId, 160, "MLUCK_TX_JOURNAL_ID_UNGUELTIG");
  text(e.transaktionsId, 128, "MLUCK_TX_ID_UNGUELTIG");
  if (e.sequenz === 1 && e.art !== "INTENT") {
    throw new Error("MLUCK_TX_JOURNAL_INTENT_ERFORDERLICH");
  }
  if (e.sequenz > 1 && e.art === "INTENT") {
    throw new Error("MLUCK_TX_JOURNAL_INTENT_POSITION_UNGUELTIG");
  }
...
function validateState(s, txId) {
  if (s === null || typeof s !== "object"
    ...
    throw new Error("MLUCK_TX_JOURNAL_STATE_UNGUELTIG");
  }
  let seq = 1;
  let terminal = false;
...
export class NodeMluckTransaktionsJournal {
  #fs;
  ...
  constructor(dateisystem) {
    if (dateisystem === null
      ...
      throw new Error("MLUCK_TX_JOURNAL_DATEISYSTEM_UNGUELTIG");
    }
    this.#fs = dateisystem;
  }
  ...
  async haengeDurableAn(eintragWert) {
    ...
    if (e.sequenz === 1) {
      ...
      if (current === undefined || current.status === "TERMINAL") {
        await this.#fs.schreibeAtomarDurable(
          ...
          "mluck-current-" + txSafe,
        );
      }
    } else if (current?.status !== "OFFEN"
        || current.transaktionsId !== e.transaktionsId) {
      throw new Error("MLUCK_TX_CURRENT_BINDUNG_UNGUELTIG");
    }

    let state = parse(
      await this.#fs.liesText(statePath),
      "MLUCK_TX_JOURNAL_STATE_UNGUELTIG",
    );
    if (state === undefined) {
      if (e.sequenz !== 1) throw new Error("MLUCK_TX_JOURNAL_STATE_FEHLT");
      state = {
        schemaVersion: 1,
        transaktionsId: e.transaktionsId,
        status: "OFFEN",
    ...
    if (existingMeta !== undefined) {
      if (existingMeta.art !== e.art) {
        throw new Error("MLUCK_TX_JOURNAL_SEQUENZ_KOLLISION");
      }
      const existing = await this.#fs.liesText(entryPath);
      if (existing !== json) throw new Error("MLUCK_TX_JOURNAL_INHALT_KOLLISION");
      return this.#ack(e, name);
    }

    if (state.status === "TERMINAL") {
      throw new Error("MLUCK_TX_JOURNAL_NACH_TERMINAL");
    }
    if (e.sequenz !== state.eintraege.length + 1) {
      throw new Error("MLUCK_TX_JOURNAL_SEQUENZ_LUECKE");
    }

    const created = await this.#fs.erstelleExklusivDurable(entryPath, json);
    if (!created) {
      const race = await this.#fs.liesText(entryPath);
      if (race !== json) throw new Error("MLUCK_TX_JOURNAL_INHALT_KOLLISION");
    }
    ...
    await this.#fs.schreibeAtomarDurable(
      ...
      "mluck-current-" + txSafe + "-" + e.sequenz,
    );
    return this.#ack(e, name);
  }

  async liesTransaktion(transaktionsId) {
    const txSafe = safe(transaktionsId, "MLUCK_TX_ID_UNGUELTIG");
    const statePath = BASIS + "/" + txSafe + "/state.json";
    const state = parse(
      await this.#fs.liesText(statePath),
      "MLUCK_TX_JOURNAL_STATE_UNGUELTIG",
    ...
    for (const meta of state.eintraege) {
      ...
      if (raw === undefined) throw new Error("MLUCK_TX_JOURNAL_DATEI_FEHLT");
      const e = validateEntry(parse(raw, "MLUCK_TX_JOURNAL_EINTRAG_UNGUELTIG"));
      if (e.transaktionsId !== transaktionsId
          || e.sequenz !== meta.sequenz
          || e.art !== meta.art) {
        throw new Error("MLUCK_TX_JOURNAL_STATE_WIDERSPRUCH");
      }
  ...
  async pruefeStartBereit() {
    ...
    if (current.status !== "OFFEN"
      ...
      throw new Error("MLUCK_TX_CURRENT_UNGUELTIG");
    }
    return Object.freeze({
      bereit: false,
