...
function normalisiere(intent) {
  if (intent === null || typeof intent !== "object"
      ...
      || intent.art !== "BANK_SWAP_EINMAL_AUTHORITY_VOR_WIRKUNG"
      || intent.faehigkeitId !== "merchant.bank.intern_tauschen"
      || intent.anbieterModulId !== "merchant-bank-core"
      || intent.anbieterVersion !== "1"
      || intent.actionContractId !== "AL-ACTION-BANK-SWAP"
      || intent.recoveryContractId !== "AL-RECOVERY-BANK-SWAP"
      || intent.verifierId !== "AL-VERIFIER-BANK-SWAP"
      || intent.policyId !== "BANK-SWAP-PRODUKTION-EINMAL-V1"
    ...
    throw new Error("BANK_SWAP_AUTHORITY_AUDIT_FORMAT_UNGUELTIG");
  }
  text(intent.aktivierungsId, 128, "BANK_SWAP_AUTHORITY_ID_UNGUELTIG");
  text(intent.transaktionsId, 128, "BANK_SWAP_AUTHORITY_TX_UNGUELTIG");
  const evidenceIds = Object.freeze([...intent.evidenceIds].sort());
  for (let i = 0; i < evidenceIds.length; i += 1) {
    text(evidenceIds[i], 192, "BANK_SWAP_AUTHORITY_EVIDENCE_UNGUELTIG");
    if (i > 0 && evidenceIds[i] === evidenceIds[i - 1]) {
      throw new Error("BANK_SWAP_AUTHORITY_EVIDENCE_DOPPELT");
    }
  }
...
export class NodeBankSwapEinmalAuthorityProtokoll {
  #dateisystem;
  ...
  constructor(dateisystem) {
    if (dateisystem === null
      ...
      throw new Error("BANK_SWAP_AUTHORITY_DATEISYSTEM_UNGUELTIG");
    }
  ...
  async schreibeDurable(intentWert) {
    ...
    const pfad = "runtime/authority/mutieren/bank-swap/"
      + safe(intent.aktivierungsId, "BANK_SWAP_AUTHORITY_ID_UNGUELTIG")
      + ".json";
    const json = JSON.stringify(intent) + "\n";
    const vorhanden = await this.#dateisystem.liesText(pfad);
    if (vorhanden !== undefined) {
      if (vorhanden !== json) {
        throw new Error("BANK_SWAP_AUTHORITY_AUDIT_ID_KOLLISION");
      }
