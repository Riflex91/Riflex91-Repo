...
function normalisiere(intent) {
  if (intent === null || typeof intent !== "object"
      ...
      || intent.art !== "BANK_WITHDRAW_EINMAL_AUTHORITY_VOR_WIRKUNG"
      || intent.faehigkeitId !== "merchant.bank.gold_auslagern"
    ...
    throw new Error("BANK_WITHDRAW_AUTHORITY_AUDIT_FORMAT_UNGUELTIG");
  }
  text(intent.aktivierungsId, 128, "BANK_WITHDRAW_AUTHORITY_ID_UNGUELTIG");
  text(intent.transaktionsId, 128, "BANK_WITHDRAW_AUTHORITY_TX_UNGUELTIG");
  const evidenceIds = Object.freeze([...intent.evidenceIds].sort());
  for (let i = 0; i < evidenceIds.length; i += 1) {
    text(evidenceIds[i], 192, "BANK_WITHDRAW_AUTHORITY_EVIDENCE_UNGUELTIG");
    if (i > 0 && evidenceIds[i] === evidenceIds[i - 1]) {
      throw new Error("BANK_WITHDRAW_AUTHORITY_EVIDENCE_DOPPELT");
    }
  }
...
export class NodeBankWithdrawEinmalAuthorityProtokoll {
  #dateisystem;
  ...
  constructor(dateisystem) {
    if (dateisystem === null
      ...
      throw new Error("BANK_WITHDRAW_AUTHORITY_DATEISYSTEM_UNGUELTIG");
    }
  ...
  async schreibeDurable(intentWert) {
    ...
    const pfad = "runtime/authority/mutieren/bank-withdraw/"
      + safe(intent.aktivierungsId, "BANK_WITHDRAW_AUTHORITY_ID_UNGUELTIG")
      + ".json";
    const json = JSON.stringify(intent) + "\n";
    const vorhanden = await this.#dateisystem.liesText(pfad);
    if (vorhanden !== undefined) {
      if (vorhanden !== json) {
        throw new Error("BANK_WITHDRAW_AUTHORITY_AUDIT_ID_KOLLISION");
      }
