...
test("PR20.9 manual development override advances roadmap without inventing Craft evidence",()=>{
  ...
  assert.equal(roadmap.pr21.status,"APPLY_EXECUTION_AUTHORIZATION_PREPARED_RECORD_ONLY");
  ...
  assert.equal(roadmap.pr21.applyExecutionAuthorizationBoundary.status,"PREPARED_EXPLICIT_ONE_SHOT_AUTHORIZATION_RECORD_ONLY");
...
test("Merchant and PR21 contracts accept only the development override, never runtime authority",()=>{
  ...
  assert.equal(
    ...
    "APPLY_EXECUTION_AUTHORIZATION_PREPARED_RECORD_ONLY",
  );
  ...
  assert.equal(foundations.stages.pr21.applyExecutionAuthorizationBoundary.status,"PREPARED_EXPLICIT_ONE_SHOT_AUTHORIZATION_RECORD_ONLY");
