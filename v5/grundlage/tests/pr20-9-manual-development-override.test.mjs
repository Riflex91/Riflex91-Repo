...
test("PR20.9 manual development override advances roadmap without inventing Craft evidence",()=>{
  ...
  assert.equal(roadmap.pr21.status,"DEFAULT_OFF_GATE_APPLY_BOUNDARY_PREPARED_NO_APPLY");
  ...
  assert.equal(roadmap.pr21.defaultOffGateApplyBoundary.status,"PREPARED_DEFAULT_OFF_NO_APPLY");
  assert.equal(roadmap.pr21.defaultOffGateApplyBoundary.executionEnabled,false);
});
...
test("Merchant and PR21 contracts accept only the development override, never runtime authority",()=>{
  ...
  assert.equal(
    ...
    "DEFAULT_OFF_GATE_APPLY_BOUNDARY_PREPARED_NO_APPLY",
  );
  ...
  assert.equal(foundations.stages.pr21.defaultOffGateApplyBoundary.status,"PREPARED_DEFAULT_OFF_NO_APPLY");
  assert.equal(foundations.stages.pr21.defaultOffGateApplyBoundary.executionEnabled,false);
});
