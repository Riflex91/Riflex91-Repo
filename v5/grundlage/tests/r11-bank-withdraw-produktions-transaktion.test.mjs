...
test("bank_withdraw(1) committed nur ueber durable Intent, Admission, Execution und exaktes Gold-Settlement", async () => {
  const e = await env();
  const auth = e.requestAuth;
  const result = await new ProduktiveBankWithdrawTransaktionsOrchestrierung()
...
test("Operator-Deny blockiert nach durable Intent vor Adapter-Send", async () => {
  ...
  await assert.rejects(
    () => new ProduktiveBankWithdrawTransaktionsOrchestrierung()
      .fuehreEinmalAus(
        request(e.requestAuth, e.leaseToken.epoche),
        e.deps,
      ),
