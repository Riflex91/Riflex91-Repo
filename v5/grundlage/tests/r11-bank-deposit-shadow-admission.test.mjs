...
test("Bank-Deposit-Shadow durchlaeuft Lease, Channel, Budget, Intent und R9-Admission ohne Send", async () => {
  const env = umgebung();
  const ergebnis = await new ProduktiveBankDepositShadowAdmission().pruefe(
    anfrage(),
    env,
  );
...
test("External-Fence-Drift blockiert vor Intent und vor Authority-Verbrauch", async () => {
  ...
  await assert.rejects(
    () => new ProduktiveBankDepositShadowAdmission().pruefe(a, env),
    /BANK_DEPOSIT_SHADOW_EXTERNAL_FENCE_UNGUELTIG/,
  );
...
test("stale oder unvollstaendige Live-Evidence blockiert nach durable Intent aber vor Send", async () => {
  ...
  await assert.rejects(
    () => new ProduktiveBankDepositShadowAdmission().pruefe(
      anfrage(),
      env,
    ),
