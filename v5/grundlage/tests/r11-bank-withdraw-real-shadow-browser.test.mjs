...
import {
  BANK_WITHDRAW_REAL_SHADOW_BROWSER_READ_ONLY,
...
test("Real-Browser-Shadow startet nur ausserhalb der Bank und bleibt read-only", () => {
  assert.equal(BANK_WITHDRAW_REAL_SHADOW_BROWSER_READ_ONLY, true);
  assert.equal(BANK_WITHDRAW_REAL_SHADOW_GAMEPLAY_WRITES, 0);
  const start = validiereBankWithdrawShadowAusgangsBeobachtung(observation());
...
test("Release-Beobachter gibt Lease erst nach stabilem manuellen Bank-Exit frei", async () => {
  ...
  const phases = [];
  const observer = erstelleBankWithdrawShadowReleaseBeobachter(
    fakeSession([
      observation({
        map: "bank",
        bankGemountet: true,
