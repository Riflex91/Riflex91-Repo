...
import {
  ...
  erstelleBankDepositEinGoldVorherBindung,
  erstelleProduktivenBankDepositBeobachter,
} from "../../werkzeuge/bank-deposit-produktions-write-browser.mjs";
...
test("Bank-Deposit-Write-Adapter enthaelt exakt einen erlaubten Public-Function-Write und keinen Raw-Socket-Bypass", () => {
  const source = fs.readFileSync(
    "werkzeuge/bank-deposit-produktions-write-browser.mjs",
    "utf8",
  );
  assert.equal((source.match(/root\.bank_deposit\(1\)/g) ?? []).length, 1);
  for (const muster of [
    /\.emit\s*\(/,
    /\bbank_withdraw\s*\(/,
    /\bbank_store\s*\(/,
...
test("Bank-Deposit-Adapter sendet bei exakter Bindung genau einmal", async () => {
  let calls = 0;
  let expression = "";
  const session = {
  ...
  const adapter = new ProduktionsCdpBankDepositEinGoldAdapter(session, 7);
  ...
  assert.ok(expression.includes("root.bank_deposit(1)"));
...
test("Bank-Deposit-Adapter blockiert ungueltige Anfrage vor Browseraufruf", async () => {
  let calls = 0;
  const adapter = new ProduktionsCdpBankDepositEinGoldAdapter({
    async evaluate() {
      calls += 1;
      return { sent: true };
    },
...
test("produktiver Bank-Beobachter bindet frischen Mount an Lease- und Mount-Epoche", async () => {
  ...
  const beobachter = erstelleProduktivenBankDepositBeobachter(
    session,
    3,
  ...
  const vorher = erstelleBankDepositEinGoldVorherBindung(
    {
      ...raw(),
      beobachtetAmMs: 1200,
