...
" const cg=Number(c.gold),bg=bank?Number(bank.gold):NaN; let bridge=false,codeActive=false; for(const r of roots){try{if(typeof r?.call_code_function_f==='function')bridge=true}catch{};try{if(r?.code_active===true)codeActive=true}catch{}}",
