/* AUTO-GENERIERT: V5 R19 SOAK_5M Ingame-Testpaket
 * Quellen:
 * - v5/werkzeuge/v5-adventure-land-test-gui.js
 * - v5/werkzeuge/r19-soak-5m-test-gui.js
...
(() => {
  ...
  const API_NAME = 'V5R19Soak5mGui';
  const VERSION = '1.0.0';
  const SESSION_KEY = 'AIO_V5_R19_SOAK_5M_SESSION_V1';
  const PROBE_KEY = 'AIO_V5_R19_SOAK_5M_PROBE_V1';
  const BESTAETIGUNG = 'R19-SOAK-5M-START';
  ...
  function persistenzRoundtrip() {
    ...
    const wert = JSON.stringify({ zeit: Date.now(), probe: 'R19_SOAK_5M' });
    const start = now();
  ...
  function bewerte(session) {
    ...
    return {
      ...
      hinweis: blocker.length
        ? '5-Minuten-Soak nicht bestanden. Nicht wiederholen, bevor der Bericht ausgewertet wurde.'
        : '5-Minuten-Canary-Scope-Soak bestanden. Diese Evidence gibt die breite Runtime nicht frei.'
    };
  }
  ...
  async function tick() {
    ...
    try {
      ...
      if (elapsed >= DAUER_MS) {
        ...
        gui.protokolliere('SOAK_5M abgeschlossen', result);
        gui.setzeErgebnis(
          result,
          result.status === 'BESTANDEN' ? 'bestanden' : 'fehler',
          result.status === 'BESTANDEN'
            ? 'SOAK_5M BESTANDEN · Jetzt Gesamtbericht kopieren.'
            : 'SOAK_5M NICHT BESTANDEN · Kein Neustart. Gesamtbericht kopieren.'
        );
      }
    } catch (error) {
      laeuft = false;
      ...
      gui.protokolliere('SOAK_5M FEHLER', message);
    } finally {
      sampling = false;
    }
  }
  ...
  gui.registriereAktion({
    ...
    async ausfuehren() {
      ...
      gui.protokolliere('SOAK_5M Vorpruefung', result);
      gui.setzeErgebnis(
        result,
        result.status === 'BESTANDEN' ? 'bestanden' : 'blockiert',
        result.status === 'BESTANDEN'
          ? 'Vorprüfung bestanden. 5-Minuten-Soak kann gestartet werden.'
  ...
  gui.registriereAktion({
    ...
    ausfuehren() {
      ...
      gui.protokolliere('SOAK_5M Zwischenstand', result);
      gui.setzeErgebnis(result, session?.status === 'COMPLETED' ? 'info' : 'laeuft', 'SOAK_5M Zwischenstand.');
      return result;
    }
  });
  ...
  gui.registriereAktion({
    ...
    ausfuehren() {
      ...
      const result = { status: 'ABGEBROCHEN', zertifizierungsStufe: 'SOAK_5M', bestanden: false };
      gui.protokolliere('SOAK_5M abgebrochen', result);
  ...
  gui.protokolliere('R19 SOAK_5M GUI bereit', {
    version: VERSION,
    bestaetigungsText: BESTAETIGUNG,
    dauerMs: DAUER_MS,
    intervallMs: INTERVALL_MS,
