(() => {
  ...
  const TEST_ID = "pr20-8-exchange-anniversarygift-autonomy-productive-one-write-live";
  const VERSION = "1.0.0";
  const API_NAME = "V5PR208ExchangeAnniversarygiftAutonomyProductiveOneWriteLive";
  ...
  const PREREQUISITE_SHADOW_NOTIFICATION_ID = 3020;
  const PRIOR_COMMITTED_TRANSACTION_ID = "pr20-8-exchange-anniversarygift-productive-one-write-live:0c6a1129be4c9c899f88274fab97108a";
  ...
  const RUNTIME_LEASE_KEY = "__V5PR208ExchangeAnniversaryGiftAutonomyProductiveOneWriteLiveLease";
  const DECISION_PREFIX = "v5:" + TEST_ID + ":autonomy-decision:";
  ...
  let state = {
    ...
    authority: {
      ...
      durableAutonomyDecisionCreated: false,
  ...
  function root(){
    ...
    throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_SPIELKONTEXT_FEHLT");
  }
  ...
  async function sha256(value){
    ...
    if(!cryptoApi?.subtle?.digest || typeof Encoder !== "function"){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_WEB_CRYPTO_UNAVAILABLE");
    }
  ...
  function storage(){
    ...
    throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DURABLE_STORAGE_UNAVAILABLE");
  }
  ...
  function readJson(key){
    ...
    catch { throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DURABLE_JSON_BESCHAEDIGT"); }
  }
  ...
  function writeJsonExact(key,value){
    ...
    if(readback !== encoded){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DURABLE_READBACK_MISMATCH");
    }
  ...
  function sellDistanceEvidence(){
    ...
    if(Number.isFinite(observed) && observed !== SOURCE_PINNED_SELL_DISTANCE){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_SELL_DIST_DRIFT");
    }
    return {
  ...
  function serviceReachability(r,c){
    ...
    if(![px,py].every(Number.isFinite)){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_POSITION_UNLESBAR");
    }
    ...
    if(!Number.isFinite(distance) || distance > conservativeLimit){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_SERVICE_NICHT_ERREICHBAR");
    }
    return {
      reachable:true,
      viaComputer:false,
  ...
  function candidateRows(items,G){
    ...
    if(!exactDefinition(def)){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_ITEM_DEFINITION_DRIFT");
    }
    const rows=[];
  ...
  async function strictObservation(performance){
    ...
    if(text(c.name,192) !== EXPECTED_CHARACTER){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_RECIPIENT_DRIFT");
    }
    if(text(c.id,192) !== EXPECTED_CHARACTER){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_SESSION_DRIFT");
    }
    if(text(c.ctype || c.type,32).toLowerCase() !== EXPECTED_CLASS){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_MERCHANT_ERFORDERLICH");
    }
    const server=serverBinding(r);
    if(server.region !== EXPECTED_SERVER_REGION || server.identifier !== EXPECTED_SERVER_IDENTIFIER){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_SERVER_BINDUNG_DRIFT");
    }
    if(c.rip === true || c.dead === true){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_CHARACTER_TOT");
    }
    if(c.moving === true){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_CHARACTER_BEWEGT_SICH");
    }
    if(c.target !== null && c.target !== undefined && text(c.target,192)){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_CHARACTER_HAT_ZIEL");
    }
    if(c.q && typeof c.q === "object" && Object.keys(c.q).length){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_Q_NICHT_FREI");
    }
    if(text(c.map,96).startsWith("bank") || c.user){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_BANK_KONTEXT_VERBOTEN");
    }
    const conflict=runtimeConflict(r);
    if(conflict){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_ALTERNATIVE_RUNTIME_AKTIV:"+conflict);
    }
    if(hostileAggro(r) !== 0){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_CHARACTER_UNTER_ANGRIFF");
    }
    if(!performance?.active){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_PERFORMANCE_TRICK_NICHT_AKTIV");
    }
    if(typeof globalThis.exchange !== "function"){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_CODE_WRAPPER_EXCHANGE_FEHLT");
    }
    if(c.s?.massexchange){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_MASSEXCHANGE_AKTIV");
    }
    if(c.s?.massexchangepp){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_MASSEXCHANGEPP_AKTIV");
    }
    ...
    if(!Number.isFinite(esize) || esize < REQUIRED_EMPTY_SLOTS || visibleEmptySlots < REQUIRED_EMPTY_SLOTS){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_OUTPUTSPACE_FEHLT");
    }

    const candidates=candidateRows(c.items,r.G);
    if(candidates.length !== 1){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_KANDIDAT_NICHT_EINDEUTIG");
    }
  ...
  function decisionKey(prestate){
    return DECISION_PREFIX+prestate;
  }

  function exactAutonomyDecision(decoded,observation){
    return Boolean(decoded
      && decoded.schemaVersion === 1
      && decoded.testId === TEST_ID
      && decoded.version === VERSION
      && decoded.art === "PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DECISION"
      && decoded.status === "DECISION_DURABLE"
      && decoded.terminal === false
      && decoded.sameIntentRetry === false
      && decoded.selectionMode === "AUTONOMOUS_FRESH_CURRENT_INVENTORY_SCAN"
      && decoded.manualPinnedInventoryIndex === false
      && decoded.observedIndexCarriesAuthority === false
      && decoded.priorCommittedTransactionGrantsAuthority === false
      && decoded.priorCommittedTransactionId === PRIOR_COMMITTED_TRANSACTION_ID
      && decoded.prerequisiteAutonomyShadowNotificationId === PREREQUISITE_SHADOW_NOTIFICATION_ID
      && decoded.recipient?.characterName === observation.recipient.characterName
      && decoded.recipient?.sessionId === observation.recipient.sessionId
      && decoded.recipient?.serverRegion === observation.recipient.serverRegion
      && decoded.recipient?.serverIdentifier === observation.recipient.serverIdentifier
      && decoded.candidate?.name === ITEM_NAME
      && decoded.candidate?.index === observation.candidate.index
      && decoded.candidate?.quantity === observation.candidate.quantity
      && decoded.candidate?.exchangeQuantity === EXCHANGE_QUANTITY
      && decoded.candidate?.material === observation.candidate.material
      && decoded.fingerprints?.prestate === observation.fingerprints.prestate
      && decoded.fingerprints?.inventory === observation.fingerprints.inventory
      && decoded.fingerprints?.q === observation.fingerprints.q
      && decoded.fingerprints?.candidate === observation.fingerprints.candidate
      && decoded.fingerprints?.definition === observation.fingerprints.definition
      && decoded.fingerprints?.conditions === observation.fingerprints.conditions
      && decoded.fingerprints?.aggregate === observation.fingerprints.aggregate
      ...
      && decoded.authorityIssued === false
      && decoded.freshCandidateReresolutionRequiredBeforeSend === true
      && decoded.normalRuntimeAllowed === false);
  }

  function createAutonomyDecision(observation){
    const pre=observation.fingerprints.prestate;
    const key=decisionKey(pre);
    const existing=readJson(key);
    if(existing){
      if(!exactAutonomyDecision(existing,observation)){
        throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DECISION_DRIFT");
      }
      return {key,value:existing,existing:true};
    }
    const decisionId=TEST_ID+":decision:"+pre.slice(0,32);
    const record={
      schemaVersion:1,
      testId:TEST_ID,
      version:VERSION,
      art:"PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DECISION",
      decisionId,
      runnerInstanceId:INSTANCE_ID,
      createdAtMs:Date.now(),
      updatedAtMs:Date.now(),
      status:"DECISION_DURABLE",
      terminal:false,
      sameIntentRetry:false,
      selectionMode:"AUTONOMOUS_FRESH_CURRENT_INVENTORY_SCAN",
      manualPinnedInventoryIndex:false,
      ...
      priorCommittedTransactionId:PRIOR_COMMITTED_TRANSACTION_ID,
      prerequisiteAutonomyShadowNotificationId:PREREQUISITE_SHADOW_NOTIFICATION_ID,
      ...
      authorityIssued:false,
      freshCandidateReresolutionRequiredBeforeSend:true,
      normalRuntimeAllowed:false
    };
    const persisted=writeJsonExact(key,record);
    if(!exactAutonomyDecision(persisted,observation)){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DECISION_READBACK_DRIFT");
    }
    return {key,value:persisted,existing:false};
  }

  function updateAutonomyDecision(key,patch){
    const current=readJson(key);
    if(!current || current.testId !== TEST_ID){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DECISION_FEHLT");
    }
  ...
  function acquireRuntimeLease(){
    ...
    if(existing?.instanceId && existing.instanceId !== INSTANCE_ID){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DUPLIKAT_INSTANZ_AKTIV");
    }
    const lease={
      schemaVersion:1,
      testId:TEST_ID,
    ...
    if(r[RUNTIME_LEASE_KEY]?.instanceId !== INSTANCE_ID){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_RUNTIME_LEASE_VERLOREN");
    }
    return lease;
  }
  ...
  function acquireFences(txId){
    ...
    for(const resource of RESOURCE_CLAIMS){
      ...
      if(old && Number(old.expiresAtMs) > now && old.owner !== owner){
        throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_FENCE_BELEGT:"+resource);
      }
      writeJsonExact(key,{
        schemaVersion:1,
        testId:TEST_ID,
  ...
  function assertFences(txId){
    ...
    for(const resource of RESOURCE_CLAIMS){
      ...
      if(current?.owner !== owner
        ...
        throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_FENCE_OWNERSHIP_VERLOREN:"+resource);
      }
    }
  }
  ...
  function findExistingIntent(){
    ...
    if(found.length > 1){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_MEHRERE_INTENTS");
    }
    return found[0] || null;
  }

  function createIntent(observation,autonomyDecision,autonomyDecisionKeyValue){
    const pre=observation.fingerprints.prestate;
    ...
    const record={
      ...
      testId:TEST_ID,
      version:VERSION,
      art:"PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_ONE_WRITE_INTENT",
      transactionId:txId,
      ...
      priorCommittedTransactionId:PRIOR_COMMITTED_TRANSACTION_ID,
      priorCommittedTransactionGrantsAuthority:false,
      autonomyDecisionId:autonomyDecision.decisionId,
      autonomyDecisionKey:autonomyDecisionKeyValue,
      selectionMode:"AUTONOMOUS_FRESH_CURRENT_INVENTORY_SCAN",
      manualPinnedInventoryIndex:false,
      observedIndexCarriesAuthority:false,
      runnerInstanceId:INSTANCE_ID,
      ...
      authority:{
        class:"Pr208AnniversaryGiftExchangeAutonomyOneShotAuthority",
        maximumUses:1,
    ...
    if(persisted.runnerInstanceId !== INSTANCE_ID){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_INTENT_OWNERSHIP_VERLOREN");
    }
    return {key,value:persisted,existing:false};
  }
  ...
  function issueAuthority(intent,observation){
    ...
    return writeJsonExact(authorityKey(intent.transactionId),{
      ...
      authorityClass:"Pr208AnniversaryGiftExchangeAutonomyOneShotAuthority",
      actionContractId:"AL-ACTION-EXCHANGE",
      recoveryContractId:"AL-RECOVERY-EXCHANGE",
      verifierId:"AL-VERIFIER-EXCHANGE",
      issuedAtMs:now,
      ...
      binding:{
        ...
        sourceCommit:SOURCE_COMMIT,
        dropGraphSha256:DROP_GRAPH_SHA256,
        autonomyDecisionId:intent.autonomyDecisionId
      }
    });
  }
  ...
  function consumeAuthority(intent,authority,observation){
    ...
    const matches=authority.testId === TEST_ID
      ...
      && authority.authorityClass === "Pr208AnniversaryGiftExchangeAutonomyOneShotAuthority"
      ...
      && b.autonomyDecisionId === intent.autonomyDecisionId;
    if(!matches){
      ...
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_AUTHORITY_BINDING_DRIFT");
    }
    return writeJsonExact(authorityKey(intent.transactionId),{
      ...authority,
      uses:1,
  ...
  function emit(type,severity,data={}){
    ...
    events.push({
      ...
      component:"v5-pr20-8-exchange-anniversarygift-autonomy-productive-one-write-live",
      data
    });
  ...
  function terminalEvidence(intent,outcome,extra={}){
    return {
      evidenceArt:"V5_PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_ONE_WRITE_LIVE",
  ...
  function settleIntent(existing,outcome){
    ...
    if(settled.autonomyDecisionKey){
      const decision=readJson(settled.autonomyDecisionKey);
      if(decision){
        updateAutonomyDecision(settled.autonomyDecisionKey,{
          status:journalStatus,
          terminal,
          authorityIssued:state.authority.authorityIssued === true,
          authorityConsumed:state.authority.authorityConsumed === true,
          outcome
        });
      }
    }
    ...
    setState({
      ...
      terminal,
      blocker:terminal ? [] : ["PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_RECOVERY_PENDING"],
    ...
    emit(
      terminal ? "PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_TERMINAL"
        : "PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_RECOVERY_PENDING",
  ...
  async function recoverExisting(existing){
    ...
    if(intent.schemaVersion !== 1
      ...
      || intent.selectionMode !== "AUTONOMOUS_FRESH_CURRENT_INVENTORY_SCAN"
      || intent.manualPinnedInventoryIndex !== false
      || intent.observedIndexCarriesAuthority !== false
      || intent.priorCommittedTransactionGrantsAuthority !== false
      || intent.priorCommittedTransactionId !== PRIOR_COMMITTED_TRANSACTION_ID
      || !intent.autonomyDecisionId
      || !intent.autonomyDecisionKey
      || !Number.isInteger(intent.sendCount)
      || intent.sendCount < 0
      || intent.sendCount > 1){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_EXISTING_INTENT_UNGUELTIG");
    }
    const persistedDecision=readJson(intent.autonomyDecisionKey);
    if(!persistedDecision
      || persistedDecision.decisionId !== intent.autonomyDecisionId
      || persistedDecision.testId !== TEST_ID
      || persistedDecision.priorCommittedTransactionGrantsAuthority !== false
      || persistedDecision.priorCommittedTransactionId !== PRIOR_COMMITTED_TRANSACTION_ID
      || persistedDecision.manualPinnedInventoryIndex !== false
      || persistedDecision.observedIndexCarriesAuthority !== false
      || persistedDecision.fingerprints?.prestate !== intent.prestate?.fingerprints?.prestate){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_EXISTING_DECISION_UNGUELTIG");
    }
    ...
    setState({
      autonomyDecision:{
        ...
        selectionMode:persistedDecision.selectionMode,
        manualPinnedInventoryIndex:false,
        observedIndexCarriesAuthority:false,
        priorCommittedTransactionGrantsAuthority:false,
      ...
      authority:{
        ...state.authority,
        durableAutonomyDecisionCreated:true,
  ...
  async function performFreshOneWrite(performance){
    ...
    if(!sameObservation(first,second)){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_DOUBLE_OBSERVE_DRIFT");
    }

    const existing=findExistingIntent();
    if(existing) return recoverExisting(existing);

    const decision=createAutonomyDecision(second);
    if(decision.existing === true){
      throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_EXISTING_DECISION_OHNE_INTENT");
    }

    const created=createIntent(second,decision.value,decision.key);
    ...
    if(fresh.fingerprints.prestate !== intent.prestate.fingerprints.prestate
      ...
      return setState({
        ...
        phase:"COMPLETE",
        terminal:true,
        blocker:["PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_FRESH_ADMISSION_DRIFT"],
    ...
    updateAutonomyDecision(intent.autonomyDecisionKey,{
      status:"AUTHORITY_ISSUED",
      authorityIssued:true,
      authorityIssuedAtMs:Date.now()
    });
    state.authority={
      ...state.authority,
      authorityIssued:true,
    ...
    if(sendFresh.fingerprints.prestate !== intent.prestate.fingerprints.prestate
      ...
      return setState({
        ...
        phase:"COMPLETE",
        terminal:true,
        blocker:["PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_FINAL_SEND_DRIFT"],
        evidence:terminalEvidence(intent,intent.outcome),
        intents:[{
          transactionId:intent.transactionId,
          status:intent.status,
    ...
    updateAutonomyDecision(intent.autonomyDecisionKey,{
      ...
      authorityConsumedAtMs:consumed.consumedAtMs
    });
    state.authority={
      ...state.authority,
  ...
  function fail(error){
    ...
    emit("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_FEHLER","error",{reason:message});
  ...
  async function run(){
    ...
    runPromise=(async()=>{
      ...
      if(!performance.active){
        throw new Error("PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_PRODUCTIVE_PERFORMANCE_TRICK_BLOCKED");
      }
      setState({
