(() => {
  ...
  function installTelemetryFacade(owner) {
    ...
    if (existing?.__v5Pr208CandidateFacadeVersion === VERSION) {
      let current = null;
      try {
        const value = typeof existing.status === 'function'
          ? existing.status()
          : null;
        current = value?.v5AutonomousTest && typeof value.v5AutonomousTest === 'object'
          ? value.v5AutonomousTest
          : null;
      } catch {}
      if (current
          && text(current.testId,192) === TEST_ID
          && text(current.version,32) === VERSION) {
        return;
      }
    }
