...
if (r10.status === "DONE") {
  ...
  const spaeterePhasen = new Set(["R12","R13","R14","R15","R16","R17","R18","R19"]);
  const direkterUebergang = gates.currentPhase === "R11" && r11?.status === "IN_PROGRESS";
  const bereitsWeiter = spaeterePhasen.has(gates.currentPhase) && r11?.status === "DONE";
  if (!direkterUebergang && !bereitsWeiter) {
    fehler("R10 DONE verlangt mindestens R11 IN_PROGRESS oder einen formal abgeschlossenen R11-Uebergang.");
  }
}
...
if (bereitschaft.status === "FREIGEGEBEN") {
  if (!fs.existsSync("roadmap/gesamtfreigabe.json")) {
    fehler("R10 darf die Runtime nicht selbst freigeben; finale Betreiber-Gesamtfreigabe-Evidence fehlt.");
  }
  ...
  if (gesamtfreigabe.kennung !== "V5_GESAMTFREIGABE"
    ...
    fehler("R10 darf die Runtime nicht selbst freigeben; nur die spaetere explizite Post-R19-Gesamtfreigabe ist zulaessig.");
  }
}
