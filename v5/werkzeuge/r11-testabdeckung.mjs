...
for (const eintrag of katalog.pfade) {
  if (!fs.existsSync(eintrag.quelle)) fehler("Corequelle fehlt: " + eintrag.quelle);
  ...
  for (const test of [...eintrag.positiv, ...eintrag.negativ]) {
    if (!fs.existsSync(test)) fehler("Testdatei fehlt: " + test);
  }
}
...
for (const art of [
  ...
  for (const test of eintrag.nachweis) if (!fs.existsSync(test)) fehler("Fault-Test fehlt: " + test);
}
