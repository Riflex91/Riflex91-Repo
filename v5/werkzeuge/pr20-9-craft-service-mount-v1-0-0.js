(() => {
  ...
  const TEST_ID = "pr20-9-craft-service-mount";
  const VERSION = "1.0.0";
  const API_NAME = "V5PR209CraftServiceMount";
  ...
  let state = {
    ...
    testId: TEST_ID,
    version: VERSION,
    phase: "PR20_9_CRAFT_SERVICE_MOUNT",
    status: "BOOT",
    terminal: false,
    blocker: [],
  ...
  function root() {
    ...
    throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_SPIELKONTEXT_FEHLT");
  }
  ...
  function storage() {
    ...
    throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_DURABLE_STORAGE_FEHLT");
  }
  ...
  function readIntent() {
    ...
    catch { throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_INTENT_BESCHAEDIGT"); }
  }
  ...
  function writeIntent(value) {
    ...
    if (readback !== encoded) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_DURABLE_READBACK_MISMATCH");
    }
  ...
  function recipientSnapshot(r) {
    ...
    return {
      ...
      distanceToCraftsman: distanceToService(c)
    };
  }
  ...
  function strictPreflight(r) {
    ...
    if (recipient.characterName !== EXPECTED_CHARACTER) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_EXAKTER_MERCHANT_ERFORDERLICH");
    }
    if (recipient.sessionId !== EXPECTED_CHARACTER) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_SESSION_DRIFT");
    }
    if (recipient.ctype !== EXPECTED_CLASS) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_MERCHANT_KLASSE_ERFORDERLICH");
    }
    if (recipient.serverRegion !== EXPECTED_SERVER_REGION
        || recipient.serverIdentifier !== EXPECTED_SERVER_IDENTIFIER) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_SERVER_BINDUNG_DRIFT");
    }
    if (c.rip === true || c.dead === true) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_CHARACTER_TOT");
    }
    if (c.moving === true) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_CHARACTER_BEWEGT_SICH");
    }
    if (c.target !== null && c.target !== undefined && text(c.target, 192)) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_CHARACTER_HAT_ZIEL");
    }
    if (c.q && typeof c.q === "object" && Object.keys(c.q).length > 0) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_Q_NICHT_FREI");
    }
    const conflict = runtimeConflict(r);
    if (conflict) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_ALTERNATIVE_RUNTIME_AKTIV:" + conflict);
    }
    const sellDistance = Number(r.B?.sell_dist);
    if (Number.isFinite(sellDistance)
        && sellDistance > 0
        && sellDistance !== SOURCE_PINNED_SELL_DISTANCE) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_SELL_DIST_DRIFT");
    }
    return recipient;
  }
  ...
  function emit(type, severity, data = {}) {
    ...
    events.push({
      ...
      type,
      severity,
      component: "v5-pr20-9-craft-service-mount",
  ...
  function finish(status, blocker = [], extra = {}) {
    ...
    publish();
    emit("PR20_9_CRAFT_SERVICE_MOUNT_TERMINAL",
  ...
  async function recover(intent) {
    if (!validateIntent(intent)) {
      throw new Error("PR20_9_CRAFT_SERVICE_MOUNT_EXISTING_INTENT_UNGUELTIG");
    }
    const r = root();
    const c = r.character;
    const recipient = recipientSnapshot(r);
    ...
    if (serviceReady(c)) {
      return finish("BESTANDEN", [], {
        ...
        nextAction: "RESTORE_PR20_9_CRAFT_DURABLE_SHADOW_MANIFEST",
    ...
    if (intent.sendCount === 0) {
      return finish("BLOCKIERT", [
        "PR20_9_CRAFT_SERVICE_MOUNT_DURABLE_INTENT_OHNE_SEND_RECORD"
      ], {
    ...
    if (c.moving === true) {
      state = {
        ...
        terminal: false,
        blocker: ["PR20_9_CRAFT_SERVICE_MOUNT_MOVEMENT_IN_FLIGHT_NO_RETRY"],
  ...
  async function freshRun() {
    ...
    if (!smartMove) {
      return finish("BLOCKIERT", ["PR20_9_CRAFT_SERVICE_MOUNT_SMART_MOVE_FEHLT"], {
    ...
    let intent = writeIntent({
      ...
      testId: TEST_ID,
      version: VERSION,
      art: "PR20_9_CRAFT_SERVICE_MOUNT_INTENT",
      target: TARGET,
      targetPoint: TARGET_POINT,
      sourceCommit: SOURCE_COMMIT,
      createdAtMs,
    ...
    while (Date.now() - started <= MOVE_TIMEOUT_MS) {
      if (serviceReady(c)) {
        intent = writeIntent({
          ...readIntent(),
          terminal: true,
          outcome: "ARRIVED_IN_SAFE_CRAFT_SERVICE_RANGE",
          updatedAtMs: Date.now()
        });
        return finish("BESTANDEN", [], {
          movementCompleted: true,
      ...
      if (movementError && c.moving !== true) {
        return finish("BLOCKIERT", [
          "PR20_9_CRAFT_SERVICE_MOUNT_MOVE_FEHLER_NO_RETRY"
        ], {
          movementError,
          nextAction: "RECONCILE_MOVEMENT_MANUALLY_NO_RETRY",
          recipient: recipientSnapshot(r)
    ...
    return finish("BLOCKIERT", [
      "PR20_9_CRAFT_SERVICE_MOUNT_TIMEOUT_NO_RETRY"
    ], {
      movementError,
      nextAction: "RECONCILE_MOVEMENT_MANUALLY_NO_RETRY",
      recipient: recipientSnapshot(r)
  ...
  function fail(error) {
    ...
    emit("PR20_9_CRAFT_SERVICE_MOUNT_FEHLER", "error", { reason: message });
    state = {
      ...state,
      status: "FEHLER",
      phase: "ERROR",
