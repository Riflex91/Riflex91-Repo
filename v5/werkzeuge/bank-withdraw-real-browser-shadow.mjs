...
import {
  BANK_WITHDRAW_REAL_SHADOW_GAMEPLAY_WRITES,
...
export async function fuehreBankWithdrawRealBrowserShadow({
  cdpText,
  sourceSha,
  bestaetigungText,
  ...
  if (bestaetigungText !== BANK_WITHDRAW_REAL_SHADOW_BESTAETIGUNG) {
  ...
  try {
    ...
    const bericht = Object.freeze({
      schemaVersion: 1,
      evidenceArt: BANK_WITHDRAW_REAL_SHADOW_EVIDENCE_ART,
      ...
      browserGameplayWrites: BANK_WITHDRAW_REAL_SHADOW_GAMEPLAY_WRITES,
    ...
    if (host !== null) {
      await host.stoppe("BANK_WITHDRAW_REAL_SHADOW_ENDE").catch(() => {});
    }
    live.session.close();
  }
}
...
if (direkt) {
  fuehreBankWithdrawRealBrowserShadow({
