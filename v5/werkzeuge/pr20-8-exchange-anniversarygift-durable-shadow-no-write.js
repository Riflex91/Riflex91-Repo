(() => {
  ...
  const state = {
    ...
    testId: TEST_ID,
    version: VERSION,
    phase: 'PR20_8_EXCHANGE_ANNIVERSARYGIFT_DURABLE_SHADOW',
    status: 'BOOT',
    terminal: false,
    blocker: [],
    ...
    sourceEvidence: {
      ...
      notificationId: SOURCE_NOTIFICATION_ID
    },
  ...
  function root() {
    ...
    throw new Error('PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_SPIELKONTEXT_FEHLT');
  }
  ...
  async function sha256(value) {
    ...
    if (!cryptoApi?.subtle?.digest || typeof Encoder !== 'function') {
      throw new Error('PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_WEB_CRYPTO_UNAVAILABLE');
    }
  ...
  function storage() {
    ...
    throw new Error('PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_DURABLE_STORAGE_UNAVAILABLE');
  }
  ...
  function resolveCandidate(r) {
    ...
    if (!exactDefinition(def)) {
      return { blocker:'PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_ITEM_DEFINITION_DRIFT' };
    }
    const matches = [];
    ...
    if (!matches.length) {
      return { blocker:'PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_EXAKTER_INPUT_FEHLT' };
    }
  ...
  function installTelemetryFacade(owner) {
    ...
    if (existing?.__v5Pr208ExchangeAnniversaryShadowVersion === VERSION
    ...
    owner.AIO_V3.operations = {
      ...(existing || {}),
      __v5Pr208ExchangeAnniversaryShadowVersion: VERSION,
      status: () => {
        let base = {};
        try {
  ...
  function exactIntent(decoded, record) {
    return Boolean(decoded
      ...
      && decoded.art === 'PR20_8_EXCHANGE_ANNIVERSARYGIFT_DURABLE_SHADOW_INTENT_NO_GAMEPLAY_WRITE'
      ...
      && decoded.sourceEvidence?.notificationId === SOURCE_NOTIFICATION_ID
      ...
      && decoded.exactPhysicalCandidateIndexPinnedAtShadowTime === true
      && decoded.freshReresolutionRequiredBeforeFutureSend === true
      && decoded.fullRewardDomainProofRequiredBeforeFutureSend === true
      && decoded.massExchangeAllowed === false
      && decoded.recursiveDropAuthority === false
      && decoded.specialMultiOutputAuthority === false
      ...
      && decoded.normalExchangeWriteRatification === false
  ...
  function persistShadow(record) {
    ...
    if (existingRaw) {
      ...
      if (!exactIntent(decoded, record)) {
        throw new Error('PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_TERMINAL_INTENT_DRIFT');
      }
    ...
    const payload = {
      ...
      testId:TEST_ID,
      version:VERSION,
      art:'PR20_8_EXCHANGE_ANNIVERSARYGIFT_DURABLE_SHADOW_INTENT_NO_GAMEPLAY_WRITE',
      ...
      exactPhysicalCandidateIndexPinnedAtShadowTime:true,
      freshReresolutionRequiredBeforeFutureSend:true,
      fullRewardDomainProofRequiredBeforeFutureSend:true,
    ...
    if (!exactIntent(readback, record)) {
      throw new Error('PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_DURABLE_READBACK_DRIFT');
    }
  ...
  async function run() {
    ...
    if (text(c.name,192) !== EXPECTED_CHARACTER) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_EXAKTER_MERCHANT_ERFORDERLICH']);
      return;
    }
    if (text(c.ctype || c.type,32).toLowerCase() !== EXPECTED_CLASS) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_MERCHANT_KLASSE_ERFORDERLICH']);
      return;
    }
    if (!text(c.id,192)) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_SESSION_FEHLT']);
      return;
    }
    if (server.region !== EXPECTED_SERVER_REGION || server.identifier !== EXPECTED_SERVER_IDENTIFIER) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_SERVER_BINDUNG_DRIFT']);
      return;
    }
    if (c.rip === true || c.dead === true) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_CHARACTER_TOT']);
      return;
    }
    if (c.moving === true) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_CHARACTER_BEWEGT_SICH']);
      return;
    }
    if (c.target !== null && c.target !== undefined && text(c.target,192)) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_CHARACTER_HAT_ZIEL']);
      return;
    }
    if (c.q && typeof c.q === 'object' && Object.keys(c.q).length) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_Q_NICHT_FREI']);
      return;
    }
    const conflict = runtimeConflict(r);
    if (conflict) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_ALTERNATIVE_RUNTIME_AKTIV:' + conflict]);
      return;
    }
    state.performanceTrick = await performanceStatus();
    if (!state.performanceTrick.active) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_PERFORMANCE_TRICK_BLOCKED']);
      return;
    }
    if (!publicExchangeAvailable()) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_EXCHANGE_SERVICE_FEHLT']);
      return;
    }
    ...
    if (!sameObservation(first,second)) {
      finish('BLOCKIERT',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_DOPPELBEOBACHTUNG_DRIFT']);
      return;
    }
  ...
  Promise.resolve().then(run).catch(error => {
    ...
    finish('FEHLER',['PR20_8_EXCHANGE_ANNIVERSARYGIFT_SHADOW_UNEXPECTED_ERROR']);
  });
})();
