(() => {
  ...
  const VERSION = '1.0.0';
  const TEST_ID = 'pr20-7-gear-occupied-slot-read-only-preflight';
  ...
  function emit(type, severity, data = {}) {
    ...
    events.push({
      ...
      component: 'v5-pr20-7-gear-read-only-preflight',
      data
    });
  ...
  async function run() {
    ...
    emit('PR20_7_GEAR_READ_ONLY_PREFLIGHT_STARTED', 'INFO');
    ...
    if (!performanceTrick.active) {
      ...
      emit('PR20_7_GEAR_READ_ONLY_PREFLIGHT_BLOCKED', 'ERROR', {
    ...
    if (!firstCandidate) {
      ...
      emit('PR20_7_GEAR_READ_ONLY_PREFLIGHT_BLOCKED', 'WARN', {
    ...
    const evidence = {
      schemaVersion: 1,
      evidenceArt: 'V5_PR20_7_OCCUPIED_SLOT_REAL_BROWSER_PREFLIGHT',
      status: 'BESTANDEN',
      observedAtMs: Date.now(),
      recipient: {
    ...
    emit('PR20_7_GEAR_READ_ONLY_PREFLIGHT_PASSED', 'INFO', {
  ...
  installTelemetryFacade();
  globalThis.V5PR207GearReadOnlyTest = Object.freeze({
    version: VERSION,
    testId: TEST_ID,
    status: () => state,
    start: () => run()
  ...
  Promise.resolve().then(run).catch(error => {
    ...
    emit('PR20_7_GEAR_READ_ONLY_PREFLIGHT_ERROR', 'ERROR', { reason: message });
  });
})();
