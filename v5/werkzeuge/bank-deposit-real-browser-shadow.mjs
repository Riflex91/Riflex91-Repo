...
export const BANK_DEPOSIT_REAL_SHADOW_BESTAETIGUNG =
  "V5 BANK DEPOSIT SHADOW OHNE WRITE AUSFUEHREN";
export const BANK_DEPOSIT_REAL_SHADOW_EVIDENCE_ART =
...
export async function fuehreBankDepositRealBrowserShadow({
  cdpText,
  sourceSha,
  bestaetigungText,
  ...
  if (bestaetigungText !== BANK_DEPOSIT_REAL_SHADOW_BESTAETIGUNG) {
  ...
  try {
    ...
    const mountBeobachter = Object.freeze({
      async warteAufMount() {
        return warteAufManuellenBankMountReadOnly(
          live.session,
          live.contextId,
          ausgang,
          {
    ...
    const bericht = Object.freeze({
      schemaVersion: 1,
      evidenceArt: BANK_DEPOSIT_REAL_SHADOW_EVIDENCE_ART,
      ...
      browserGameplayWrites: BANK_DEPOSIT_REAL_SHADOW_GAMEPLAY_WRITES,
    ...
    if (host !== null) {
      await host.stoppe("BANK_DEPOSIT_REAL_SHADOW_ENDE").catch(() => {});
    }
    live.session.close();
  }
}
...
if (direkt) {
  fuehreBankDepositRealBrowserShadow({
