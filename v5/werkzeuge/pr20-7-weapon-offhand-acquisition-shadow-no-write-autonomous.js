(() => {
  ...
  function observation() {
    ...
    const offhandKinds = Array.isArray(merchantClass?.offhand)
      ? merchantClass.offhand.map(x => text(x, 64))
      : [];
    ...
    if (currentMainhand?.name) {
      ...
      const doublehand = Array.isArray(merchantClass?.doublehand)
        ? merchantClass.doublehand.map(x => text(x, 64))
        : [];
