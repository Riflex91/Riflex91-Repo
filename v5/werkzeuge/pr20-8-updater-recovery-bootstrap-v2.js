...
function installPr208UpdaterRecoveryBootstrapV2() {
  'use strict';

  const API_NAME = 'V5PR208UpdaterRecoveryBootstrap';
  const TEST_ID = 'pr20-8-native-updater-recovery-bootstrap-v2';
  const VERSION = '1.0.0';
  const UPDATER_VERSION = '1.0.8';
  const PERSIST_MARKER_KEY = 'AIO_V5_PR20_8_UPDATER_BOOTSTRAP_V2_PERSISTED';
  ...
  function installObservabilityBridgeOn(host) {
  ...
  function installObservabilityBridge() {
    for (const host of observabilityRoots()) {
      try { installObservabilityBridgeOn(host); } catch {}
    }
  }
  ...
  function cleanBundleSource() {
    return installV5AutonomousTestIngameUpdater.toString()
      + '\ninstallV5AutonomousTestIngameUpdater();\n\n'
      + installPr208UpdaterRecoveryBootstrapV2.toString()
      + '\ninstallPr208UpdaterRecoveryBootstrapV2();\n';
  }
  ...
  if (marker) {
    ...
    setTimeout(() => { void persistOnce(); }, 3000);
  }
  return api;
}
