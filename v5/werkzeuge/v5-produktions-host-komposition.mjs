...
import {
  NodeBankRetrieveTransaktionsJournal,
} from "../grundlage/adapter/persistenz/node-bank-retrieve-transaktionsjournal.mjs";
import {
...
class NodeV5ProduktionsHost {
  ...
  constructor(
    ...
    this.#bankRetrieveJournal = bankRetrieveJournal;
  ...
  async pruefeBankDepositStartBereit() {
    ...
    const retrieveJournal = await this.#bankRetrieveJournal.pruefeStartBereit();
    ...
    return Object.freeze({
      bereit: journal.bereit && withdrawJournal.bereit && swapJournal.bereit
        && retrieveJournal.bereit && storeJournal.bereit
  ...
  async #pruefeBankItemTransferStartBereit() {
    ...
    const retrieveJournal = await this.#bankRetrieveJournal.pruefeStartBereit();
    const storeJournal = await this.#bankStoreJournal.pruefeStartBereit();
    const offeneLeases = this.#bankLeaseController.sicht().filter(x => x.zustand !== "RELEASED");
    return Object.freeze({
      bereit: depositJournal.bereit && withdrawJournal.bereit && swapJournal.bereit && retrieveJournal.bereit && storeJournal.bereit && offeneLeases.length === 0,
      offeneBankDepositTransaktionId: depositJournal.offeneTransaktionsId,
      offeneBankWithdrawTransaktionId: withdrawJournal.offeneTransaktionsId,
      offeneBankSwapTransaktionId: swapJournal.offeneTransaktionsId,
      offeneBankRetrieveTransaktionId: retrieveJournal.offeneTransaktionsId,
      offeneBankStoreTransaktionId: storeJournal.offeneTransaktionsId,
      offeneTransaktionsId: depositJournal.offeneTransaktionsId ?? withdrawJournal.offeneTransaktionsId ?? swapJournal.offeneTransaktionsId ?? retrieveJournal.offeneTransaktionsId ?? storeJournal.offeneTransaktionsId,
      offeneBankLease: offeneLeases.length === 0 ? null : Object.freeze({
        accountId: offeneLeases[0].accountId, ownerCharacterId: offeneLeases[0].ownerCharacterId, epoche: offeneLeases[0].epoche,
        zustand: offeneLeases[0].zustand, serverRegion: offeneLeases[0].serverRegion, serverIdentifier: offeneLeases[0].serverIdentifier,
      }),
    });
  }

  async pruefeBankRetrieveStartBereit() {
    return this.#pruefeBankItemTransferStartBereit();
  }
  ...
  async fuehreEquipEinmalTransaktion(anfrage, jetztMs) {
    ...
    if (tick.zustand !== "LAEUFT"
        ...
        || tick.bankSwapEinmalAuthorityOffen) {
  ...
  async fuehreBankSwapZweiSlotTransaktion(anfrage, jetztMs) {
    pruefeZeit(jetztMs);
    if (anfrage === null || typeof anfrage !== "object") {
      throw new Error("NODE_BANK_SWAP_PROD_TX_ANFRAGE_UNGUELTIG");
    }
    for (const feld of [
      ...
      if (typeof wert !== "string" || wert.trim().length === 0 || wert.length > 192) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_FELD_UNGUELTIG:" + feld);
      }
    }
    if (anfrage.bestaetigungText !== BANK_SWAP_EINMAL_BESTAETIGUNG) {
      throw new Error("NODE_BANK_SWAP_PROD_TX_BESTAETIGUNG_FEHLT");
    }
    if (!anfrage.ausgang || typeof anfrage.ausgang !== "object"
      ...
      throw new Error("NODE_BANK_SWAP_PROD_TX_PORT_ODER_AUSGANG_UNGUELTIG");
    }
    for (const feld of ["accountId", "charakterName", "sessionId", "serverRegion", "serverKennung"]) {
      const wert = anfrage.ausgang[feld];
      if (typeof wert !== "string" || wert.trim().length === 0 || wert.length > 192) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_BINDUNG_UNGUELTIG:" + feld);
      }
    }
    if (!Array.isArray(anfrage.wissensSnapshot.quellenSha256)
        || typeof anfrage.wissensSnapshot.gitCommit !== "string") {
      throw new Error("NODE_BANK_SWAP_PROD_TX_WISSENSSNAPSHOT_UNGUELTIG");
    }

    const startBereit = await this.pruefeBankSwapStartBereit();
    if (!startBereit.bereit) throw new Error("NODE_BANK_SWAP_PROD_TX_START_BLOCKIERT");
    const tick = await this.#host.tick(jetztMs);
    if (tick.zustand !== "LAEUFT"
        || tick.aktivePlanenFaehigkeiten.length !== 0
        || tick.equipEinmalAuthorityOffen
        ...
        || tick.bankWithdrawEinmalAuthorityOffen
        || tick.bankSwapEinmalAuthorityOffen) {
      throw new Error("NODE_BANK_SWAP_PROD_TX_HOST_NICHT_BEREIT:" + tick.grund);
    }
    ...
    try {
      leaseToken = await this.#bankLeaseController.beanspruche(
        ...
        anfrage.ausgang.charakterName,
        anfrage.ablaufId,
        "bank_swap_two_slot_one_shot_live",
        anfrage.ausgang.serverRegion,
        anfrage.ausgang.serverKennung,
        jetztMs,
        300_000,
      ...
      if (!mount || typeof mount !== "object" || mount.bankGemountet !== true
          ...
          || typeof mount.inventoryFingerprint !== "string" || !/^[a-f0-9]{64}$/i.test(mount.inventoryFingerprint)
          || !k || !/^items[0-9]+$/.test(String(k.pack || ""))
          || !Number.isInteger(k.a) || k.a < 0 || k.a > 41
          || !Number.isInteger(k.b) || k.b < 0 || k.b > 41 || k.a === k.b
          || !k.itemA || !k.itemB || k.itemA.name === k.itemB.name
          || !/^[a-f0-9]{64}$/i.test(String(k.itemA.fingerprint || ""))
          || !/^[a-f0-9]{64}$/i.test(String(k.itemB.fingerprint || ""))
          || !/^[a-f0-9]{64}$/i.test(String(k.packRestFingerprint || ""))) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_MOUNT_EVIDENCE_UNGUELTIG");
      }
      ...
      if (mount.beobachtetAmMs > authorityMs || authorityMs - mount.beobachtetAmMs > 1_000) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_MOUNT_EVIDENCE_STALE");
      }
      const authorityErgebnis = await this.#host.erteileBankSwapEinmalAuthority(
        Object.freeze({
          schemaVersion: 1,
          aktivierungsId: anfrage.aktivierungsId,
          transaktionsId: anfrage.transaktionsId,
      ...
      if (!authorityErgebnis.erfolgreich || authorityErgebnis.authority === null) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_AUTHORITY_BLOCKIERT:" + authorityErgebnis.grund);
      }
      authority = authorityErgebnis.authority;
      const admissionMs = Date.now();
      if (!authority.gueltigFuer(admissionMs)) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_AUTHORITY_VOR_ADMISSION_ABGELAUFEN");
      }
      if (admissionMs - mount.beobachtetAmMs > 1_000) {
        throw new Error("NODE_BANK_SWAP_PROD_TX_PRESTATE_STALE");
      }
      const fence = Object.freeze({
        serverRegion: mount.serverRegion,
        serverIdentifier: mount.serverKennung,
      ...
      const vorher = Object.freeze({
        ...
        slotA: k.a,
        slotB: k.b,
        slotAItem: Object.freeze({ name: k.itemA.name, fingerprint: k.itemA.fingerprint }),
        slotBItem: Object.freeze({ name: k.itemB.name, fingerprint: k.itemB.fingerprint }),
        packRestFingerprint: k.packRestFingerprint,
        inventoryFingerprint: mount.inventoryFingerprint,
        characterGold: mount.characterGold,
        bankGold: mount.bankGold,
        fingerprint: mount.fingerprint,
      });
      const gate = new ProduktivesBankSwapEinmalAdmissionGate(
        this.#gesamtfreigabeGate,
        () => this.#host.status(),
        authority,
      );
      const ergebnis = await this.#runtime.fuehreBankSwapZweiSlotTransaktion(
        Object.freeze({
          schemaVersion: 1,
          freigabeId: anfrage.freigabeId,
          auftragId: anfrage.auftragId,
  ...
  async fuehreBankSwapRealShadow(anfrage, jetztMs) {
    ...
    try {
      ...
      const authorityMs = Date.now();
      const authorityErgebnis =
        await this.#host.erteileBankSwapEinmalAuthority(
          Object.freeze({
            schemaVersion: 1,
            aktivierungsId: anfrage.aktivierungsId,
            transaktionsId: anfrage.transaktionsId,
      ...
      const ergebnis = await this.#runtime.fuehreBankSwapShadowAdmission(
        Object.freeze({
          schemaVersion: 1,
          freigabeId: anfrage.freigabeId,
          auftragId: anfrage.auftragId,
...
export async function erstelleNodeV5ProduktionsHost({
  ...
  const bankRetrieveJournal = new NodeBankRetrieveTransaktionsJournal(dateisystem);
