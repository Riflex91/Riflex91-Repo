...
export async function fuehreBankSwapWritePreflight({
  ...
  const testLimit = new NodeBankSwapLiveTestLimit(ds);
  const testLimitState = await testLimit.lade(source);
  if (testLimitState.attempts.length !== 0) {
    throw new Error("BANK_SWAP_WRITE_PREFLIGHT_TESTBUDGET_BEREITS_BENUTZT");
  }
  ...
  try {
    ...
    const basisBereit = current.bereit
    ...
    const bereit = basisBereit
      && runner?.codeActive === true
      && runner?.bankSwapType === "function";
