(() => {
  ...
  const ASYNC_STAGE_TIMEOUT_MS = 2000;
  const RECOVERABLE_VERSIONS = Object.freeze(['1.0.0', VERSION]);
  ...
  const state = {
    ...
    startMode: 'DIRECT_ASYNC_ENTRY',
    bootStallRecovery: true,
    progressSequence: 0,
  ...
  async function withTimeout(promise, ms, blocker) {
    let timer = null;
    try {
      return await Promise.race([
        promise,
        new Promise((_, reject) => {
          timer = setTimeout(() => reject(new Error(blocker)), ms);
        })
      ]);
    } finally {
      if (timer !== null) {
  ...
  async function sha256(value) {
    ...
    const digest = await withTimeout(
      ...
      ASYNC_STAGE_TIMEOUT_MS,
      'PR20_8_EXCHANGE_ANNIVERSARYGIFT_AUTONOMY_SHADOW_WEB_CRYPTO_TIMEOUT'
    );
  ...
  function exactIntent(decoded, record) {
    return Boolean(decoded
      ...
      && RECOVERABLE_VERSIONS.includes(decoded.version)
  ...
  async function run() {
    ...
    setPhase('PERFORMANCE_TRICK_CHECK');
    ...
    setPhase('STABILITY_WAIT');
    await sleep(DOUBLE_OBSERVE_DELAY_MS);
    setPhase('SECOND_OBSERVATION');
    let second;
    ...
    setPhase('DURABLE_DECISION_PREP');
    ...
    setPhase('DURABLE_DECISION_WRITE');
    try {
