(() => {
  ...
  const VERSION = "1.0.0";
  const TEST_ID = "pr20-7-gear-occupied-slot-live-5m";
  ...
  function observation() {
    ...
    if (!account) throw new Error("PR20_7_GEAR_LIVE_ACCOUNT_BINDUNG_FEHLT");
    if (!txt(c.name, 192)) throw new Error("PR20_7_GEAR_LIVE_CHARACTER_BINDUNG_FEHLT");
    if (!txt(c.id, 192)) throw new Error("PR20_7_GEAR_LIVE_SESSION_BINDUNG_FEHLT");
    if (!server.region || !server.identifier) throw new Error("PR20_7_GEAR_LIVE_SERVER_BINDUNG_FEHLT");
    if (txt(c.ctype, 32).toLowerCase() !== "merchant") {
      throw new Error("PR20_7_GEAR_LIVE_MERCHANT_COORDINATOR_ERFORDERLICH");
    }
    if (c.rip === true || c.dead === true) throw new Error("PR20_7_GEAR_LIVE_CHARACTER_TOT");
    if (c.moving === true) throw new Error("PR20_7_GEAR_LIVE_CHARACTER_BEWEGT_SICH");
    if (c.target !== null && c.target !== undefined && txt(c.target, 192)) {
      throw new Error("PR20_7_GEAR_LIVE_CHARACTER_HAT_ZIEL");
    }
    if (c.q && typeof c.q === "object" && Object.keys(c.q).length) {
      throw new Error("PR20_7_GEAR_LIVE_CHARACTER_QUEUE_AKTIV");
    }
    const conflict = runtimeConflict(r);
    if (conflict) throw new Error("PR20_7_GEAR_LIVE_ALTERNATIVE_RUNTIME_AKTIV:" + conflict);
    ...
    if (hostile !== 0) throw new Error("PR20_7_GEAR_LIVE_CHARACTER_UNTER_ANGRIFF");
    ...
    return {
      account,
      characterName:txt(c.name,192),
      sessionId:txt(c.id,192),
      ctype:txt(c.ctype,32).toLowerCase(),
  ...
  function chooseCandidate(observed) {
    ...
    for (const item of observed.inventory) {
      ...
      if (item.classList.length && !item.classList.includes("merchant")) continue;
      if (item.requiredLevel > observed.level) continue;
  ...
  function identity(observed, selected) {
    return canonical({
      ...
      previousMaterial:selected.previous.material,
  ...
  function classify(plan, observed) {
    ...
    const restInventory = canonical(observed.inventory
      .filter(row => row.index !== plan.inventoryIndex)
      .map(row => [row.index,row.material]));
    const restEquipment = canonical(SAFE_SLOTS
      .filter(slot => slot !== plan.slot)
      .map(slot => [slot,observed.slots[slot]?.material ?? null]));
    const restOk = restInventory === plan.restInventoryMaterial
      && restEquipment === plan.restEquipmentMaterial;
    const postSlot = slotItem?.material === plan.candidateMaterial;
    const postIndex = indexItem?.material === plan.previousMaterial;
    const preSlot = slotItem?.material === plan.previousMaterial;
    const preIndex = indexItem?.material === plan.candidateMaterial;

    if (restOk && postSlot && postIndex) {
      return { status:"COMMITTED", settlement:"BESTAETIGT", reason:null };
    }
    if (restOk && preSlot && preIndex) {
      return { status:"NOT_APPLIED", settlement:"NICHT_AUSGEFUEHRT", reason:null };
    }
    if (restOk && (postSlot || postIndex)) {
      return { status:"PARTIAL_OPERATOR_REQUIRED", settlement:"TEILWEISE", reason:"NUR_TEILMENGE_DER_SWAP_POSTCONDITION" };
    }
  ...
  function emit(type, severity, data = {}) {
    ...
    events.push({
      ...
      component:"v5-pr20-7-gear-occupied-slot-live-5m",
      data
    });
  ...
  async function run() {
    ...
    if (!firstCandidate) {
      setState({
        ...
        blocker:["PR20_7_GEAR_KEIN_KOMPATIBLER_BELEGTER_SLOT_KANDIDAT"],
        performanceTrick,gameplayWrites:0,publicFunctionCalls:0
      });
      return;
    }
    ...
    const plan = {
      ...
      previousMaterial:secondCandidate.previous.material,
    ...
    const intent = {
      ...
      previousName:secondCandidate.previous.name,
      previousLevel:secondCandidate.previous.level,
      previousFingerprintMaterial:plan.previousMaterial,
    ...
    setState({
      ...
      authority:{
        ...
        rawWriteAuthority:false,swapWriteRatification:true,maximumUses:1,
    ...
    const r = root();
    if (typeof r.equip !== "function") throw new Error("PR20_7_GEAR_LIVE_EQUIP_UNAVAILABLE");
    ...
    if (!reconciliation || reconciliation.status !== "COMMITTED") {
      setState({
        ...
        evidence:{
          schemaVersion:1,evidenceArt:"V5_PR20_7_OCCUPIED_SLOT_LIVE_ONE_SHOT",
    ...
    const [candidateFingerprintSha256,previousFingerprintSha256] = await Promise.all([
      sha256(plan.candidateMaterial),sha256(plan.previousMaterial)
    ]);
    ...
    for (let i = 0; i < SOAK_SAMPLES; i += 1) {
      ...
      if (classified.status !== "COMMITTED") {
        setState({
          ...
          evidence:{
            schemaVersion:1,evidenceArt:"V5_PR20_7_OCCUPIED_SLOT_LIVE_5M",
    ...
    const evidence = {
      ...
      previousSlotItem:{
        name:secondCandidate.previous.name,level:secondCandidate.previous.level,
  ...
  installTelemetryFacade();
  globalThis.V5PR207GearOccupiedLiveTest = Object.freeze({
    version:VERSION,
    testId:TEST_ID,
    status:() => state,
    start:() => run()
  ...
  Promise.resolve().then(run).catch(error => {
    ...
    emit("PR20_7_GEAR_OCCUPIED_SLOT_LIVE_ERROR","ERROR",{
