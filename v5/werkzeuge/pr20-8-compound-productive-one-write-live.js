(() => {
  ...
  async function strictObservation(performance) {
    ...
    if (text(c.ctype || c.type, 32).toLowerCase() !== EXPECTED_CLASS) throw new Error("PR20_8_COMPOUND_LIVE_MERCHANT_ERFORDERLICH");
    ...
    const selectedRows = candidates.slice(0, 3);
    const indexes = selectedRows.map(row => row.index);
    if (new Set(indexes).size !== 3) throw new Error("PR20_8_COMPOUND_LIVE_KANDIDAT_INDEX_DOPPELT");
    const scroll = scrolls[0];
    if (indexes.includes(scroll.index)) throw new Error("PR20_8_COMPOUND_LIVE_INPUT_INDEX_KOLLISION");

    const candidateMaterial = canonical(selectedRows.map(row => [row.index, row.material]));
    const candidate = {
      name: ITEM_NAME,
      level: ITEM_LEVEL,
      indexes,
      ...
      quantityEach: 1,
      baseGold: ITEM_BASE_GOLD,
      materials: selectedRows.map(row => row.material),
    ...
    const conditions = {
      massproductionPresent: !!c.s?.massproduction,
      massproductionppPresent: !!c.s?.massproductionpp
    };
    ...
    const scrollDefMaterial = canonical({
      ...
      grade: Number(scrollDef?.grade),
      g: Number(scrollDef?.g)
    });
    const recipient = {
    ...
    return {
      ...
      recipient,
      service,
      candidate: {...candidate, matchingCandidateCount: candidates.length, fingerprintSha256: candidateFingerprintSha256},
  ...
  function createIntent(observation) {
    ...
    const record = {
      ...
      prerequisiteShadowEvidenceObservedAtMs: RATIFIED_SHADOW_EVIDENCE_OBSERVED_AT_MS,
      ...
      candidate: {
        ...
        quantityEach: observation.candidate.quantityEach,
  ...
  function issueAuthority(intent, observation) {
    ...
    const record = {
      ...
      binding: {
        ...
        massproductionPresent: observation.conditions.massproductionPresent,
        massproductionppPresent: observation.conditions.massproductionppPresent
      }
    };
  ...
  function consumeAuthority(intent, authority, observation) {
    ...
    const indexesMatch = Array.isArray(b.candidateIndexes)
      && b.candidateIndexes.length === observation.candidate.indexes.length
      && b.candidateIndexes.every((value, index) => value === observation.candidate.indexes[index]);
    const matches = authority.testId === TEST_ID
      ...
      && b.massproductionPresent === observation.conditions.massproductionPresent
      && b.massproductionppPresent === observation.conditions.massproductionppPresent;
    if (!matches) {
  ...
  function recoverySnapshot(intent) {
    ...
    if (indexes.length !== 3 || new Set(indexes).size !== 3) {
      return {classification: "UNRESOLVED", reason: "CANDIDATE_INDEX_DRIFT"};
    }
    const slots = indexes.map(index => c.items[index] || null);
    const materials = slots.map(stableItemMaterial);
    const scrollSlot = c.items[intent.scroll.index] || null;
    const scrollMaterial = stableItemMaterial(scrollSlot);
    const qActive = !!c.q?.compound;
    const firstPlaceholder = slots[0]?.name === "placeholder";
    const placeholderCount = c.items.reduce((sum, item) => sum + (item?.name === "placeholder" ? 1 : 0), 0);
    const secondaryConsumed = slots[1] === null && slots[2] === null;
    ...
    const targetSuccess = slots[0]?.name === ITEM_NAME
      && Number(slots[0].level || 0) === ITEM_LEVEL + 1
      && secondaryConsumed;
    const allInputsDestroyed = slots.every(slot => slot === null);
    const inputsUnchanged = materials.length === intent.candidate.materials.length
      && materials.every((material, index) => material === intent.candidate.materials[index]);
    ...
    const compoundEffectUnchanged = compoundEffectMaterial === intent.compoundEffectMaterial;
    const conditionConsumptionReconciled = !massproductionPresent && !massproductionppPresent;
    const conditionStateChanged = !compoundEffectUnchanged;

    let classification = "UNRESOLVED";
    let reason = "POSTCONDITION_WIDERSPRUCH";
    if (!qActive && !firstPlaceholder && targetSuccess && scrollConsumedExactly && conditionConsumptionReconciled) {
      classification = "COMMITTED_SUCCESS";
      reason = null;
    } else if (!qActive && !firstPlaceholder && allInputsDestroyed && scrollConsumedExactly && conditionConsumptionReconciled) {
      classification = "COMMITTED_EXPECTED_FAILURE";
      reason = null;
    } else if (!qActive && placeholderCount === 0 && inputsUnchanged && scrollUnchanged && compoundEffectUnchanged) {
      classification = "NOT_APPLIED";
      reason = null;
    } else if (qActive || firstPlaceholder || secondaryConsumed || scrollConsumedExactly || conditionStateChanged) {
    ...
    return {
      ...
      placeholderCount,
      secondaryConsumed,
      slots: slots.map(slot => slot ? {name: slot.name, level: Number(slot.level || 0), material: stableItemMaterial(slot)} : null),
      ...
      conditionConsumptionReconciled,
  ...
  async function reconcile(intent) {
    ...
    for (let attempt = 0; attempt < RECONCILE_ATTEMPTS; attempt += 1) {
      ...
      if (last.qActive || last.firstPlaceholder || last.secondaryConsumed || last.scrollConsumedExactly || last.conditionStateChanged) {
  ...
  function terminalEvidence(intent, outcome, extra = {}) {
    return {
      ...
      compoundEffectDomainBound: true,
  ...
  async function performFreshOneWrite(performance) {
    ...
    const indexesStable = fresh.candidate.indexes.length === intent.candidate.indexes.length
      && fresh.candidate.indexes.every((value, index) => value === intent.candidate.indexes[index]);
    if (fresh.fingerprints.prestateFingerprintSha256 !== intent.fingerprints.prestateFingerprintSha256
        || !indexesStable || fresh.scroll.index !== intent.scroll.index) {
    ...
    if (durableOwner?.runnerInstanceId !== INSTANCE_ID || durableOwner?.sendCount !== 0 || durableOwner?.status !== "INTENT_DURABLE") {
    ...
    if (consumed.uses !== 1 || consumed.consumed !== true) throw new Error("PR20_8_COMPOUND_LIVE_AUTHORITY_CONSUME_FEHLER");
    state.authority = {...state.authority, authorityConsumed: true, compoundAuthority: true, gameplayAuthority: true, rawWriteAuthority: false};

    const sendFresh = await strictObservation(performance);
    const sendIndexesStable = sendFresh.candidate.indexes.length === intent.candidate.indexes.length
      && sendFresh.candidate.indexes.every((value, index) => value === intent.candidate.indexes[index]);
    if (sendFresh.fingerprints.prestateFingerprintSha256 !== intent.fingerprints.prestateFingerprintSha256
        || !sendIndexesStable || sendFresh.scroll.index !== intent.scroll.index) {
      state.authority = {...state.authority, compoundAuthority: false, gameplayAuthority: false, rawWriteAuthority: false};
      return settleIntent({key: created.key, value: intent}, {
        classification: "NOT_APPLIED",
        reason: "FINAL_SEND_DRIFT",
        acceptedEvidenceObserved: false,
        attempts: 0
      });
    }
    ...
    if (sendPromise) {
      const promiseObservation = await Promise.race([
        sendPromise.then(value => ({kind: "RESOLVED", value}), error => ({kind: "REJECTED", error: text(error?.message || error, 500)})),
  ...
  globalThis.V5PR208CompoundProductiveOneWriteLive = api;
  try {
    const r = root();
    if (r !== globalThis) r.V5PR208CompoundProductiveOneWriteLive = api;
  } catch {}
  try {
    if (globalThis.parent && globalThis.parent !== globalThis) {
      globalThis.parent.V5PR208CompoundProductiveOneWriteLive = api;
    }
  } catch {}
