(() => {
  ...
  const API_NAME = "V5PR208CompoundProductiveOneWriteLive";
  ...
  const SCROLL_TYPE = "cscroll";
  const SCROLL_GRADE = 0;
  const SCROLL_BASE_GOLD = 6400;
  ...
  const RUNTIME_LEASE_KEY = "__V5PR208CompoundProductiveOneWriteLiveLease";
  ...
  function root() {
    ...
    throw new Error("PR20_8_COMPOUND_LIVE_SPIELKONTEXT_FEHLT");
  }
  ...
  async function sha256(value) {
    ...
    if (!cryptoApi?.subtle?.digest) {
      throw new Error("PR20_8_COMPOUND_LIVE_WEB_CRYPTO_UNAVAILABLE");
    }
  ...
  function createInstanceId() {
    ...
    throw new Error("PR20_8_COMPOUND_LIVE_INSTANCE_ID_CRYPTO_UNAVAILABLE");
  }
  ...
  function validCompoundDefinition(def) {
    return !!def
      && def.compound !== null
      && def.compound !== undefined
      && def.compound !== false;
  }
  ...
  function sellDistanceEvidence() {
    ...
    if (Number.isFinite(observed) && observed !== SOURCE_PINNED_SELL_DISTANCE) {
      throw new Error("PR20_8_COMPOUND_LIVE_SELL_DIST_DRIFT");
    }
    return {
  ...
  function serviceReachability(r, c) {
    ...
    if (text(c.map, 96) !== "main") {
      throw new Error("PR20_8_COMPOUND_LIVE_SERVICE_MAP_DRIFT");
    }
    ...
    if (![sx, sy, px, py].every(Number.isFinite)) {
      throw new Error("PR20_8_COMPOUND_LIVE_SERVICE_POSITION_UNLESBAR");
    }
    ...
    if (!Number.isFinite(distance) || distance > conservativeLimit) {
      throw new Error("PR20_8_COMPOUND_LIVE_SERVICE_NICHT_ERREICHBAR");
    }
    return {
      reachable: true,
      viaComputer: false,
  ...
  function scrollRows(items, G) {
    ...
    if (unsafeDefinition(def)
      ...
      throw new Error("PR20_8_COMPOUND_LIVE_SCROLL_DEFINITION_DRIFT");
    }
    const rows = [];
    for (let index = 0; index < items.length; index += 1) {
      const item = items[index];
  ...
  function compoundEffectState(r, c) {
    return {
      massproduction: stableScalarObject(c.s?.massproduction || {}, 32),
      massproductionpp: stableScalarObject(c.s?.massproductionpp || {}, 32),
  ...
  function countItemLevel(items, name, level) {
    return items.reduce((sum, item) => sum + (
      item?.name === name && Number(item.level || 0) === level ? 1 : 0
    ), 0);
  }

  function hasCompoundQueue(c) {
    const q = c.q;
    if (!q || typeof q !== "object") return false;
    return Object.keys(q).some(key => /compound/i.test(key));
  }

  function conditionOutcomeVerified(before, after) {
    for (const key of ["massproduction", "massproductionpp"]) {
      const left = before?.[key] || {};
      const right = after?.[key] || {};
      const wasActive = Object.keys(left).length > 0;
      const same = canonical(left) === canonical(right);
      if (wasActive ? same : !same) return false;
    }
    return true;
  }
  ...
  async function strictObservation(performance) {
    ...
    if (text(c.id, 192) !== EXPECTED_CHARACTER) throw new Error("PR20_8_COMPOUND_LIVE_SESSION_DRIFT");
    if (text(c.ctype, 32).toLowerCase() !== EXPECTED_CLASS) throw new Error("PR20_8_COMPOUND_LIVE_MERCHANT_ERFORDERLICH");
    const server = serverBinding(r);
    if (server.region !== EXPECTED_SERVER_REGION || server.identifier !== EXPECTED_SERVER_IDENTIFIER) throw new Error("PR20_8_COMPOUND_LIVE_SERVER_BINDUNG_DRIFT");
    if (c.rip === true || c.dead === true) throw new Error("PR20_8_COMPOUND_LIVE_CHARACTER_TOT");
    if (c.moving === true) throw new Error("PR20_8_COMPOUND_LIVE_CHARACTER_BEWEGT_SICH");
    if (c.target !== null && c.target !== undefined && text(c.target, 192)) throw new Error("PR20_8_COMPOUND_LIVE_CHARACTER_HAT_ZIEL");
    if (c.q && typeof c.q === "object" && Object.keys(c.q).length) throw new Error("PR20_8_COMPOUND_LIVE_Q_NICHT_FREI");
    const placeholderCount = c.items.reduce((sum, item) => sum + (item?.name === "placeholder" ? 1 : 0), 0);
    if (placeholderCount !== 0) throw new Error("PR20_8_COMPOUND_LIVE_PLACEHOLDER_VORHANDEN");
    const conflict = runtimeConflict(r);
    if (conflict) throw new Error("PR20_8_COMPOUND_LIVE_ALTERNATIVE_RUNTIME_AKTIV:" + conflict);
    if (hostileAggro(r) !== 0) throw new Error("PR20_8_COMPOUND_LIVE_CHARACTER_UNTER_ANGRIFF");
    if (!performance?.active) throw new Error("PR20_8_COMPOUND_LIVE_PERFORMANCE_TRICK_NICHT_AKTIV");
    if (typeof globalThis.compound !== "function") throw new Error("PR20_8_COMPOUND_LIVE_CODE_WRAPPER_COMPOUND_FEHLT");

    const service = serviceReachability(r, c);
    const candidates = candidateRows(c.items, r.G);
    const scrolls = scrollRows(c.items, r.G);
    if (candidates.length < 3) throw new Error("PR20_8_COMPOUND_LIVE_DREI_KANDIDATEN_ERFORDERLICH");
    if (!scrolls.length) throw new Error("PR20_8_COMPOUND_LIVE_SCROLL_FEHLT");
    const selected = candidates.slice(0, 3);
    const scroll = scrolls[0];
    if (selected.some(row => row.index === scroll.index)) throw new Error("PR20_8_COMPOUND_LIVE_INPUT_INDEX_KOLLISION");

    const inventoryMaterial = canonical(c.items.map((item, index) => [index, stableItemMaterial(item)]));
    const qMaterial = canonical(c.q || {});
    const conditions = conditionState(c);
    const conditionStateMaterial = canonical(conditions);
    const compoundEffectMaterial = canonical(compoundEffectState(r, c));
    const candidateMaterial = canonical(selected.map(row => [row.index, row.material]));
    const itemDefMaterial = canonical({
      name: ITEM_NAME,
      type: text(r.G.items[ITEM_NAME].type, 64),
      compoundDefinitionKind: typeof r.G.items[ITEM_NAME].compound,
      compoundDefinitionMaterial: canonical(stableScalarObject(r.G.items[ITEM_NAME].compound, 32)),
      g: Number(r.G.items[ITEM_NAME].g),
      grades: Array.isArray(r.G.items[ITEM_NAME].grades) ? [...r.G.items[ITEM_NAME].grades] : null
    });
    ...
    const candidateFingerprintSha256 = await sha256(candidateMaterial);
    const candidateFingerprintsSha256 = await Promise.all(selected.map(row => sha256(canonical(row))));
    const scrollFingerprintSha256 = await sha256(canonical(scroll));
    const conditionStateFingerprintSha256 = await sha256(conditionStateMaterial);
    const compoundEffectsFingerprintSha256 = await sha256(compoundEffectMaterial);
    ...
    return {
      ...
      candidate: {
        ...
        quantityEach: 1,
        baseGold: ITEM_BASE_GOLD,
        indexes: selected.map(row => row.index),
        materials: selected.map(row => row.material),
        matchingCandidateCount: candidates.length,
        preLevel0Count: countItemLevel(c.items, ITEM_NAME, ITEM_LEVEL),
        preLevel1Count: countItemLevel(c.items, ITEM_NAME, ITEM_LEVEL + 1),
        fingerprintSha256: candidateFingerprintSha256,
        fingerprintsSha256: candidateFingerprintsSha256
      },
      scroll: {...scroll, matchingScrollCount: scrolls.length, fingerprintSha256: scrollFingerprintSha256},
      conditions,
      inventoryMaterial,
      qMaterial,
      conditionStateMaterial,
  ...
  function storage() {
    ...
    if (!ls || typeof ls.getItem !== "function" || typeof ls.setItem !== "function") {
      throw new Error("PR20_8_COMPOUND_LIVE_DURABLE_STORAGE_UNAVAILABLE");
    }
    return ls;
  }
  ...
  function writeJsonExact(key, value) {
    ...
    if (readback !== encoded) {
      throw new Error("PR20_8_COMPOUND_LIVE_DURABLE_READBACK_MISMATCH");
    }
  ...
  function readJson(key) {
    ...
    catch { throw new Error("PR20_8_COMPOUND_LIVE_DURABLE_JSON_BESCHAEDIGT"); }
  }
  ...
  function findExistingIntent() {
    ...
    if (found.length > 1) {
      throw new Error("PR20_8_COMPOUND_LIVE_MEHRERE_INTENTS");
    }
    return found[0] || null;
  }
  ...
  function terminalZeroWriteDuplicateBlockedState(value) {
    return !!value
      ...
      && value.blocker[0] === "PR20_8_COMPOUND_LIVE_DUPLIKAT_INSTANZ_AKTIV";
  }
  ...
  function reclaimStaleRuntimeLease(r, existing) {
    ...
    if (r[RUNTIME_LEASE_KEY]?.instanceId) {
      throw new Error("PR20_8_COMPOUND_LIVE_STALE_RUNTIME_LEASE_RECLAIM_FEHLER");
    }
    emit("PR20_8_COMPOUND_LIVE_STALE_RUNTIME_LEASE_RECLAIMED","warning",{
  ...
  function acquireRuntimeLease() {
    ...
    if (existing?.instanceId && existing.instanceId !== INSTANCE_ID) {
      if (!reclaimStaleRuntimeLease(r, existing)) {
        throw new Error("PR20_8_COMPOUND_LIVE_DUPLIKAT_INSTANZ_AKTIV");
      }
    }
    const lease = {
      schemaVersion: 1,
    ...
    if (r[RUNTIME_LEASE_KEY]?.instanceId !== INSTANCE_ID
      ...
      throw new Error("PR20_8_COMPOUND_LIVE_RUNTIME_LEASE_VERLOREN");
    }
    return lease;
  }
  ...
  function acquireFences(txId) {
    ...
    for (const resource of RESOURCE_CLAIMS) {
      ...
      if (old
          ...
          && old.owner !== owner) {
        throw new Error("PR20_8_COMPOUND_LIVE_FENCE_BELEGT:" + resource);
      }
      const record = {
        schemaVersion: 1,
        testId: TEST_ID,
    ...
    for (const resource of RESOURCE_CLAIMS) {
      ...
      if (current?.owner !== owner
        ...
        throw new Error("PR20_8_COMPOUND_LIVE_FENCE_OWNERSHIP_VERLOREN:" + resource);
      }
    }
    return keys;
  }
  ...
  function createIntent(observation) {
    ...
    const record = {
      ...
      testId: TEST_ID,
      version: VERSION,
      art: "PR20_8_COMPOUND_PRODUCTIVE_ONE_WRITE_INTENT",
      transactionId: txId,
      ...
      publicFunctionSignature: "compound(item0,item1,item2,scroll_num,offering_num,only_calculate)",
      sourceSnapshotCommit: SOURCE_SNAPSHOT_COMMIT,
      prerequisiteShadowObservedAtMs: RATIFIED_SHADOW_OBSERVED_AT_MS,
      ...
      candidate: {
        ...
        indexes: [...observation.candidate.indexes],
        quantity: 3,
        quantityEach: 1,
        baseGold: observation.candidate.baseGold,
        materials: [...observation.candidate.materials],
        preLevel0Count: observation.candidate.preLevel0Count,
        preLevel1Count: observation.candidate.preLevel1Count,
        fingerprintSha256: observation.candidate.fingerprintSha256,
        fingerprintsSha256: [...observation.candidate.fingerprintsSha256]
      },
      scroll: {
      ...
      reconciliationBaseline: {
        conditionStateMaterial: observation.conditionStateMaterial,
        compoundEffectMaterial: observation.compoundEffectMaterial
      },
      offering: null,
      onlyCalculate: false,
      fingerprints: {...observation.fingerprints},
      authority: {
        class: "Pr208CompoundOneShotAuthority",
        maximumUses: 1,
    ...
    if (persisted.runnerInstanceId !== INSTANCE_ID) throw new Error("PR20_8_COMPOUND_LIVE_INTENT_OWNERSHIP_VERLOREN");
  ...
  function issueAuthority(intent, observation) {
    ...
    const record = {
      ...
      binding: {
        ...
        prestateFingerprintSha256: observation.fingerprints.prestateFingerprintSha256,
        inventoryFingerprintSha256: observation.fingerprints.inventoryFingerprintSha256,
        qFingerprintSha256: observation.fingerprints.qFingerprintSha256,
        conditionStateFingerprintSha256: observation.fingerprints.conditionStateFingerprintSha256,
        compoundEffectsFingerprintSha256: observation.fingerprints.compoundEffectsFingerprintSha256,
        runnerInstanceId: INSTANCE_ID,
        candidateFingerprintSha256: observation.fingerprints.candidateFingerprintSha256,
        candidateFingerprintsSha256: [...observation.fingerprints.candidateFingerprintsSha256],
        scrollFingerprintSha256: observation.fingerprints.scrollFingerprintSha256,
        candidateIndexes: [...observation.candidate.indexes],
  ...
  function consumeAuthority(intent, authority, observation) {
    ...
    const matches = authority.testId === TEST_ID
      ...
      && b.prestateFingerprintSha256 === observation.fingerprints.prestateFingerprintSha256
      && b.inventoryFingerprintSha256 === observation.fingerprints.inventoryFingerprintSha256
      && b.qFingerprintSha256 === observation.fingerprints.qFingerprintSha256
      && b.conditionStateFingerprintSha256 === observation.fingerprints.conditionStateFingerprintSha256
      && b.compoundEffectsFingerprintSha256 === observation.fingerprints.compoundEffectsFingerprintSha256
      && b.runnerInstanceId === INSTANCE_ID
      && b.candidateFingerprintSha256 === observation.fingerprints.candidateFingerprintSha256
      && canonical(b.candidateFingerprintsSha256) === canonical(observation.fingerprints.candidateFingerprintsSha256)
      && b.scrollFingerprintSha256 === observation.fingerprints.scrollFingerprintSha256
      && canonical(b.candidateIndexes) === canonical(observation.candidate.indexes)
    ...
    if (!matches) {
      ...
      throw new Error("PR20_8_COMPOUND_LIVE_AUTHORITY_BINDING_DRIFT");
    }
  ...
  function recoverySnapshot(intent) {
    ...
    if (text(c.name, 192) !== intent.recipient.characterName
      ...
      return {classification: "UNRESOLVED", reason: "RECIPIENT_OR_SERVER_DRIFT", acceptedInFlightEvidence: false};
    }
    const qActive = hasCompoundQueue(c);
    const placeholderCount = c.items.reduce((sum, item) => sum + (item?.name === "placeholder" ? 1 : 0), 0);
    const selected = intent.candidate.indexes.map((index, position) => {
      const item = c.items[index] || null;
      const material = stableItemMaterial(item);
      return {
        index,
        position,
        item: item ? {name: item.name, level: Number(item.level || 0)} : null,
        material,
        unchanged: material === intent.candidate.materials[position],
        placeholder: item?.name === "placeholder"
      };
    });
    const allThreeInputsUnchanged = selected.every(row => row.unchanged);
    const anySelectedInputChanged = selected.some(row => !row.unchanged);
    const scrollSlot = c.items[intent.scroll.index] || null;
    const scrollMaterial = stableItemMaterial(scrollSlot);
    const scrollQuantityNow = scrollSlot?.name === SCROLL_NAME ? itemQuantity(scrollSlot) : scrollSlot == null ? 0 : -1;
    const expectedScrollQuantity = Number(intent.scroll.observedQuantity) - SCROLL_CONSUME_QUANTITY;
    const scrollConsumedExactly = scrollQuantityNow === expectedScrollQuantity;
    const scrollUnchanged = scrollMaterial === intent.scroll.material && scrollQuantityNow === intent.scroll.observedQuantity;
    const level0Now = countItemLevel(c.items, ITEM_NAME, ITEM_LEVEL);
    const level1Now = countItemLevel(c.items, ITEM_NAME, ITEM_LEVEL + 1);
    const expectedLevel0After = Number(intent.candidate.preLevel0Count) - 3;
    const successResultCountExact = level0Now === expectedLevel0After && level1Now === Number(intent.candidate.preLevel1Count) + 1;
    const failureResultCountExact = level0Now === expectedLevel0After && level1Now === Number(intent.candidate.preLevel1Count);
    const currentConditions = conditionState(c);
    const conditionStateMaterial = canonical(currentConditions);
    const compoundEffectMaterial = canonical(compoundEffectState(r, c));
    const conditionStateUnchanged = conditionStateMaterial === intent.reconciliationBaseline.conditionStateMaterial;
    const compoundEffectStateUnchanged = compoundEffectMaterial === intent.reconciliationBaseline.compoundEffectMaterial;
    const conditionsVerified = conditionOutcomeVerified(intent.conditions, currentConditions);
    const queueAndPlaceholdersClear = !qActive && placeholderCount === 0;

    let classification = "UNRESOLVED";
    let reason = "POSTCONDITION_WIDERSPRUCH";
    if (queueAndPlaceholdersClear && scrollConsumedExactly && conditionsVerified && successResultCountExact) {
      classification = "COMMITTED_SUCCESS";
      reason = null;
    } else if (queueAndPlaceholdersClear && scrollConsumedExactly && conditionsVerified && failureResultCountExact) {
      classification = "COMMITTED_EXPECTED_FAILURE";
      reason = null;
    } else if (queueAndPlaceholdersClear
        && allThreeInputsUnchanged
        && scrollUnchanged
        && conditionStateUnchanged
        && compoundEffectStateUnchanged
        && level0Now === Number(intent.candidate.preLevel0Count)
        && level1Now === Number(intent.candidate.preLevel1Count)) {
      classification = "NOT_APPLIED";
      reason = null;
    }
    const acceptedInFlightEvidence = qActive
      || placeholderCount > 0
      || scrollConsumedExactly
      || anySelectedInputChanged
      || level0Now < Number(intent.candidate.preLevel0Count)
      || !conditionStateUnchanged
      || !compoundEffectStateUnchanged;
    if (classification === "UNRESOLVED" && acceptedInFlightEvidence) {
    ...
    return {
      ...
      allThreeInputsUnchanged,
      anySelectedInputChanged,
      level0Before: intent.candidate.preLevel0Count,
      level0Now,
      expectedLevel0After,
      level1Before: intent.candidate.preLevel1Count,
      level1Now,
      successResultCountExact,
      failureResultCountExact,
  ...
  async function reconcile(intent) {
    ...
    for (let attempt = 0; attempt < RECONCILE_ATTEMPTS; attempt += 1) {
      ...
      if (last.acceptedInFlightEvidence) acceptedEvidenceObserved = true;
    ...
    return {
      ...(last || {classification: "UNRESOLVED", reason: "NO_OBSERVATION", acceptedInFlightEvidence: false}),
  ...
  function emit(type, severity, data = {}) {
    ...
    events.push({
      ...
      component: "v5-pr20-8-compound-productive-one-write-live",
      data
    });
  ...
  function terminalEvidence(intent, outcome, extra = {}) {
    return {
      evidenceArt: "V5_PR20_8_COMPOUND_PRODUCTIVE_ONE_WRITE_LIVE",
      ...
      publicFunctionSignature: "compound(item0,item1,item2,scroll_num,offering_num,only_calculate)",
      sendArguments: {
        candidateIndexes: [...intent.candidate.indexes],
  ...
  function settleIntent(existing, outcome) {
    ...
    setState({
      ...
      terminal,
      blocker: terminal ? [] : ["PR20_8_COMPOUND_LIVE_RECOVERY_PENDING"],
    ...
    emit(
      terminal ? "PR20_8_COMPOUND_LIVE_TERMINAL" : "PR20_8_COMPOUND_LIVE_RECOVERY_PENDING",
  ...
  async function recoverExisting(existing) {
    ...
    if (intent.schemaVersion !== 1
        ...
        || intent.authority?.class !== "Pr208CompoundOneShotAuthority"
        || intent.sameIntentRetry !== false
        || !Array.isArray(intent.candidate?.indexes)
        || intent.candidate.indexes.length !== 3
        || new Set(intent.candidate.indexes).size !== 3
        || !Number.isInteger(intent.sendCount)
        || intent.sendCount < 0
        || intent.sendCount > 1) {
      throw new Error("PR20_8_COMPOUND_LIVE_EXISTING_INTENT_UNGUELTIG");
    }
    const persistedAuthority = readJson(authorityKey(intent.transactionId));
    setState({
      phase: "RECOVERY",
      status: "RECOVERY",
      intents: [{transactionId: intent.transactionId, status: intent.status, sendCount: intent.sendCount}],
    ...
    const outcome = intent.sendCount === 0 && intent.sendBoundaryState === "NICHT_GESENDET"
  ...
  async function performFreshOneWrite(performance) {
    ...
    if (first.fingerprints.prestateFingerprintSha256 !== second.fingerprints.prestateFingerprintSha256) {
      throw new Error("PR20_8_COMPOUND_LIVE_DOUBLE_OBSERVE_DRIFT");
    }
    ...
    state.intents = [{transactionId: intent.transactionId, status: intent.status, sendCount: intent.sendCount}];
    publishTelemetryFacades();

    acquireFences(intent.transactionId);
    assertRuntimeLease();
    const fresh = await strictObservation(performance);
    if (fresh.fingerprints.prestateFingerprintSha256 !== intent.fingerprints.prestateFingerprintSha256
        || canonical(fresh.candidate.indexes) !== canonical(intent.candidate.indexes)
        || canonical(fresh.candidate.fingerprintsSha256) !== canonical(intent.candidate.fingerprintsSha256)
      ...
      setState({
        ...
        phase: "COMPLETE",
        terminal: true,
        blocker: ["PR20_8_COMPOUND_LIVE_FRESH_ADMISSION_DRIFT"],
    ...
    if (durableOwner?.runnerInstanceId !== INSTANCE_ID
      ...
      throw new Error("PR20_8_COMPOUND_LIVE_INTENT_OWNERSHIP_VERLOREN");
    }
