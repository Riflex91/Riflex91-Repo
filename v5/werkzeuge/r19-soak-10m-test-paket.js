/* AUTO-GENERIERT: V5 R19 SOAK_10M Ingame-Testpaket
 * Quellen:
...
(() => {
  ...
  const API_NAME = 'V5R19Soak10mGui';
  const VERSION = '1.0.0';
  const SESSION_KEY = 'AIO_V5_R19_SOAK_10M_SESSION_V1';
  const PROBE_KEY = 'AIO_V5_R19_SOAK_10M_PROBE_V1';
  const BESTAETIGUNG = 'R19-SOAK-10M-START';
  ...
  function persistenzRoundtrip() {
    ...
    const wert = JSON.stringify({ zeit: Date.now(), probe: 'R19_SOAK_10M' });
    const start = now();
  ...
  function bewerte(session) {
    ...
    const blocker = [];
    if (dauerMs < DAUER_MS) blocker.push('DAUER_UNTER_10M');
    ...
    return {
      schemaVersion: 1,
      test: 'R19_SOAK_10M_CANARY_SCOPE',
      phase: 'R19',
      zertifizierungsStufe: 'SOAK_10M',
      ...
      hinweis: blocker.length
        ? '10-Minuten-Soak nicht bestanden. Nicht wiederholen, bevor der Bericht ausgewertet wurde.'
        : '10-Minuten-Canary-Scope-Soak bestanden. Diese Evidence gibt die breite Runtime nicht frei.'
    };
  }

  const gui = guiApi().erstelleTest({
    kennung: 'r19-soak-10m',
    titel: 'V5 · R19 SOAK 10M · Canary-Scope',
    beschreibung: 'Lueckenlose 10-Minuten-Zeitreihe ohne Gameplay-Writes. Browser-Speicherreserve, Heap, Persistenz-Roundtrip und Sample-Gaps werden fail-closed bewertet.'
  });
  ...
  async function tick() {
    ...
    try {
      ...
      }, 'laeuft', 'SOAK_10M laeuft · Restzeit ' + Math.ceil(Math.max(0, DAUER_MS - elapsed) / 1000) + ' s');
      if (elapsed >= DAUER_MS) {
        laeuft = false;
        ...
        gui.protokolliere('SOAK_10M abgeschlossen', result);
        gui.setzeErgebnis(
          result,
          result.status === 'BESTANDEN' ? 'bestanden' : 'fehler',
          result.status === 'BESTANDEN'
            ? 'SOAK_10M BESTANDEN · Jetzt Gesamtbericht kopieren.'
            : 'SOAK_10M NICHT BESTANDEN · Kein Neustart. Gesamtbericht kopieren.'
        );
      }
    } catch (error) {
      laeuft = false;
      ...
      gui.protokolliere('SOAK_10M FEHLER', message);
    } finally {
      sampling = false;
    }
  }
  ...
  gui.registriereAktion({
    ...
    async ausfuehren() {
      ...
      gui.protokolliere('SOAK_10M Vorpruefung', result);
      gui.setzeErgebnis(
        result,
        result.status === 'BESTANDEN' ? 'bestanden' : 'blockiert',
        result.status === 'BESTANDEN'
          ? 'Vorprüfung bestanden. 10-Minuten-Soak kann gestartet werden.'
  ...
  gui.registriereAktion({
    ...
    async ausfuehren() {
      ...
      laeuft = true;
      starteCountdown();
      gui.protokolliere('SOAK_10M gestartet', {
  ...
  gui.registriereAktion({
    ...
    ausfuehren() {
      ...
      gui.protokolliere('SOAK_10M Zwischenstand', result);
      gui.setzeErgebnis(result, session?.status === 'COMPLETED' ? 'info' : 'laeuft', 'SOAK_10M Zwischenstand.');
      return result;
    }
  });
  ...
  gui.registriereAktion({
    ...
    ausfuehren() {
      ...
      const result = { status: 'ABGEBROCHEN', zertifizierungsStufe: 'SOAK_10M', bestanden: false };
      gui.protokolliere('SOAK_10M abgebrochen', result);
  ...
  gui.protokolliere('R19 SOAK_10M GUI bereit', {
    version: VERSION,
    bestaetigungsText: BESTAETIGUNG,
    dauerMs: DAUER_MS,
    intervallMs: INTERVALL_MS,
