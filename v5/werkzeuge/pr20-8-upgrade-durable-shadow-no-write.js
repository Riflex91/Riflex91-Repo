(() => {
  ...
  function root() {
    ...
    throw new Error('PR20_8_UPGRADE_SHADOW_SPIELKONTEXT_FEHLT');
  }
  ...
  async function sha256(value) {
    ...
    if (!cryptoApi?.subtle?.digest) {
      throw new Error('PR20_8_UPGRADE_SHADOW_WEB_CRYPTO_UNAVAILABLE');
    }
  ...
  function storage() {
    ...
    throw new Error('PR20_8_UPGRADE_SHADOW_DURABLE_STORAGE_UNAVAILABLE');
  }
  ...
  function upgradeFunctionAvailable() {
    for (const candidate of roots()) {
      try {
        if (typeof candidate?.['upgrade'] === 'function') return true;
      } catch {}
    }
    return false;
  }
  ...
  function resolveCandidate(items, G) {
    ...
    for (let index = 0; index < items.length && index < 128; index += 1) {
      ...
      if (!def?.upgrade || itemBlocked(item, def)) continue;
    ...
    if (!matches.length) throw new Error('PR20_8_UPGRADE_SHADOW_CANDIDATE_FEHLT');
    return {
      selected: matches[0],
  ...
  function resolveScroll(items, G) {
    ...
    matches.sort((a,b) => a.index-b.index);
    if (!matches.length) throw new Error('PR20_8_UPGRADE_SHADOW_SCROLL_FEHLT');
    return matches[0];
  }
  ...
  function observation() {
    ...
    if (text(c.name,192) !== EXPECTED_CHARACTER) {
      throw new Error('PR20_8_UPGRADE_SHADOW_RECIPIENT_DRIFT');
    }
    if (text(c.id,192) !== EXPECTED_CHARACTER) {
      throw new Error('PR20_8_UPGRADE_SHADOW_SESSION_DRIFT');
    }
    if (text(c.ctype || c.type,32).toLowerCase() !== EXPECTED_CLASS) {
      throw new Error('PR20_8_UPGRADE_SHADOW_MERCHANT_ERFORDERLICH');
    }
    const server = serverBinding(r);
    if (server.region !== EXPECTED_SERVER_REGION
        || server.identifier !== EXPECTED_SERVER_IDENTIFIER) {
      throw new Error('PR20_8_UPGRADE_SHADOW_SERVER_BINDUNG_DRIFT');
    }
    if (c.rip === true || c.dead === true) {
      throw new Error('PR20_8_UPGRADE_SHADOW_CHARACTER_TOT');
    }
    if (c.moving === true) {
      throw new Error('PR20_8_UPGRADE_SHADOW_CHARACTER_BEWEGT_SICH');
    }
    if (c.target !== null && c.target !== undefined && text(c.target,192)) {
      throw new Error('PR20_8_UPGRADE_SHADOW_CHARACTER_HAT_ZIEL');
    }
    if (c.q && typeof c.q === 'object' && Object.keys(c.q).length) {
      throw new Error('PR20_8_UPGRADE_SHADOW_Q_NICHT_FREI');
    }
    const conflict = runtimeConflict(r);
    if (conflict) {
      throw new Error('PR20_8_UPGRADE_SHADOW_ALTERNATIVE_RUNTIME_AKTIV:' + conflict);
    }
    let hostile = 0;
    try {
      ...
      throw new Error('PR20_8_UPGRADE_SHADOW_AGGRO_UNLESBAR');
    }
    if (hostile !== 0) {
      throw new Error('PR20_8_UPGRADE_SHADOW_CHARACTER_UNTER_ANGRIFF');
    }
    if (!upgradeFunctionAvailable()) {
      throw new Error('PR20_8_UPGRADE_SHADOW_PUBLIC_FUNCTION_FEHLT');
    }
    ...
    if (candidate.selected.index === scroll.index) {
      throw new Error('PR20_8_UPGRADE_SHADOW_INPUT_INDEX_KOLLISION');
    }
  ...
  function persistOrRecoverShadowIntent(record) {
    ...
    const payload = JSON.stringify({
      ...
      testId:TEST_ID,
      version:VERSION,
      art:'PR20_8_UPGRADE_DURABLE_SHADOW_INTENT_NO_GAMEPLAY_WRITE',
      ...
      fingerprints:{
        ...
        upgradeEffects:record.upgradeEffectsFingerprintSha256,
  ...
  function emit(type,severity,data={}) {
    ...
    events.push({
      ...
      component:'v5-pr20-8-upgrade-shadow-no-write',
      data
    });
  ...
  function installTelemetryFacadeOn(owner) {
    if (!owner) return;
    ...
    const existing = owner.AIO_V3.operations && typeof owner.AIO_V3.operations === 'object'
      ...
      : null;
    if (existing?.__v5Pr208UpgradeShadowFacadeVersion === VERSION) return;
  ...
  function installTelemetryFacade() {
    const r = root();
    installTelemetryFacadeOn(r);
    if (globalThis !== r) installTelemetryFacadeOn(globalThis);
    try {
      if (globalThis.parent
          && globalThis.parent !== globalThis
          && globalThis.parent !== r) {
        installTelemetryFacadeOn(globalThis.parent);
      }
    } catch {}
  }
  ...
  async function run() {
    ...
    if(!performanceTrick.active) {
      setState({
        ...
        terminal:true,
        blocker:['PR20_8_UPGRADE_SHADOW_PERFORMANCE_TRICK_NICHT_AKTIV'],
        performanceTrick
      });
      return;
    }
    ...
    if(firstIdentity!==secondIdentity) {
      throw new Error('PR20_8_UPGRADE_SHADOW_PREFLIGHT_SNAPSHOT_DRIFT');
    }
    ...
    const [
      ...
      sha256(second.upgradeEffectMaterial),
    ...
    if(thirdIdentity!==secondIdentity) {
      throw new Error('PR20_8_UPGRADE_SHADOW_POST_INTENT_DRIFT');
    }
    ...
    if(postIntentPrestateFingerprintSha256!==prestateFingerprintSha256) {
      throw new Error('PR20_8_UPGRADE_SHADOW_PRESTATE_HASH_DRIFT');
    }

    const evidence={
      schemaVersion:1,
      evidenceArt:'V5_PR20_8_UPGRADE_DURABLE_SHADOW_NO_WRITE',
      status:'BESTANDEN',
      ...
      candidate:{
        name:second.candidate.name,
        level:second.candidate.level,
        inventoryIndex:second.candidate.index,
        quantity:second.candidate.quantity,
        baseGold:second.candidate.baseGold,
      ...
      oneShotUpgradeAuthorityIssued:false,
      inventoryFencePrepared:true,
      qFencePrepared:true,
      upgradeActionChannelFencePrepared:true,
    ...
    emit('PR20_8_UPGRADE_SHADOW_NO_WRITE_PASSED','INFO',{
      prestateFingerprintSha256,
      candidateIndex:second.candidate.index,
  ...
  Promise.resolve().then(run).catch(error => {
    ...
    emit('PR20_8_UPGRADE_SHADOW_NO_WRITE_ERROR','ERROR',{reason:message});
  });
})();
