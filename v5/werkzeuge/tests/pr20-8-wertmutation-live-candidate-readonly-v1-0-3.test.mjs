...
const source = fs.readFileSync(
  "werkzeuge/pr20-8-wertmutation-live-candidate-readonly-v1-0-3.js",
  "utf8",
);
...
async function run(box) {
  ...
  vm.runInContext(source, box, {
    filename: "pr20-8-wertmutation-live-candidate-readonly-v1-0-3.js",
  });
...
test("PR20.8 candidate scanner v1.0.1 exposes bridge observability synchronously", () => {
  ...
  assert.equal(facade.version, "1.0.3");
  ...
  assert.equal(api?.version, "1.0.3");
});

test("PR20.8 candidate scanner v1.0.3 exposes bridge observability without G/items root", () => {
  const box = sandbox(
