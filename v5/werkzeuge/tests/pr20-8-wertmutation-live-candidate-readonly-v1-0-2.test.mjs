...
const source = fs.readFileSync(
  "werkzeuge/pr20-8-wertmutation-live-candidate-readonly-v1-0-2.js",
  "utf8",
);
...
async function run(box) {
  ...
  vm.runInContext(source, box, {
    filename: "pr20-8-wertmutation-live-candidate-readonly-v1-0-2.js",
  });
...
test("PR20.8 candidate scanner v1.0.1 exposes bridge observability synchronously", () => {
  const box = sandbox([
    { name:"gloves", level:0 },
    { name:"scroll0", q:1 },
  ]);
  ...
  assert.equal(facade.version, "1.0.2");
  ...
  assert.equal(api?.version, "1.0.2");
});

test("PR20.8 candidate scanner v1.0.2 exposes bridge observability without G/items root", () => {
  const box = sandbox(
...
test("PR20.8 Discovery darf mit nur einer verfuegbaren Normalfamilie sauber abschliessen", async () => {
  const status = await run(sandbox([
    { name:"gloves", level:0 },
    { name:"scroll0", q:1 },
  ]));
...
test("PR20.8 Discovery waehlt niedrigwertigen Upgrade-Kandidaten deterministisch", async () => {
  const status = await run(sandbox([
...
test("PR20.8 Discovery blockiert wenn kein einfacher normaler Kandidat existiert", async () => {
  ...
  assert.deepEqual(
    status.blocker,
    ["PR20_8_CANDIDATE_KEIN_NORMALER_NIEDRIGWERTIGER_KANDIDAT"],
  );
