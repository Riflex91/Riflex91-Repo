...
const source = fs.readFileSync(
  "werkzeuge/pr20-8-wertmutation-live-candidate-readonly-v1-0-5.js",
  "utf8",
);
...
async function run(box) {
  ...
  vm.runInContext(source, box, {
    filename: "pr20-8-wertmutation-live-candidate-readonly-v1-0-5.js",
  });
...
test("PR20.8 candidate scanner v1.0.5 exposes bridge observability synchronously", () => {
  const box = sandbox([
    { name:"gloves", level:0 },
    { name:"scroll0", q:1 },
  ]);
  ...
  assert.equal(facade.version, "1.0.5");
  ...
  assert.equal(api?.version, "1.0.5");
});

test("PR20.8 candidate scanner v1.0.5 exposes bridge observability without G/items root", () => {
  const box = sandbox(
...
test("PR20.8 candidate scanner v1.0.5 replaces copied stale facade marker from terminal Compound observer", async () => {
  const items = [
    { name:"gloves", level:0 },
    { name:"scroll0", q:35 },
  ...
  box.AIO_V3 = {
    operations: {
      __v5Pr208CandidateFacadeVersion: "1.0.5",
      status: () => ({
        schemaVersion:1,
        ...
        v5AutonomousTest:{
          ...
          intents:[{transactionId:"source"}],
          normalRuntimeAllowed:false,
        },
      }),
      hostHeartbeat: () => ({
        schemaVersion:1,
        v5TestId:"pr20-8-compound-live-5m",
      }),
    },
  };
  ...
  const immediate = box.AIO_V3.operations.status().v5AutonomousTest;
  assert.equal(immediate.testId, "pr20-8-wertmutation-live-candidate-readonly");
  assert.equal(immediate.version, "1.0.5");
  assert.equal(immediate.status, "BOOT");
  assert.equal(immediate.terminal, false);
  assert.equal(immediate.gameplayWrites, 0);
  assert.equal(immediate.publicFunctionCalls, 0);
  assert.equal(immediate.rawWriteCalls, 0);
  assert.equal(immediate.normalRuntimeAllowed, false);

  for (let i = 0; i < 20; i += 1) {
    await new Promise(resolve => setImmediate(resolve));
    const current = box.V5PR208ValueMutationLiveCandidateReadonly?.status?.();
    if (current?.terminal) break;
  }

  const terminal = box.V5PR208ValueMutationLiveCandidateReadonly.status();
  assert.equal(terminal.terminal, true);
  assert.equal(terminal.status, "BLOCKIERT");
  assert.equal(terminal.observations.EXCHANGE.status, "KEIN_KANDIDAT");
  assert.equal(terminal.observations.EXCHANGE.candidateCount, 0);
  assert.equal(terminal.gameplayWrites, 0);
  assert.equal(terminal.publicFunctionCalls, 0);
  assert.equal(terminal.rawWriteCalls, 0);
  assert.equal(terminal.authority.exchangeAuthority, false);
  assert.equal(terminal.authority.gameplayAuthority, false);
  assert.equal(terminal.authority.rawWriteAuthority, false);
