...
test("PR20.8 candidate scanner v1.0.5 replaces a stale inherited v1.0.4 marker wrapping Compound status", () => {
  const box = sandbox([
    { name:"gloves", level:0 },
    { name:"scroll0", q:35 },
  ]);
  box.AIO_V3 = {
    operations: {
      __v5Pr208CandidateFacadeVersion: "1.0.4",
      status: () => ({
        schemaVersion: 1,
  ...
  const facade = box.AIO_V3?.operations?.status?.()?.v5AutonomousTest;
  assert.equal(box.AIO_V3.operations.__v5Pr208CandidateFacadeVersion, "1.0.5");
  assert.equal(facade?.testId, "pr20-8-wertmutation-live-candidate-readonly");
  assert.equal(facade?.version, "1.0.5");
  assert.equal(facade?.status, "BOOT");
  assert.equal(facade?.terminal, false);
...
test("PR20.8 candidate scanner v1.0.5 does not trust its own stale marker when live status belongs to Compound", () => {
  const box = sandbox([
    { name:"gloves", level:0 },
    { name:"scroll0", q:35 },
  ]);
