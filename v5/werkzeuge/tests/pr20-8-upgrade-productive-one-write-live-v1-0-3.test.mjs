...
const source=fs.readFileSync(
  "werkzeuge/pr20-8-upgrade-productive-one-write-live-v1-0-3.js",
  "utf8",
);
...
function sandbox({
  ...
  const box={
    ...
    G:{
      items:{
        gloves:{type:"gloves",upgrade:{},scroll:true,g:3400},
...
test("PR20.8 v1.0.3 accepts authoritative object-form gloves upgrade definition", async () => {
  const env=sandbox({mode:"success"});
  assert.equal(typeof env.box.G.items.gloves.upgrade,"object");
  assert.equal(Array.isArray(env.box.G.items.gloves.upgrade),false);
  const status=await execute(env);
  assert.equal(status.status,"BESTANDEN");
  assert.equal(status.terminal,true);
  assert.equal(env.upgradeCalls(),1);
...
test("PR20.8 v1.0.3 rejects non-object upgrade definition before intent or send", async () => {
  for (const invalid of [true,false,null,"upgrade"]) {
    const env=sandbox({mode:"success"});
    env.box.G.items.gloves.upgrade=invalid;
    const status=await execute(env);
...
function duplicateBlockedPriorApi(version="1.0.1") {
  const status={
    schemaVersion:1,
    testId:"pr20-8-upgrade-productive-one-write-live",
    version,
...
test("PR20.8 v1.0.2 reclaims only a stale foreign lease after exact zero-write duplicate failure", async () => {
  const env=sandbox({mode:"success"});
  env.box.V5PR208UpgradeProductiveOneWriteLive=duplicateBlockedPriorApi();
  env.box.__V5PR208UpgradeProductiveOneWriteLiveLease={
    schemaVersion:1,
    testId:"pr20-8-upgrade-productive-one-write-live",
    instanceId:"foreign-v1-0-0-instance",
    acquiredAtMs:env.clock.now()-120001,
  };
  ...
  assert.equal(status.sameIntentRetry,false);
  assert.ok(
    env.box.V5PR208UpgradeProductiveOneWriteLive
      .telemetry()
      .some(row => row.type==="PR20_8_UPGRADE_LIVE_STALE_RUNTIME_LEASE_RECLAIMED"),
  );
});

test("PR20.8 v1.0.2 never reclaims a fresh foreign lease", async () => {
  const env=sandbox({mode:"success"});
  env.box.V5PR208UpgradeProductiveOneWriteLive=duplicateBlockedPriorApi();
  env.box.__V5PR208UpgradeProductiveOneWriteLiveLease={
    schemaVersion:1,
    testId:"pr20-8-upgrade-productive-one-write-live",
    instanceId:"fresh-foreign-instance",
    acquiredAtMs:env.clock.now()-1000,
  };
  ...
  assert.equal(status.publicFunctionCalls,0);
  assert.equal(status.rawWriteCalls,0);
  assert.match(status.blocker[0],/DUPLIKAT_INSTANZ_AKTIV/);
  assert.equal(
    env.box.__V5PR208UpgradeProductiveOneWriteLiveLease.instanceId,
    "fresh-foreign-instance",
  );
});

test("PR20.8 v1.0.2 never reclaims stale lease when durable mutation guard state exists", async () => {
  const storage=new MemoryStorage();
  storage.setItem(
    "v5:pr20-8-upgrade-productive-one-write-live:intent:guard",
    JSON.stringify({testId:"pr20-8-upgrade-productive-one-write-live"}),
  );
  const env=sandbox({mode:"success",storage});
  env.box.V5PR208UpgradeProductiveOneWriteLive=duplicateBlockedPriorApi();
  env.box.__V5PR208UpgradeProductiveOneWriteLiveLease={
    schemaVersion:1,
    testId:"pr20-8-upgrade-productive-one-write-live",
    instanceId:"stale-but-guarded-instance",
    acquiredAtMs:env.clock.now()-180000,
  };
...
test("PR20.8 Upgrade one-write package exposes exact safety markers and no raw socket write", () => {
  for(const marker of [
    ...
    '"PR20_8_UPGRADE_LIVE_STALE_RUNTIME_LEASE_RECLAIMED"',
