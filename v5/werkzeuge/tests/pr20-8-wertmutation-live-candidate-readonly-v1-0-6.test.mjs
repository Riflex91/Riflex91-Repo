...
const source = fs.readFileSync(
  "werkzeuge/pr20-8-wertmutation-live-candidate-readonly-v1-0-6.js",
  "utf8",
);
...
async function run(items) {
  ...
  vm.runInContext(source, box, {
    filename:"pr20-8-wertmutation-live-candidate-readonly-v1-0-6.js",
  });
...
test("v1.0.6 ist ausschliesslich ein neuer read-only Rescan-Epoch", () => {
  assert.ok(source.includes("const VERSION = '1.0.6';"));
  assert.equal(
    source.replace("const VERSION = '1.0.6';", "const VERSION = '1.0.5';"),
    previous,
  );
});

test("v1.0.6 bleibt ohne mutierenden Gameplay-Pfad und ohne Authority", () => {
  for (const marker of [
    "upgrade(",
    "compound(",
    "exchange(",
...
test("v1.0.6 no-candidate Lauf bleibt terminal fail-closed und zero-write", async () => {
  const {box,status} = await run([
    {name:"gloves",level:0},
    {name:"scroll0",q:35},
    {name:"anniversarygift",q:106},
  ]);
  assert.equal(box.V5PR208ValueMutationLiveCandidateReadonly.version,"1.0.6");
...
test("v1.0.6 erkennt einen natuerlichen Exchange-Kandidaten nur read-only", async () => {
