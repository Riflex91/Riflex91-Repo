...
const source=fs.readFileSync(
  "werkzeuge/pr20-8-exchange-anniversarygift-service-mount-v1-0-0.js",
  "utf8",
);
...
function makeBox({
  ...
  if(withSmartMove){
    box.smart_move=(destination)=>{
      ...
      box.character.x=-25;
      box.character.y=-478;
      box.character.real_x=-25;
      box.character.real_y=-478;
...
async function execute(env){
  ...
  vm.runInContext(source,env.box,{
    filename:"pr20-8-exchange-anniversarygift-service-mount-v1-0-0.js",
  });
  const deadline=Date.now()+4000;
  let last=null;
  while(Date.now()<deadline){
    await new Promise(resolve=>setImmediate(resolve));
    last=env.box.V5PR208ExchangeAnniversarygiftServiceMount?.status?.();
...
test("already in safe Exchange range completes with zero movement writes",async()=>{
  ...
  assert.equal(status.nextAction,
    "RESTORE_ANNIVERSARYGIFT_PRODUCTIVE_ONE_WRITE_MANIFEST");
});
...
test("out-of-range Merchant issues exactly one smart_move exchange and arrives",async()=>{
  ...
  assert.ok(status.recipient.distanceToExchange<=300);
  assert.equal(status.exchangeAuthority,false);
...
test("reloading after a completed movement reconciles without second smart_move",async()=>{
  ...
  delete env.box.V5PR208ExchangeAnniversarygiftServiceMount;
...
test("existing sent intent outside service range never sends a second movement",async()=>{
  const seed=new Map();
  const key="v5:pr20-8-exchange-anniversarygift-service-mount:intent:exchange-service";
  seed.set(key,JSON.stringify({
    schemaVersion:1,
    testId:"pr20-8-exchange-anniversarygift-service-mount",
    version:"1.0.0",
    target:"exchange",
    sourceCommit:"90052162eb3ebda36c893e1eb4af643913c8f984",
    sourceMapsBlob:"78350dac1a18c4eb0e7c6f545ebf08bb3d6729e4",
    sendCount:1,
...
test("missing smart_move blocks before any gameplay write",async()=>{
  ...
  assert.equal(status.status,"BLOCKIERT");
  assert.ok(status.blocker.includes(
    "PR20_8_EXCHANGE_SERVICE_MOUNT_SMART_MOVE_FEHLT"
  ));
  assert.equal(env.getMoveCalls(),0);
  assert.equal(status.gameplayWrites,0);
  assert.equal(status.publicFunctionCalls,0);
...
test("package contains exactly one smart_move call surface and no Exchange write surface",()=>{
  ...
  assert.equal(source.includes('const TARGET = "exchange"'),true);
