...
const source=fs.readFileSync('werkzeuge/pr21-merchant-integration-live-15m-v1-0-2.js','utf8');
const previous=fs.readFileSync('werkzeuge/pr21-merchant-integration-live-15m-v1-0-1.js','utf8');
const authorization=JSON.parse(fs.readFileSync('roadmap/pr21-merchant-live-execution-authorization-v3.json','utf8'));
...
function sandbox({nativeHealth='HEALTHY',nativeDropped=0,captureErrors=0,preRunning=false}={}){
  ...
  const nativeOperations={
    ...
    status:()=>({
      ...
      health:{state:nativeHealth,snapshotAgeMs:1000,heartbeatAgeMs:1000},
...
async function execute(box){
  vm.runInNewContext(source,box,{filename:'pr21-merchant-integration-live-15m-v1-0-2.js'});
  const api=box.V5PR21MerchantIntegrationLive15mV102;
  await api.start();
  return {api,status:api.status(),counts:box.__counts()};
}

test('v3 authorization binds parser-only recovery to the same PR21 checkpoint',()=>{
  ...
  assert.equal(authorization.scope.testId,'pr21-merchant-integration-live-15m-v1-0-2');
  assert.equal(authorization.scope.controllerVersion,'1.0.2');
  ...
  assert.equal(authorization.parserCompatibility.semanticChangeAllowed,false);
  assert.equal(authorization.parserCompatibility.sameRuntimeAndSafetyContractAsV1_0_1,true);
});

test('Adventure Land parser-risk constructs introduced in v1.0.1 are removed',()=>{
  assert.match(previous,/for\s*\(\s*const\s*\[[^\]]+\]\s+of/);
  assert.match(previous,/\)\?\./);
  assert.doesNotMatch(source,/for\s*\(\s*const\s*\[[^\]]+\]\s+of/);
  assert.doesNotMatch(source,/\)\?\./);
  assert.doesNotMatch(source,/async async/);
  assert.ok(source.includes("const VERSION = '1.0.2'"));
  assert.ok(source.includes("const TEST_ID = 'pr21-merchant-integration-live-15m-v1-0-2'"));
  assert.ok(source.includes("const API_NAME = 'V5PR21MerchantIntegrationLive15mV102'"));
});

test('parser compatibility recovery preserves clean 15m behavior',async()=>{
...
test('pre-running Merchant runtime remains untouched by parser-compatible observer',async()=>{
...
test('unhealthy native runtime still fails closed',async()=>{
  const box=sandbox({nativeHealth:'DEGRADED'});
  vm.runInNewContext(source,box,{filename:'pr21-merchant-integration-live-15m-v1-0-2.js'});
  const api=box.V5PR21MerchantIntegrationLive15mV102;
  await assert.rejects(api.start(),/PR21_MERCHANT_RUNTIME_READINESS_TIMEOUT/);
...
test('v1.0.2 still exposes no direct gameplay/raw-write surface',()=>{
  for(const marker of [
