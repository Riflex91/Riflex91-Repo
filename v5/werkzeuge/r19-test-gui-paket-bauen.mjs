...
const HEADER = `/* AUTO-GENERIERT: V5 R19 Ingame-Testpaket
 * Quellen:
...
const ziel = "werkzeuge/r19-controlled-live-test-paket.js";
const erwartet = HEADER + gui + "\n\n" + controller + "\n";

if (process.argv.includes("--check")) {
  if (!fs.existsSync(ziel)) throw new Error("R19_TEST_GUI_PAKET_FEHLT");
  const ist = fs.readFileSync(ziel, "utf8");
  if (ist !== erwartet) throw new Error("R19_TEST_GUI_PAKET_NICHT_SOURCE_LOCKED");
  console.log("[V5-R19-TEST-GUI] Source-Lock OK");
} else {
  fs.writeFileSync(ziel, erwartet, "utf8");
  console.log("[V5-R19-TEST-GUI] Paket erzeugt:", ziel);
}
