...
for (const marker of [
  ...
  if (!bankWithdrawAuthority.includes(marker)) {
    fehler.push("BANK_WITHDRAW_AUTHORITY_QUELLE_FEHLT:" + marker);
  }
}
for (const [kennung, muster] of [
  ["BANK_WITHDRAW_WRITE", /\bbank_withdraw\s*\(/],
  ...
  if (muster.test(bankWithdrawAuthority)) {
    fehler.push("BANK_WITHDRAW_AUTHORITY_NO_WRITE_VERLETZT:" + kennung);
  }
}

const bankWithdrawAuthorityAdapter = liesText(
...
for (const marker of [
  ...
  if (!bankWithdrawAuthorityAdapter.includes(marker)) {
    fehler.push("BANK_WITHDRAW_AUTHORITY_ADAPTER_FEHLT:" + marker);
  }
}

const bankWithdrawJournal = liesText(
...
for (const marker of [
  ...
  if (!bankWithdrawJournal.includes(marker)) {
    fehler.push("BANK_WITHDRAW_JOURNAL_GRENZE_FEHLT:" + marker);
  }
}

const bankWithdrawRuntime = liesText(
  "grundlage/quelle/runtime/produktions-runtime.ts",
);
for (const marker of [
  "erteileBankWithdrawEinmalAuthority",
  ...
  if (!bankWithdrawRuntime.includes(marker)) {
    fehler.push("BANK_WITHDRAW_RUNTIME_AUTHORITY_FEHLT:" + marker);
  }
}

const bankWithdrawHost = liesText(
  "grundlage/quelle/host/produktions-host-controller.ts",
);
for (const marker of [
  "erteileBankWithdrawEinmalAuthority",
  ...
  if (!bankWithdrawHost.includes(marker)) {
    fehler.push("BANK_WITHDRAW_HOST_AUTHORITY_FEHLT:" + marker);
  }
}


const bankWithdrawPreflightBrowser = liesText(
...
for (const marker of [
  ...
  if (!bankWithdrawPreflightBrowser.includes(marker)) {
    fehler.push("BANK_WITHDRAW_PREFLIGHT_BROWSER_MARKER_FEHLT:" + marker);
  }
}
for (const [kennung, muster] of [
  ["BANK_WITHDRAW_WRITE", /\bbank_withdraw\s*\(/],
  ["BANK_DEPOSIT_WRITE", /\bbank_deposit\s*\(/],
  ["BANK_STORE_WRITE", /\bbank_store\s*\(/],
  ["BANK_RETRIEVE_WRITE", /\bbank_retrieve\s*\(/],
  ["RAW_EMIT", /\.emit\s*\(/],
]) {
  if (muster.test(bankWithdrawPreflightBrowser)) {
    fehler.push("BANK_WITHDRAW_PREFLIGHT_BROWSER_NO_WRITE_VERLETZT:" + kennung);
  }
}

const bankWithdrawPreflightRunner = liesText(
...
for (const marker of [
  ...
  if (!bankWithdrawPreflightRunner.includes(marker)) {
    fehler.push("BANK_WITHDRAW_PREFLIGHT_RUNNER_MARKER_FEHLT:" + marker);
  }
}
for (const [kennung, muster] of [
  ["AUTHORITY_GRANT", /erteileBankWithdrawEinmalAuthority\s*\(/],
  ["BANK_WITHDRAW_WRITE", /\bbank_withdraw\s*\(/],
  ["RAW_EMIT", /\.emit\s*\(/],
]) {
  if (muster.test(bankWithdrawPreflightRunner)) {
    fehler.push("BANK_WITHDRAW_PREFLIGHT_RUNNER_NO_WRITE_VERLETZT:" + kennung);
  }
}

const bankWithdrawShadow = liesText(
...
for (const marker of [
  ...
  if (!bankWithdrawShadow.includes(marker)) {
    fehler.push("BANK_WITHDRAW_SHADOW_MARKER_FEHLT:" + marker);
  }
}
for (const [kennung, muster] of [
  ["BANK_WITHDRAW_WRITE", /\bbank_withdraw\s*\(/],
  ...
  if (muster.test(bankWithdrawShadow)) {
    fehler.push("BANK_WITHDRAW_SHADOW_NO_WRITE_VERLETZT:" + kennung);
  }
}
if (!bankWithdrawRuntime.includes("fuehreBankWithdrawShadowAdmission")
    || !bankWithdrawRuntime.includes(
...
const bankWithdrawRealShadowEvidence = JSON.parse(
  liesText("roadmap/pr20-2-bank-withdraw-real-browser-shadow-evidence.json"),
);
...
for (const [key, value] of Object.entries(
  bankWithdrawRealShadowEvidence.exitGate ?? {},
)) {
  if (value !== true) {
    fehler.push("BANK_WITHDRAW_REAL_SHADOW_EXIT_GATE_FEHLT:" + key);
  }
}
