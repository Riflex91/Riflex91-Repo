(() => {
  ...
  const VERSION = '1.0.0';
  const TEST_ID = 'pr20-7-gear-occupied-slot-shadow-no-write';
  ...
  function persistShadowIntent(record) {
    ...
    if (!storage) throw new Error('PR20_7_GEAR_SHADOW_DURABLE_STORAGE_UNAVAILABLE');
    const key = 'v5:' + TEST_ID + ':intent:' + record.prestateFingerprintSha256;
    const payload = JSON.stringify({
      schemaVersion: 1,
      testId: TEST_ID,
      art: 'SHADOW_DURABLE_INTENT_NO_GAMEPLAY_WRITE',
      ...
      slot: record.slot,
      inventoryIndex: record.inventoryIndex,
      prestateFingerprintSha256: record.prestateFingerprintSha256,
      resourceClaims: [
        'character:' + record.recipient.characterName + ':equipment',
        'character:' + record.recipient.characterName + ':inventory'
      ]
    });
    storage.setItem(key, payload);
    const readback = storage.getItem(key);
    if (readback !== payload) {
      throw new Error('PR20_7_GEAR_SHADOW_DURABLE_READBACK_MISMATCH');
    }
    let decoded = null;
    ...
    if (!decoded
        ...
        || decoded.swapWriteRatification !== false) {
      throw new Error('PR20_7_GEAR_SHADOW_DURABLE_INTENT_INVALID');
    }
    return {
      storage: 'LOCAL_STORAGE_SHADOW_ONLY',
      keySha256Material: key,
  ...
  function emit(type, severity, data = {}) {
    ...
    events.push({
      ...
      component: 'v5-pr20-7-gear-shadow-no-write',
      data
    });
  ...
  async function run() {
    ...
    emit('PR20_7_GEAR_SHADOW_NO_WRITE_STARTED', 'INFO');
    ...
    if (!performanceTrick.active) {
      ...
      emit('PR20_7_GEAR_SHADOW_NO_WRITE_BLOCKED', 'ERROR', {
    ...
    if (!firstCandidate) {
      setState({
        status: 'BLOCKIERT',
        phase: 'REAL_SHADOW_NO_WRITE',
        terminal: true,
        blocker: ['PR20_7_GEAR_KEIN_SICHERER_BELEGTER_SLOT_KANDIDAT'],
        performanceTrick,
        observedRecipient: {
          ...
          occupiedSafeSlots: SAFE_SLOTS.filter(slot => first.slots[slot] !== null),
          inventoryItemCount: first.inventory.length
        }
      });
      emit('PR20_7_GEAR_SHADOW_NO_WRITE_BLOCKED', 'WARN', {
        reason: 'PR20_7_GEAR_KEIN_SICHERER_BELEGTER_SLOT_KANDIDAT'
      });
      return;
    }
    ...
    const [
      ...
      sha256(secondCandidate.previous.material),
    ...
    const thirdCandidate = chooseCandidate(third);
    if (!thirdCandidate) {
      throw new Error('PR20_7_GEAR_SHADOW_KANDIDAT_NACH_INTENT_VERLOREN');
    }
    const thirdIdentity = stableCandidateIdentity(third, thirdCandidate);
    if (thirdIdentity !== secondIdentity) {
      throw new Error('PR20_7_GEAR_SHADOW_POST_INTENT_DRIFT');
    }
    ...
    if (postIntentPrestateFingerprintSha256 !== prestateFingerprintSha256) {
      throw new Error('PR20_7_GEAR_SHADOW_PRESTATE_HASH_DRIFT');
    }

    const evidence = {
      schemaVersion: 1,
      evidenceArt: 'V5_PR20_7_OCCUPIED_SLOT_REAL_BROWSER_SHADOW_NO_WRITE',
      status: 'BESTANDEN',
      observedAtMs: Date.now(),
      recipient: {
        characterName: second.characterName,
      ...
      shadowIntent,
      shadowDurableIntentCreated: true,
      shadowDurableReadback: true,
      oneShotBindingPrepared: true,
      equipmentInventoryFenceClaimsPrepared: true,
      sendBoundaryState: 'NICHT_GESENDET',
    ...
    emit('PR20_7_GEAR_SHADOW_NO_WRITE_PASSED', 'INFO', {
  ...
  installTelemetryFacade();
  globalThis.V5PR207GearShadowTest = Object.freeze({
    version: VERSION,
    testId: TEST_ID,
    status: () => state,
    start: () => run()
  ...
  Promise.resolve().then(run).catch(error => {
    ...
    emit('PR20_7_GEAR_SHADOW_NO_WRITE_ERROR', 'ERROR', { reason: message });
  });
})();
