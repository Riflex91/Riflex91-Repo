/* AUTO-GENERIERT: V5 R19 Canary Ingame-Testpaket
 * Quellen:
...
(() => {
  ...
  const API_NAME = 'V5R19CanaryGui';
  const VERSION = '1.0.0';
  const JOURNAL_KEY = 'AIO_V5_R19_CANARY_JOURNAL_V1';
  const PROBE_KEY = 'AIO_V5_R19_CANARY_PROBE_V1';
  const BESTAETIGUNG = 'R19-CANARY-EQUIP-ONCE';
  ...
  function waehleKandidat(obs) {
    ...
    const hardErlaubt = obs.inventar
      ...
      .map(item => {
        ...
        const slotPrioritaet = SLOT_PRIORITAET.indexOf(slot);
        const basisScore = (vorher === null ? 1000 : 0) + (100 - slotPrioritaet) * 10 - item.index;
        return {
          index: item.index,
          itemName: item.name,
    ...
    if (!hardErlaubt.length) return null;

    const deterministisch = [...hardErlaubt]
      .sort((a,b)=>b.basisScore-a.basisScore || a.index-b.index)[0];

    const MAX_DELTA = 25;
    const bewertet = hardErlaubt.map(item => {
      const raw = Math.max(-MAX_DELTA, Math.min(MAX_DELTA, item.itemLevel * 4 - item.index / 10));
      return { ...item, learningDelta: raw, learningScore: item.basisScore + raw };
    });
    const learning = [...bewertet]
      .sort((a,b)=>b.learningScore-a.learningScore || a.index-b.index)[0];

    return {
      ...learning,
      learningEinfluss: {
        schemaVersion: 1,
        modellKennung: 'r19-canary-bounded-test-ranker',
        modellVersion: '1',
        testModell: true,
        ...
        deterministischerFallbackIndex: deterministisch.index,
        deterministischerFallbackItem: deterministisch.itemName,
        gewaehltIndex: learning.index,
        gewaehltItem: learning.itemName,
        hardErlaubteKandidaten: hardErlaubt.length
      }
    };
  }
  ...
  function passiveVorpruefung() {
    ...
    return {
      schemaVersion: 1,
      test: 'R19_CANARY_EQUIP_CERTIFICATION',
      phase: 'R19',
  ...
  const gui = guiApi().erstelleTest({
    kennung: 'r19-canary-equip',
    titel: 'V5 · R19 Canary · Equip-Zertifizierung',
  ...
  gui.registriereAktion({
    ...
    async ausfuehren() {
      if (sendVerbraucht) throw new Error('R19_CANARY_ONE_SHOT_BEREITS_VERBRAUCHT');
      ...
      const intent = {
        ...
        maximaleAktionen: 1,
        learningEinfluss: kandidat.learningEinfluss,
  ...
  gui.protokolliere('R19 Canary-Test-GUI bereit', {
    version: VERSION,
    bestaetigungsText: BESTAETIGUNG,
    actionContractId: ACTION,
    learningModell: 'r19-canary-bounded-test-ranker',
    zertifizierungsStufe: 'CANARY',
    maximaleAktionen: 1,
    sameIntentRetry: false,
    breiteRuntimeFreigabe: false
