(() => {
  ...
  function sellDistanceEvidence() {
    ...
    if (Number.isFinite(observed) && observed !== SOURCE_PINNED_SELL_DISTANCE) {
      throw new Error('PR20_7_ACQUISITION_SELL_DIST_DRIFT');
    }
    return {
  ...
  function observe() {
    ...
    if (server.region !== EXPECTED_SERVER_REGION || server.identifier !== EXPECTED_SERVER_IDENTIFIER) {
      throw new Error('PR20_7_ACQUISITION_SERVER_BINDUNG_DRIFT');
    }
    ...
    const sellDist = Number(sellDistance.value);
    if (!Number.isFinite(sellDist) || sellDist <= 0) {
      throw new Error('PR20_7_ACQUISITION_SELL_DIST_UNGUELTIG');
    }
    ...
    return {
      ...
      acquisition: {
        ...
        sellDistance: sellDist,
        sellDistanceSource: sellDistance.source,
        sellDistanceBrowserObserved: sellDistance.browserObserved,
        sourcePinnedSellDistance: sellDistance.sourcePinnedSellDistance,
        officialServerSourceCommit: sellDistance.officialServerSourceCommit,
        officialServerBlobSha: sellDistance.officialServerBlobSha
      }
    };
  }
  ...
  function stableIdentity(value) {
    return JSON.stringify({
      ...
      acquisition: {
        ...
        sellDistanceSource: value.acquisition.sellDistanceSource,
        sellDistanceBrowserObserved: value.acquisition.sellDistanceBrowserObserved,
        sourcePinnedSellDistance: value.acquisition.sourcePinnedSellDistance,
        officialServerSourceCommit: value.acquisition.officialServerSourceCommit,
        officialServerBlobSha: value.acquisition.officialServerBlobSha
      }
    });
  }
