(() => {
  ...
  const TEST_ID='pr20-9-craft-durable-shadow-no-write';
  const VERSION='1.0.0';
  const EXPECTED_GLOBAL='V5PR209CraftDurableShadowNoWrite';
  ...
  function emit(type,severity,data={}) {
    ...
    events.push({
      ...
      component:'v5-pr20-9-craft-durable-shadow-no-write',
      data
    });
  ...
  function finish(status,blocker=[],evidence=null) {
    setState({status,phase:'COMPLETE',terminal:true,blocker,evidence});
  }
  ...
  async function execute() {
    ...
    if(!first.scan.selected) {
      finish('BLOCKIERT',['PR20_9_CRAFT_SHADOW_KEIN_NORMALKANDIDAT'],{
