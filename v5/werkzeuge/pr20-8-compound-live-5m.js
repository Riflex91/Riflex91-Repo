(() => {
  ...
  function ensurePerformanceTrick() {
    const r = root();
    const fn = typeof r.performance_trick === "function"
      ...
      : typeof globalThis.performance_trick === "function"
        ...
        : null;
    if (!fn) throw new Error("PR20_8_COMPOUND_5M_PERFORMANCE_TRICK_FEHLT");
    try { fn.call(r); }
    catch (error) {
      throw new Error(
        "PR20_8_COMPOUND_5M_PERFORMANCE_TRICK_FEHLER:" +
        text(error?.message || error, 200)
      );
    }
    const sound = r.sounds?.empty || globalThis.sounds?.empty;
    let playing = false;
    try {
      playing = typeof sound?.playing === "function"
        ? !!sound.playing()
        : !!sound?.playing;
    } catch {}
    const active = !!sound && (sound.cplaying === true || playing === true);
    if (!active) {
      throw new Error("PR20_8_COMPOUND_5M_PERFORMANCE_TRICK_NICHT_AKTIV");
    }
    return {
      active: true,
      audioFound: !!sound,
      cplaying: sound?.cplaying === true,
      playing
    };
  }
  ...
  async function run() {
    ...
    try {
      ...
      const performance = ensurePerformanceTrick();
