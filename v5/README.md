...
PR20.1 Equip ist real abgeschlossen. **PR20.2 Bank ist breit freigegeben** (`VOLL_FREIGEGEBEN_MIT_DOKUMENTIERTEN_EVIDENCE_AUSNAHMEN`) und blockiert die Roadmap nicht mehr. Deposit ist real bestanden, Retrieve/Store/Swap jeweils 2/2 bestanden und der Bank-NO-WRITE-5M-Lauf ist bestanden. Withdraw bleibt historisch ohne vollstaendige Live-Evidence und lokal gegatet; Open-Pack bleibt bis ausreichenden Ressourcen durch Admission blockiert. Das aktive Gate ist **PR20.3 Markt**. `buy_with_gold(item, 1)` und der persistente NPC-Sell-Stufentest sind jeweils real 7/7 bestanden, jeweils mit zwei bestaetigten Live-Settlements und bestandenem 5m-NO-WRITE-Lauf; beide 2/2-Live-Testbudgets sind ausgeschoepft. Die Sell-Evidence ist wegen eines nicht kopierten GUI-Gesamtberichts transparent aus den operatorbestaetigten Ergebnissen rekonstruiert. `trade_buy`/`trade_sell` sind fuer die Roadmap operator-abgenommen, ohne Live-Evidence zu behaupten. Naechster Schritt ist die formale PR20.3-Exit-Gate-Pruefung; produktive Market-Gesamt-Authority bleibt bis dahin gesperrt.
