...
PR20.1 Equip ist real abgeschlossen. **PR20.2 Bank ist breit freigegeben** (`VOLL_FREIGEGEBEN_MIT_DOKUMENTIERTEN_EVIDENCE_AUSNAHMEN`) und blockiert die Roadmap nicht mehr. Deposit ist real bestanden, Retrieve/Store/Swap jeweils 2/2 bestanden und der Bank-NO-WRITE-5M-Lauf ist bestanden. Withdraw bleibt historisch ohne vollstaendige Live-Evidence und lokal gegatet; Open-Pack bleibt bis ausreichenden Ressourcen durch Admission blockiert. PR20.3 Markt ist fuer die Roadmap formal abgeschlossen. `buy_with_gold(item, 1)` und NPC-Sell sind real 7/7 mit je 2/2 bestaetigten Settlements und 5m-NO-WRITE bestanden. Die Sell-Evidence ist transparent rekonstruiert, weil der GUI-Gesamtbericht nicht kopiert wurde. `trade_buy`/`trade_sell` zaehlen per Operator-Ratifikation fuer die Roadmap als abgeschlossen, jedoch nicht als Live-Evidence und ohne automatische produktive Trade-Authority. Das aktive Gate ist jetzt **PR20.4 Logistik/Transfers**; dessen Vorbereitung bleibt NO-WRITE/default-off.
