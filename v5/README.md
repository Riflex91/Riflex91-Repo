...
PR20.1 Equip ist real abgeschlossen. **PR20.2 Bank ist breit freigegeben** (`VOLL_FREIGEGEBEN_MIT_DOKUMENTIERTEN_EVIDENCE_AUSNAHMEN`) und blockiert die Roadmap nicht mehr. Deposit ist real bestanden, Retrieve/Store/Swap jeweils 2/2 bestanden und der Bank-NO-WRITE-5M-Lauf ist bestanden. Withdraw bleibt historisch ohne vollstaendige Live-Evidence und lokal gegatet; Open-Pack bleibt bis ausreichenden Ressourcen durch Admission blockiert. Das aktive Gate ist **PR20.3 Markt**. Der erste Pfad `buy_with_gold(item, 1)` ist real 7/7 bestanden: zwei exakte Live-Settlements plus bestandener 5m-NO-WRITE-Lauf. Das 2/2-Buy-Gold-Testbudget ist ausgeschöpft. Naechstes Ingame-Gate ist der bereits gemergte persistente NPC-Sell-Stufentest; produktive Market-Gesamt-Authority bleibt weiterhin gesperrt.
