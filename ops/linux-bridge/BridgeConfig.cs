...
public sealed record BridgeConfig
{
    ...
    public string DashboardListenUrl { get; init; } = "http://127.0.0.1:8791";
    ...
    public string LiveWissensdatenbankPfad { get; init; } = DefaultLiveKnowledgePath();
    ...
    public static string AppDirectory => ResolveXdgDirectory("XDG_CONFIG_HOME", ".config", "aio-bot-linux-bridge");
    public static string LocalAppDirectory => ResolveXdgDirectory("XDG_STATE_HOME", Path.Combine(".local", "state"), "aio-bot-linux-bridge");
    ...
    public static string StatePath => Path.Combine(LocalAppDirectory, "bridge-state.json");
    public static string StatusPath => Path.Combine(LocalAppDirectory, "bridge-status.json");
    public static string SelfUpdateDirectory => Path.Combine(LocalAppDirectory, "self-update");

    // Compatibility names for the store classes. Linux secrets are not written to these paths.
    public static string TokenPath => "secret-service:telemetry-token";
    public static string WebDashboardWriteKeyPath => "secret-service:web-dashboard-write-key";
    public static string BackblazeCredentialsPath => "secret-service:backblaze-credentials";
    ...
    public static async Task<BridgeConfig> LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            loaded = loaded with { ConfigVersion = CurrentConfigVersion };
    ...
    public void Validate()
    {
        ...
        if (!Uri.TryCreate(DashboardListenUrl, UriKind.Absolute, out var dashboard)
            || dashboard.Scheme != Uri.UriSchemeHttp
            || !IsLoopback(dashboard.Host))
            throw new InvalidOperationException("DASHBOARD_MUST_BE_LOOPBACK_HTTP");
        ...
        var browsers = new[] { "Brave", "Chrome", "Chromium", "Edge" };
        if (!browsers.Contains(PreferredBrowser, StringComparer.OrdinalIgnoreCase))
    ...
    public static string NormalisiereLiveWissenspfad(string wert)
    {
        ...
        if (!Path.IsPathFullyQualified(voll))
            throw new InvalidOperationException("LIVE_WISSEN_PFAD_MUSS_ABSOLUT_SEIN");

        var wurzel = Path.GetPathRoot(voll);
        if (string.IsNullOrWhiteSpace(wurzel) || string.Equals(voll, Path.TrimEndingDirectorySeparator(wurzel), StringComparison.Ordinal))
            throw new InvalidOperationException("LIVE_WISSEN_DATEISYSTEMWURZEL_VERBOTEN");

        return voll;
    }

    private static string DefaultLiveKnowledgePath() =>
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "AdventureLand-V5", "wissensdatenbank");

    private static string ResolveXdgDirectory(string variable, string fallbackRelative, string appName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(configured) && Path.IsPathFullyQualified(configured))
            ...
            root = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), fallbackRelative);
    ...
    private static void EnsureLinux()
    {
        if (!OperatingSystem.IsLinux())
            throw new PlatformNotSupportedException("AioBotLinuxBridge requires Linux.");
    }

    internal static void TryRestrictFile(string path)
    {
        try
        {
...
public sealed record BridgeState(long LastEventSeq = 0)
{
    ...
    public async Task SaveAsync(CancellationToken cancellationToken = default)
    {
        ...
        BridgeConfig.TryRestrictFile(BridgeConfig.StatePath);
    }
}
    ...
    string? BackblazeError)
{
    public async Task SaveAsync(CancellationToken cancellationToken = default)
    {
        ...
        await JsonSerializer.SerializeAsync(stream, this, BridgeConfig.JsonOptions, cancellationToken);
        BridgeConfig.TryRestrictFile(BridgeConfig.StatusPath);
    }
}
