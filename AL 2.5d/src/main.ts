...
async function boot(): Promise<void> {
  ...
  const hud = new HudOverlay(
    {
      ...
      onPartyInvite: (name) => {
        ...
        return legacyRuntime.dispatchPartyInvite(name);
      },
      onPartyRequest: (name) => {
        if (!legacyRuntime?.ready) {
          throw new Error("Original Adventure Land runtime is not ready");
      ...
      onPartyLeave: () => {
        ...
        return legacyRuntime.dispatchPartyLeave();
      }
    },
    document.body
  );
  ...
  const findEntityHit = (clientX: number, clientY: number) => {
    ...
    const chestHit = latestSnapshot?.lootChests
      ?.map((chest) => {
        const screen = worldToViewport(chest, camera, viewport);
        const dx = point.x - screen.x;
        const dy = point.y - (screen.y - 12 * camera.zoom);
        const rx = 20 * camera.zoom;
        const ry = 18 * camera.zoom;
        const score = (dx * dx) / (rx * rx) + (dy * dy) / (ry * ry);
        return { chest, score };
      })
      .filter((candidate) => candidate.score <= 1)
    ...
    return { rect, point, viewport, hit, chestHit };
  };
  ...
  host.addEventListener("pointerup", (event) => {
    ...
    const { rect, point, viewport, hit, chestHit } =
      findEntityHit(event.clientX, event.clientY);
    try {
      if (chestHit) {
        const result = legacyRuntime.dispatchChestOpen(chestHit.chest.id);
        if (result instanceof Promise) {
          void result.catch((error) =>
            console.warn("AL 2.5D chest open failed", error)
          );
        }
        event.preventDefault();
        return;
  ...
  host.addEventListener("contextmenu", (event) => {
    ...
    const { hit, chestHit } = findEntityHit(event.clientX, event.clientY);
    if (!hit && !chestHit) return;
