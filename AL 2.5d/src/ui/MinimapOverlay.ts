...
export class MinimapOverlay {
  ...
  render(snapshot: GameFrameSnapshot): void {
    ...
    const loot = snapshot.lootChests?.length ?? 0;
    this.meta.textContent =
      `${monsters} HOSTILE · ${players} PLAYER${loot ? ` · ${loot} LOOT` : ""}`;
    ...
    for (const chest of snapshot.lootChests ?? []) {
      const point = mapPoint(chest.x, chest.y);
      ctx.save();
      ctx.translate(point.x, point.y);
      ctx.fillStyle = chest.opening ? "#ffe39a" : "#cda85d";
      ctx.fillRect(-2.5, -2.5, 5, 5);
      ctx.strokeStyle = "rgba(255, 234, 173, .72)";
      ctx.lineWidth = 1;
