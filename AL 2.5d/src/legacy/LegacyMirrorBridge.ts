...
export function classifySkillVisual(
  action: string,
  definition:
    | Readonly<Record<string, unknown>>
    | undefined
): RenderSkillVisualKind {
  if (action === "use_hp" || action === "use_mp") return "support";
  if (action === "attack") return "offense";

  const explanation = stringValue(definition?.explanation)?.toLowerCase() ?? "";
  const hostile = definition?.hostile === true;
  const damage = finiteNumber(definition?.damage);
  const damageType = stringValue(definition?.damage_type);
  const condition = stringValue(definition?.condition);
  const aura = definition?.aura === true;
  const normalized = action.toLowerCase();

  if (
    normalized === "blink" ||
    normalized === "charge" ||
    /teleport|dash|rush|leap/.test(explanation)
  ) {
    return "mobility";
  }
  ...
  if (
    /heal|restore|revive|recover/.test(normalized) ||
    /heal|restore|revive|recover/.test(explanation)
  ) {
    return "support";
  }

  if (condition || aura || /shield|guard|protect|armor/.test(explanation)) {
    return "defense";
  }
  ...
  for (const key of globals.skillbar ?? []) {
    ...
    hotbar.push(
      Object.freeze({
        ...
        visualKind: classifySkillVisual(action, skillDefinitions?.[action])
      })
    );
  }
  ...
  for (const [name, definition] of Object.entries(skillDefinitions ?? {})) {
    ...
    skills.push(
      Object.freeze({
        ...
        visualKind: classifySkillVisual(name, definition),
        ...(key ? { key } : {}),
...
export function snapshotLegacyChests(
  globals: LegacyGlobalsLike,
  mapId: string
): readonly RenderEntity[] {
  const result: RenderEntity[] = [];

  for (const [chestId, rawChest] of Object.entries(globals.chests ?? {})) {
    const chest = recordValue(rawChest);
    if (!chest) continue;

    const map = stringValue(chest.map);
    if (map && map !== mapId) continue;

    const x =
      finiteNumber(chest.real_x) ??
      finiteNumber(chest.x);
    const y =
      finiteNumber(chest.real_y) ??
      finiteNumber(chest.y);
    if (x === undefined || y === undefined) continue;

    const items = finiteNumber(chest.items);
    const alpha = finiteNumber(chest.alpha);
    ...
    result.push(
      Object.freeze({
        ...
        interaction: Object.freeze({
          kind: "loot-chest" as const,
          id: chestId
        }),
        name:
          items !== undefined
            ? `Loot Chest · ${Math.max(0, Math.floor(items))} item${items === 1 ? "" : "s"}`
            : "Loot Chest",
...
export class LegacyMirrorBridge {
  ...
  renderOnce(): GameFrameSnapshot {
    ...
    const chests = snapshotLegacyChests(globals, map);
    const worldSnapshot: GameFrameSnapshot = chests.length
      ? Object.freeze({
          ...entitySnapshot,
