...
export function snapshotLegacyLootChests(
  globals: LegacyGlobalsLike,
  mapId: string
): readonly RenderLootChest[] {
  const result: RenderLootChest[] = [];

  for (const [key, raw] of Object.entries(globals.chests ?? {})) {
    const chest = recordValue(raw);
    if (!chest) continue;

    const x = finiteNumber(chest.x);
    const y = finiteNumber(chest.y);
    if (x === undefined || y === undefined) continue;

    const map = stringValue(chest.map);
    if (map && map !== mapId) continue;

    const id = stringValue(chest.id) ?? key;
    if (!id) continue;
