...
export type LegacyCompatibilitySource = LegacyGlobalsLike & {
  ...
  party_say?: (message: string) => unknown;
...
export class LegacyCompatibilityRuntime {
  ...
  dispatchChestLoot(chestId: string): unknown {
    const id = chestId.trim();
    if (!id) {
      throw new Error("Chest id is required");
    }

    const source = this.requireSource();
    if (typeof source.open_chest !== "function") {
      throw new Error("Original Adventure Land open_chest() is not available");
    }

    return source.open_chest.call(source, id);
  }
  ...
  dispatchChatMessage(request: Readonly<{
    kind: "main" | "party" | "pm";
    message: string;
    peer?: string;
  }>): unknown {
    ...
    if (request.kind === "main") {
      if (typeof source.say !== "function") {
        throw new Error("Original Adventure Land say() is not available");
      }
      return source.say.call(source, message);
    }

    if (request.kind === "party") {
      if (typeof source.party_say !== "function") {
        throw new Error("Original Adventure Land party_say() is not available");
      }
      return source.party_say.call(source, message);
    }

    const peer = request.peer?.trim();
    if (!peer) {
      throw new Error("Private-message recipient is required");
    }
    if (typeof source.private_say !== "function") {
      throw new Error("Original Adventure Land private_say() is not available");
    }
