...
class AndroidHeadlessRuntime(context: Context) : AutoCloseable {
    ...
    @Synchronized fun ensureStarted() {
        ...
        if (config == null || config.characters.isEmpty()) { event("NATIVE_CLIENTLESS_CONFIG_REQUIRED", "WARNING"); return }
        manager = NativeClientlessManager(config, logicUpdater.active()).also { it.start() }
        running = true; startedAt = System.currentTimeMillis(); event("ANDROID_NATIVE_CLIENTLESS_STARTED", "INFO")
    }
    ...
    @Synchronized fun refreshLogic(force: Boolean = false) {
        ...
        val result = logicUpdater.checkNow(); logicVersion = result.version; logicUpdateError = result.error
        if (result.changed) {
            event("BOT_LOGIC_UPDATED", "INFO", result.version)
            if (running) {
                manager?.close()
                val config = NativeClientlessConfigStore(appContext).load()
                manager = if (config != null && config.characters.isNotEmpty()) NativeClientlessManager(config, logicUpdater.active()).also { it.start() } else null
            }
        } else if (result.error != null) event("BOT_LOGIC_UPDATE_FAILED", "WARNING", result.error)
    }
    ...
    fun debugEvents(limit: Int): JSONObject { val rows = JSONArray(); synchronized(this) { events.takeLast(limit).forEach { rows.put(JSONObject(it.toString())) } }; return JSONObject().put("ok", true).put("events", rows) }
    ...
    @Synchronized private fun event(name: String, severity: String, reason: String? = null) { val row = JSONObject().put("event", name).put("severity", severity).put("eventAt", System.currentTimeMillis()); if (reason != null) row.put("reason", reason); events.addLast(row); while (events.size > 100) events.removeFirst() }
    override fun close() { running = false; manager?.close(); manager = null }

    companion object { const val LOGIC_CHECK_INTERVAL_MS = 60_000L }
}
