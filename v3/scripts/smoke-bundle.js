...
assert.equal(status.brain.mode, 'teacher-student-strategic-brain-v2');
assert.equal(status.brain.operatingMode, 'shadow');
...
assert.ok(sandbox.AIO_V3.liveServices);
assert.ok(sandbox.AIO_V3.cloud);
assert.ok(sandbox.AIO_V3.autoUpdate);
const liveServices = sandbox.AIO_V3.liveServices.status();
assert.equal(liveServices.mode, 'production-live-services-v1');
assert.ok(liveServices.cloud);
assert.ok(liveServices.autoUpdater);
assert.equal(sandbox.AIO_V3.__runtime.productionLiveServices.cloudControlPlaneInstalled, true);
assert.equal(sandbox.AIO_V3.__runtime.productionLiveServices.safeAutoUpdaterInstalled, true);
...
assert.ok(sandbox.AIO_V3_V5_TEST_BOOTSTRAP);
assert.equal(typeof sandbox.AIO_V3_V5_TEST_BOOTSTRAP.status, 'function');
assert.equal(sandbox.AIO_V3_V5_TEST_BOOTSTRAP.status().windowsBridgeDeployment, false);
sandbox.AIO_V3_V5_TEST_BOOTSTRAP.stop();
assert.equal(sandbox.AIO_V3_V5_TEST_BOOTSTRAP.status().running, false);
