...
test('Alpha20.19 stale offer expires and Merchant status exposes explicit loot signal and 300 potion reserve', () => {
  ...
  assert.equal(logistics._safeLootCandidate(runtime.adapter.snapshot()), null);
  root.character.name = 'My_Merchant'; root.character.ctype = 'merchant'; root.character.items = [null, null]; root.character.isize = 2;
...
test('visible integrated release version is Alpha20.23', () => {
  assert.equal(RELEASE_VERSION, '3.0.0-alpha.20.134');
});
