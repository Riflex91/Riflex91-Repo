...
function updaterFixture(overrides = {}) {
  return {
    localVersion: '3.0.0-alpha.20.32',
    pendingVersion: '3.0.0-alpha.20.32',
    lastApply: null,
...
test('planned auto-update reload is handed off and emitted by the newly loaded runtime', async () => {
  ...
  const firstUpdater = updaterFixture({
    lastApply: {
      at: 1_000_000,
      from: '3.0.0-alpha.20.32',
      to: '3.0.0-alpha.20.32',
      slot: 7,
      bytes: 1_234_567,
      saved: true,
      reloaded: false
  ...
  assert.equal(markerSeenDuringReload.from, '3.0.0-alpha.20.32');
  assert.equal(markerSeenDuringReload.to, '3.0.0-alpha.20.32');
  ...
  assert.equal(secondUpdater.lastApply.from, '3.0.0-alpha.20.32');
  assert.equal(secondUpdater.lastApply.to, '3.0.0-alpha.20.32');
