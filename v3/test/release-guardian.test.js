...
function fixture() {
  ...
  fs.writeFileSync(path.join(root, 'version.json'), JSON.stringify({ version: '3.0.0-alpha.20.71' }));
  fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ version: '3.0.0-alpha.20.71' }));
  fs.writeFileSync(path.join(root, 'src', 'release-version.js'), expectedReleaseVersionSource('3.0.0-alpha.20.71'));
  fs.writeFileSync(path.join(root, 'dist', 'aio-v3.js'), '/* Adventure Land AiO Bot 3.0.0-alpha.20.71 | generated | shadow mode by default */\n');
  return root;
}
