...
class CloudControlPlane {
  constructor(options = {}) {
    ...
    this.stats = { runtimePushes: 0, configPulls: 0, configChanges: 0, teacherCalls: 0, teacherBlocked: 0, teacherErrors: 0, feedbackPushes: 0, brainImports: 0, failures: 0 };
    this._load();
  }
  ...
  async syncState() {
    ...
    if (result.settings && this.control) {
      ...
      if (patch.changed.length) this.stats.configChanges += patch.changed.length;
      ...
      if (patch.changed.length && this.log) this.log.emit({ component: 'cloud-control-plane', event: 'REMOTE_SETTINGS_APPLIED', data: { changed: patch.changed.map((x) => x.key), hotApplied: applied.applied, restartRequired: applied.restartRequired, revision: this.remoteRevision } });
    }
  ...
  async cycle() {
    ...
    this.busy = true;
    try {
      const now = this.now(); const outcome = this.brain && this.brain.tickOutcome ? this.brain.tickOutcome() : null;
      if (outcome) this.pendingFeedback.push(outcome);
  ...
  status() {
    return {
      schemaVersion: 1, mode: 'cloudflare-v3-control-plane-v1', enabledBySettings: !!(this.control && this.control.get('cloud.enabled', false)), ready: !!(this.credentials.baseUrl && this.credentials.writeKey && this.fetchFn), configured: { baseUrl: this.credentials.baseUrl || null, account: this.credentials.account, writeKeyPresent: !!this.credentials.writeKey },
      ...
      policies: { secretsNeverExposedInStatus: true, onlyMerchantCallsTeacher: true, cloudFailureDoesNotBlockLocalRuntime: true, remoteSettingsValidatedLocally: true, brainHasNoDirectExecutorAccess: true }
    };
  }
}
