...
function eventSignal(event) {
  ...
  return {
    ...
    type: type || 'UNKNOWN_PROBLEM',
    severity: severity || 'UNKNOWN',
    reason: reason || type || 'UNKNOWN_PROBLEM',
...
function hostSignal(host = {}) {
  if (host.harnessError) {
    return {
      seq: 0,
      type: 'HOST_HARNESS_ERROR',
      severity: 'ERROR',
      reason: String(host.harnessError).slice(0, 300),
      component: 'production-host-harness',
      character: null,
      fingerprint: `production-host-harness|HOST_HARNESS_ERROR|${String(host.harnessError).slice(0, 180)}`
    };
  }
  ...
  if (state && !['HEALTHY', 'STARTING', 'UNKNOWN'].includes(state)) {
    return {
      seq: 0,
      type: 'HOST_WATCHDOG_UNHEALTHY',
      severity: state === 'CRITICAL' || state === 'DEAD' ? 'CRITICAL' : 'ERROR',
      reason: state,
      component: 'host-watchdog',
      character: null,
      fingerprint: `host-watchdog|HOST_WATCHDOG_UNHEALTHY|${state}`
    };
  }
  return null;
}
...
class ProblemDiagnosticsArchive {
  constructor(options = {}) {
    ...
    this.stats = { ticks: 0, captures: 0, deduped: 0, failures: 0, prunedFiles: 0, prunedBytes: 0, uploadFlushes: 0 };
  }
  ...
  _isDuplicate(fingerprint, now) {
    const at = this.fingerprints.get(fingerprint);
    return at != null && now - at < this.dedupeMs;
  }

  async _ensureDirs() {
    await fs.mkdir(path.join(this.spoolDir, 'pending'), { recursive: true });
  }
  ...
  async _enforceRetention() {
    ...
    try {
      ...
      for (const name of names.filter((entry) => /^problem-.*\.json\.gz$/.test(entry))) {
        ...
        try {
          ...
          rows.push({ name, fullPath, bytes: stat.size, mtimeMs: stat.mtimeMs });
        } catch (_) {}
      }
    } catch (error) {
    ...
    while (rows.length > this.maxPendingFiles || bytes > this.maxPendingBytes) {
      ...
      if (!row) break;
      await fs.rm(row.fullPath, { force: true });
      await fs.rm(`${row.fullPath}.meta.json`, { force: true });
      bytes -= row.bytes;
      this.stats.prunedFiles += 1;
      this.stats.prunedBytes += row.bytes;
    }
  }
  ...
  async _writeBundle(bundle, signal) {
    ...
    const metadata = {
      ...
      day: bundle.capturedAt.slice(0, 10),
  ...
  async tick(botClient, host = {}) {
    ...
    if (!botClient || typeof botClient.debugEvents !== 'function' || typeof botClient.debugSnapshot !== 'function') {
      return { captured: false, reason: 'PROBLEM_DIAGNOSTICS_BOT_CLIENT_UNAVAILABLE' };
    }

    const now = this.now();
    try {
      const batch = await botClient.debugEvents(this.lastEventSeq, this.eventLimit);
      const events = Array.isArray(batch && batch.events) ? batch.events : [];
      ...
      const bundle = {
        ...
        host: sanitize({
          ...
          watchdogState: host.watchdogState || null,
          harnessError: host.harnessError || null
        }),
      ...
      this.lastBundle = { ...written };
      this.lastError = null;
      this.stats.captures += 1;
      this._remember(signal.fingerprint, now);
      const upload = await this.flush();
  ...
  status() {
    return {
      mode: 'host-problem-diagnostics-archive',
      enabled: this.enabled(),
      botId: this.botId,
      spoolDir: this.spoolDir,
      eventLimit: this.eventLimit,
      dedupeMs: this.dedupeMs,
      maxPendingFiles: this.maxPendingFiles,
      maxPendingBytes: this.maxPendingBytes,
      lastEventSeq: this.lastEventSeq,
      lastCaptureAt: this.lastCaptureAt,
      lastBundle: this.lastBundle && { ...this.lastBundle, localPath: undefined },
      lastError: this.lastError && { ...this.lastError },
      lastUploadResult: this.lastUploadResult && { ...this.lastUploadResult },
