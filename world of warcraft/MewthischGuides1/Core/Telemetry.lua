...
local function safe(value,depth)
    ...
    for key,child in pairs(value) do
        count=count+1
        if count>16 then out["<truncated>"]=true break end
        out[tostring(key)]=safe(child,depth+1)
    end
    return out
end
...
function T:Count(name,data)
    ...
    self.last[name]={at=time and time() or 0,data=safe(data)}
    if MG.db then
        MG.db.runtime=MG.db.runtime or {}
        MG.db.runtime.telemetry={
            counters=self.counters,last=self.last,localOnly=true,
        }
    end
end

function T:GetSnapshot()
    return {counters=self.counters,last=self.last,localOnly=true}
end
