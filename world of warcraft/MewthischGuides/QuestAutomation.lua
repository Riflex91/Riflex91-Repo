            ...
            for _, info in ipairs(quests) do
                if info and info.questID and info.isComplete then
                    ...
                    self:Log("INFO", "quest.auto.select_turnin", "Fertige Gossip-Quest automatisch ausgewaehlt.", {
                        questID = info.questID, title = info.title,
                    })
                    local selected, err = pcall(C_GossipInfo.SelectActiveQuest, info.questID)
                    if not selected then self:Log("WARN", "quest.auto.select_turnin_failed", tostring(err), { questID = info.questID }) end
                    return
                end
            end
        end
            ...
            for _, info in ipairs(quests) do
                if info and info.questID and not info.isIgnored then
                    ...
                    self:Log("INFO", "quest.auto.select_available", "Verfuegbare Gossip-Quest automatisch ausgewaehlt.", {
                        questID = info.questID, title = info.title, trivial = info.isTrivial and true or false,
                    })
                    local selected, err = pcall(C_GossipInfo.SelectAvailableQuest, info.questID)
                    if not selected then self:Log("WARN", "quest.auto.select_available_failed", tostring(err), { questID = info.questID }) end
                    return
                end
            end
        end
...
function MG:HandleQuestAutomationEvent(event)
    ...
    if event == "QUEST_DETAIL" and self.db.settings.autoAcceptQuests then
        ...
        if AcceptQuest then
            ...
            if not ok then self:Log("WARN", "quest.auto.accept_failed", tostring(err), { questID = questID }) end
        else
    ...
    if event == "QUEST_PROGRESS" and self.db.settings.autoTurnInQuests then
        ...
        if completable and CompleteQuest then
            ...
            self:Log("INFO", "quest.auto.complete_attempt", "Fertige Quest wird automatisch fortgesetzt.", { questID = questID })
            local ok, err = pcall(CompleteQuest)
            if not ok then self:Log("WARN", "quest.auto.complete_failed", tostring(err), { questID = questID }) end
        end
        return
    end
    ...
    if event == "QUEST_COMPLETE" and self.db.settings.autoTurnInQuests then
        ...
        if GetQuestReward then
            ...
            if not ok then self:Log("WARN", "quest.auto.turnin_failed", tostring(err), { questID = questID, choices = choices }) end
        else
