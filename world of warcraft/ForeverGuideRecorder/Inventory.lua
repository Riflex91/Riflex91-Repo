...
local function observeItem(itemID, itemLink, source)
    ...
    FGR:EnsureDB()
    FGR.db.seenItems = FGR.db.seenItems or {}

    local key = tostring(itemLink or itemID)
    if FGR.db.seenItems[key] then return end
    FGR.db.seenItems[key] = true

    local data = itemMetadata(itemID, itemLink)
    data.source = source
    FGR:Record("item.observed", itemID, data, "gameplay-api")
end
...
local function scanBags(reason)
    ...
    if not lastBagCounts or reason == "login" then
        FGR:Record("inventory.snapshot", nil, {
            reason = reason,
            items = items,
            position = FGR:GetPosition(),
        }, "gameplay-api")
        ...
        if #delta > 0 then
            FGR:Record("inventory.delta", nil, {
                reason = reason,
                changes = delta,
                position = FGR:GetPosition(),
            }, "gameplay-api")
