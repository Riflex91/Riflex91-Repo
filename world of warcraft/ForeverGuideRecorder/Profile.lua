...
function FGR:RecordTalentProfile(reason)
...
frame:SetScript("OnEvent", function(_, event)
    if event == "PLAYER_LOGIN" then
        C_Timer.After(3, function() FGR:RecordTalentProfile("login") end)
    else
        C_Timer.After(0.5, function() FGR:RecordTalentProfile(event) end)
    end
end)
