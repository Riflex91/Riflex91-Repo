...
export function hasResolutionSuffix(value,scale) {
  if(typeof value!=="string"||!Number.isInteger(scale)) return false;
...
export function replacementPolicyFor(p) {
  ...
  return {
    kind,
    requiresOriginalFallback:true,
    requiresUniformIntegerScale:true,
    requiresLogicalSizePreservation:true,
    requiresResolutionAwareTexture:kind==="world-atlas"||kind==="entity-sprite"||kind==="cosmetic-sprite"||kind==="vfx",
    manualRectScalingRequired:false,
    activationAllowedBeforeOverrideLayer:false
  };
}
