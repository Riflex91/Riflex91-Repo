...
function imageSize(file){
  ...
  if(e===".svg"){
    ...
    const width=Number((t.match(/\bwidth=["'](\d+(?:\.\d+)?)["']/)||[])[1]||0);
    const height=Number((t.match(/\bheight=["'](\d+(?:\.\d+)?)["']/)||[])[1]||0);
...
for(const item of manifest.replacements||[]){
  ...
  if(item.scale!==manifest.rules.defaultScale && !(typeof item.scaleExceptionReason==="string"&&item.scaleExceptionReason.trim())) errors.push(item.sourcePath+": non-default scale requires scaleExceptionReason");
  ...
  if(!original||!high){errors.push(item.sourcePath+": unsupported image format for dimension validation");continue;}
...
console.log("HD asset manifest valid:",(manifest.replacements||[]).length,"replacements; default scale=8x");
