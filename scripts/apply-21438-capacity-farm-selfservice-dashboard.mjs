...
const patch=fs.readFileSync('scripts/v21438-bot-patch.txt','utf8'),patchMarker='/* v2.14.38 targeted capacity relief, rate-ranked fallback, autonomous farmer loot service and dashboard heartbeat */';
if(!bot.includes(patchMarker)){
  ...
  const end=bot.lastIndexOf('\n})();');if(end<0)throw new Error('bot IIFE end missing');bot=bot.slice(0,end)+patch+bot.slice(end);
}else{
  const start=bot.lastIndexOf('\n\n  '+patchMarker),end=bot.lastIndexOf('\n})();');if(start<0||end<0||start>=end)throw new Error('existing v2.14.38 patch bounds missing');bot=bot.slice(0,start)+patch+bot.slice(end);
}
...
v.version='2.14.38';v.dashboardVersion='2.14.38';v.build='2026-09-11';fs.writeFileSync(vp,JSON.stringify(v,null,2)+'\n');
const pp='cloudflare-dashboard/package.json',p=JSON.parse(fs.readFileSync(pp,'utf8'));p.version='2.14.38';fs.writeFileSync(pp,JSON.stringify(p,null,2)+'\n');
