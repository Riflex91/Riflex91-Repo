...
r=await worker.fetch(new Request('https://example.test/api/brain-status',{headers:{'x-aio-read-key':'read-test'}}),env);j=await r.json();assert.equal(j.ok,true);assert.equal(j.usage.limit,10000);assert.equal(j.usage.target,9950);assert.equal(j.students[0].stats.promoted,true);assert.equal(j.students[0].stats.league.canaryActive,true);assert.equal(j.students[0].stats.league.generation,2);assert.equal(j.students[0].stats.life.state,'challenging');assert.equal(j.students[0].stats.quality.status,'healthy');assert.equal(j.students[0].stats.quality.score,93);assert.equal(j.students[0].stats.quality.autonomyAllowed,true);assert.equal(j.students[0].stats.diary.entries[0].title,'Teacher-Lektion');assert.equal(j.students[0].stats.diary.stats.teacherLessons,1);assert.equal(j.liveBrain.life.state,'thinking');assert.equal(j.liveBrain.league.generation,2);assert.equal(j.liveBrain.quality.status,'watch');assert.equal('w1' in j.students[0].stats,false);assert.ok(j.decisions.length>=1);assert.ok(j.events.length>=1);
...
console.log('Worker Brain v2.13 smoke OK · state sync · teacher · league feedback · living dashboard · hard budget');
