...
if (versionMatch) {
  ...
  ok(version.version === "2.14.6", "prepared release must be 2.14.6");
  ok(version.dashboardVersion === "2.14.5", "dashboard version must be 2.14.6 for dashboard release");
...
ok(bot.includes("function v2145MovePriority"), "2.14.6 Merchant move arbitration missing");
ok(bot.includes("move_deferred") && bot.includes("requestedPriority"), "2.14.6 route deferral diagnostics missing");
ok(bot.includes("function v2145VendorReady") && bot.includes("dist(character,dest)<=75"), "2.14.6 settled vendor guard missing");
ok(bot.includes("maxed-upgrade-surplus") && bot.includes("safe-trash-surplus"), "2.14.6 safe surplus selling missing");
ok(bot.includes("function v2145EconomyMaintenanceTick"), "2.14.6 economy maintenance priority missing");
ok(bot.includes(".mainbox .maincontent{overflow-y:auto"), "2.14.6 main GUI scrolling missing");
ok(bot.includes("vital('HP',r.hp,r.max_hp,'hp')") && bot.includes("vital('MP',r.mp,r.max_mp,'mp')") && bot.includes("v2145VitalFlow"), "2.14.6 animated party HP/MP bars missing");
ok(dash.includes('data-section="characters"') && dash.includes('data-section="group"'), "2.14.6 dashboard collapsible sections missing");
