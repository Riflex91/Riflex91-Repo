...
if (versionMatch) {
  ...
  ok(version.version === "2.14.22", "prepared release must be 2.14.22");
  ok(version.dashboardVersion === "2.14.22", "dashboard version must be 2.14.22 for layered brain/dashboard release");
...
ok(bot.includes('S.exchangeRouteFlight21418') && bot.includes("Eintausch-Anreise läuft") && bot.includes("merchant_exchange_route_timeout"), '2.14.22 exchange route flight lock missing');
ok(bot.includes('function v21418MerchantLootBlocked') && bot.includes('function v21418GuardedLoot') && bot.includes('if(S.lootFlight21418)return true') && bot.includes('!v21418MerchantLootBlocked()'), '2.14.22 merchant loot flight gate missing');
ok(bot.includes('merchant_capacity_blocked') && bot.includes('blocked.retryAt=now+15000') && bot.includes("S.mode='Merchant · Kapazität'") && !bot.slice(bot.lastIndexOf('v290InventoryPressureTick=function(){'),bot.indexOf('function v21418ConfigProvenance')).includes('v273CompoundTick()'), '2.14.22 capacity blocked state missing');
ok(bot.includes('S.inputSaveTimers21418') && bot.includes('},450)') && bot.includes('if(sameJSON(before,C[key]))return false'), '2.14.22 config write debounce/dedupe missing');
ok(bot.includes("audit('config_source_selected'") && bot.includes('function v21418ConfigProvenance'), '2.14.22 config provenance missing');
ok(bot.includes('function v21418ProfileCall') && bot.includes("audit('merchant_phase_profile'") && bot.includes("v21418ProfileWrap('inventory'") && bot.includes("v21418ProfileWrap('dashboard'"), '2.14.22 Merchant phase profiler missing');
...
for (const feature of ["merchant-bank-cleanup-confirmation","brain-teaching-hints","dashboard-terrain-tiles","dashboard-learning-feed"]) ok(contract.includes(feature), `2.14.17 static protected feature missing: ${feature}`);
ok(!bot.includes("v2144AuditEconomy("), "2.14.17 undefined Merchant economy audit helper reference remains");
ok(bot.includes("function v21413LiveBankCount"), "2.14.17 live bank confirmation helper missing");
ok(bot.includes("beforeBank:v21413LiveBankCount(name,lv)"), "2.14.17 bank cleanup baseline must use live bank state");
ok(bot.includes("confirmation:confirmation"), "2.14.17 bank cleanup confirmation source diagnostic missing");

for (const feature of ["merchant-performance-budget","merchant-performance-telemetry","dashboard-terrain-pass-through","dashboard-vector-map-fallback","cloud-unconfigured-idle"]) ok(contract.includes(feature), `2.14.17 protected feature missing: ${feature}`);
ok(bot.includes("function v21414CloudConfigured"), "2.14.17 cloud configuration guard missing");
ok(bot.includes("now-Number(c.at||0)<1400"), "2.14.17 sell candidate cache missing");
ok(bot.includes("now-Number(c.at||0)<850"), "2.14.17 service candidate cache missing");
ok(bot.includes("now-Number(S.recipeAnalysis.at||0)<8000"), "2.14.17 recipe analysis budget missing");
ok(bot.includes("lootCadence=character.ctype==='merchant'"), "2.14.17 Merchant loot cadence missing");
ok(bot.includes("merchant_performance_sample"), "2.14.17 Merchant performance telemetry missing");
ok(bot.includes("v21414CompactMoveTarget"), "2.14.17 move audit compaction missing");
...
for (const feature of ["merchant-bank-progress-lease","merchant-bank-sync-diagnostics"]) ok(contract.includes(feature), `2.14.17 protected feature missing: ${feature}`);
ok(bot.includes("function v21415BankCleanupLeaseExpired"), "2.14.17 bank progress lease helper missing");
ok(bot.includes("v21415BankCleanupLeaseExpired(st,now)"), "2.14.17 bank timeout is not progress-relative");
ok(bot.includes("beforeExact:Number(a.beforeExact)||0") && bot.includes("afterBank:bank"), "2.14.17 sync-wait before/after diagnostics missing");
ok(bot.includes("function v2148BankCleanupFinish(kind,detail,level,extra)"), "2.14.17 cleanup finish extra diagnostics channel missing");

for (const feature of ["config-stable-mirror","config-update-namespace-recovery","merchant-compound-flight-guard","merchant-bank-unlock-affordability","merchant-audit-memory-cap"]) ok(contract.includes(feature), `2.14.17 protected feature missing: ${feature}`);
ok(bot.includes("V21416_STABLE_CONFIG_KEY") && bot.includes("v21416FindUpdateBackup"), "2.14.17 stable config recovery missing");
...
ok(bot.includes("function v21416BankPackCost") && bot.includes("merchant_bank_unlock_deferred"), "2.14.17 bank unlock affordability guard missing");
ok(bot.includes("character.ctype==='merchant'?1200:3000"), "2.14.17 Merchant audit memory cap missing");
ok(bot.includes("auditRecentCap:1200"), "2.14.17 Merchant performance memory-cap telemetry missing");

if (!process.exitCode) console.log(`Regression checks OK · ${requiredFeatures.length} protected features · version ${version.version} · Brain v2.14 · Research Bridge · newest config + Merchant serialization/vendor guards`);
