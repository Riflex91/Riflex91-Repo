...
if (versionMatch) {
  ...
  ok(version.version === "2.14.21", "prepared release must be 2.14.21");
  ok(version.dashboardVersion === "2.14.21", "dashboard version must be 2.14.21 for layered brain/dashboard release");
...
ok(bot.includes('S.exchangeRouteFlight21418') && bot.includes("Eintausch-Anreise läuft") && bot.includes("merchant_exchange_route_timeout"), '2.14.21 exchange route flight lock missing');
ok(bot.includes('function v21418MerchantLootBlocked') && bot.includes('function v21418GuardedLoot') && bot.includes('if(S.lootFlight21418)return true') && bot.includes('!v21418MerchantLootBlocked()'), '2.14.21 merchant loot flight gate missing');
ok(bot.includes('merchant_capacity_blocked') && bot.includes('blocked.retryAt=now+15000') && bot.includes("S.mode='Merchant · Kapazität'") && !bot.slice(bot.lastIndexOf('v290InventoryPressureTick=function(){'),bot.indexOf('function v21418ConfigProvenance')).includes('v273CompoundTick()'), '2.14.21 capacity blocked state missing');
ok(bot.includes('S.inputSaveTimers21418') && bot.includes('},450)') && bot.includes('if(sameJSON(before,C[key]))return false'), '2.14.21 config write debounce/dedupe missing');
ok(bot.includes("audit('config_source_selected'") && bot.includes('function v21418ConfigProvenance'), '2.14.21 config provenance missing');
ok(bot.includes('function v21418ProfileCall') && bot.includes("audit('merchant_phase_profile'") && bot.includes("v21418ProfileWrap('inventory'") && bot.includes("v21418ProfileWrap('dashboard'"), '2.14.21 Merchant phase profiler missing');
