...
import {
  ...
  type GruppenKoordinationsKonfiguration,
  type GruppenTeilnehmerBewertung,
...
const GEFAHREN_RANG: Readonly<Record<KampfGefahrenStufe, number>> = Object.freeze({
  sicher: 0,
  angespannt: 1,
  gefaehrlich: 2,
  kritisch: 3,
...
export function erstelleGruppenKoordinationsKonfiguration(
  aenderungen: Partial<GruppenKoordinationsKonfiguration> = {}
): GruppenKoordinationsKonfiguration {
...
function sortiereMeldungen(meldungen: readonly GruppenTeilnehmerMeldung[]): readonly GruppenTeilnehmerMeldung[] {
...
function verdichteNeuesteMeldungen(meldungen: readonly GruppenTeilnehmerMeldung[]): readonly GruppenTeilnehmerMeldung[] {
...
function bewerteTeilnehmer(
  meldung: GruppenTeilnehmerMeldung,
  eigenerTeilnehmer: GruppenTeilnehmerMeldung,
  jetzt: number,
  konfiguration: GruppenKoordinationsKonfiguration
): GruppenTeilnehmerBewertung {
  const alterMillisekunden = jetzt - meldung.gesendetAm;

  let status: GruppenTeilnehmerStatus = 'aktiv';
  ...
  if (!Number.isFinite(alterMillisekunden) || alterMillisekunden < 0 || alterMillisekunden > konfiguration.lebensnachweisMaximalAlterMillisekunden) {
    ...
    meldung.serverRegion !== eigenerTeilnehmer.serverRegion ||
    meldung.serverKennung !== eigenerTeilnehmer.serverKennung
  ) {
    status = 'falsche_welt';
...
function waehleFaehigkeitsTraeger(
  faehigkeit: GruppenFaehigkeit,
  aktiveTeilnehmer: readonly GruppenTeilnehmerMeldung[]
): string | null {
...
function verteileAufgaben(aktiveTeilnehmer: readonly GruppenTeilnehmerMeldung[]): GruppenAufgabenZuordnung {
  const eintraege = GRUPPEN_FAEHIGKEITEN.map((faehigkeit) => [
    faehigkeit,
    waehleFaehigkeitsTraeger(faehigkeit, aktiveTeilnehmer)
  ] as const);
  return Object.freeze(Object.fromEntries(eintraege) as Record<GruppenFaehigkeit, string | null>);
}

function gemeinsameGefahrenStufe(aktiveTeilnehmer: readonly GruppenTeilnehmerMeldung[]): KampfGefahrenStufe {
...
function gemeinsamesZiel(aktiveTeilnehmer: readonly GruppenTeilnehmerMeldung[]): string | null {
...
export function koordiniereGruppe(
  meldungen: readonly GruppenTeilnehmerMeldung[],
  eigenerTeilnehmerKennung: string,
  jetzt: number,
  konfiguration: GruppenKoordinationsKonfiguration = erstelleGruppenKoordinationsKonfiguration()
  ...
  const bewertungNachKennung = new Map(bewertungen.map((bewertung) => [bewertung.charakterKennung, bewertung] as const));
  ...
  const aktiveTeilnehmer = verdichtet.filter(
    (meldung) => bewertungNachKennung.get(meldung.charakterKennung)?.status === 'aktiv'
  );
