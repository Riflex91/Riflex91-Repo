...
import { AdventureLandGruppenZielAusfuehrung } from '../../erzeugt/ausfuehrung/adventure-land-gruppen-ziel-ausfuehrung.js';
...
test('Block 8 Gruppenziel-Ausfuehrung: exakt freigegebener gemeinsamer Angriff erreicht attack genau einmal', async () => {
  const aufrufe = [];
  ...
  assert.equal(steuerung.holeAktionsZustand(anfrage().kennung)?.phase, 'abgeschlossen');
  assert.equal(steuerung.listeRessourcenSperren().length, 0);
});

test('Block 8 Gruppenziel-Ausfuehrung: verlorener Ressourcenbesitz blockiert vor attack und bricht zentral ab', async () => {
  const aufrufe = [];
  const { fenster } = spiel(aufrufe);
  const ressourcenVergabe = new RessourcenVergabe();
  const { steuerung, schritt } = starte(anfrage(), ressourcenVergabe);
  ressourcenVergabe.gibRessourcenFuerBesitzerFrei(anfrage().kennung);
  const ausfuehrung = new AdventureLandGruppenZielAusfuehrung(fenster, { aktivFreigegeben: true });
...
test('Block 8 Gruppenziel-Ausfuehrung: Safety-Wechsel oder stale Safety blockiert vor attack', async () => {
