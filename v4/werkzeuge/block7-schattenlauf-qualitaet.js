(() => {
  ...
  const API_NAME = 'V4Block7SchattenQualitaet';
  const VERSION = '1.0.0';
  ...
  function holeSchattenApi() {
    const api = globalThis.V4Block7SchattenRanger ?? holeElternFenster()?.V4Block7SchattenRanger;
    if (!api || typeof api.starte !== 'function' || typeof api.status !== 'function' || typeof api.ergebnis !== 'function' || typeof api.stoppe !== 'function') {
      throw new Error('Zuerst v4/werkzeuge/block7-schattenlauf-ranger.js laden.');
    }
    return api;
  }
  ...
  function aktivierePerformanceTrick() {
    ...
    const funktion = typeof eltern?.performance_trick === 'function' ? eltern.performance_trick : globalThis.performance_trick;
    const kontext = typeof eltern?.performance_trick === 'function' ? eltern : globalThis;
    if (typeof funktion !== 'function') {
      throw new Error('Adventure-Land-Funktion performance_trick() ist nicht verfuegbar. Der Block-7-Schattenlauf wird nicht gestartet.');
    }
    try {
  ...
  function prozent(name, wert, standard) {
    const ergebnis = wert ?? standard;
  ...
  function samplingQualitaet(rohbericht, zeitpunkt) {
    ...
    const echteDauer = Number.isFinite(rohbericht?.dauerMillisekunden)
      ? rohbericht.dauerMillisekunden
      : (gestartetAm !== null ? Math.max(0, zeitpunkt - gestartetAm) : null);
    const erwartungsDauer = rohbericht?.status === 'abgeschlossen' && vorgesehen !== null
      ? vorgesehen
      : (echteDauer !== null && vorgesehen !== null ? Math.min(echteDauer, vorgesehen) : echteDauer);
    const erwartet = erwartungsDauer !== null && intervall !== null ? erwarteteSchritte(erwartungsDauer, intervall) : null;
    ...
    if (lauf?.letzterSchrittBeobachtetAm !== null && lauf?.letzterSchrittBeobachtetAm !== undefined && rohbericht?.status !== 'laeuft') {
      maxLuecke = Math.max(maxLuecke, Math.max(0, zeitpunkt - lauf.letzterSchrittBeobachtetAm));
    }
    const mindestAbdeckung = lauf?.mindestAbdeckungProzent ?? STANDARD_MINDEST_ABDECKUNG_PROZENT;
    const erlaubteLuecke = lauf?.erlaubteMaximaleTickLueckeMillisekunden ?? (Number.isFinite(intervall) ? intervall * STANDARD_MAX_TICK_LUECKE_FAKTOR : null);
  ...
  function kompakterBericht(rohbericht, zeitpunkt = Date.now()) {
    ...
    if (rohbericht.status === 'abgeschlossen') {
      if (qualitaet.browserSamplingAusreichend) {
        ...
        grund = `Die konfigurierte Block-7-Schattenlaufzeit von ${formatDauer(rohbericht.vorgesehenMillisekunden)} ist mit ausreichender Sampling-Qualitaet abgelaufen.`;
      } else {
    ...
    return Object.freeze({
      ...
      modus: 'block6_7_schatten_mit_samplingqualitaet',
      ...
      sicherheitsZaehler: Object.freeze({ ...(rohbericht.sicherheitsZaehler ?? {}) }),
      farmZaehler: Object.freeze({ ...(rohbericht.farmZaehler ?? {}) }),
      bereitschaftZaehler: Object.freeze({ ...(rohbericht.bereitschaftZaehler ?? {}) }),
      gefahrenGrundZaehler: Object.freeze({ ...(rohbericht.gefahrenGrundZaehler ?? {}) }),
  ...
  function finalisiereAusQuelle(zeitpunkt = Date.now()) {
    ...
    lauf = null;
    holeKonsole().ausgeben(letzterBericht, `Block 7 Schatten · Sampling ${letzterBericht?.status ?? 'unbekannt'}`);
  ...
  function monitor() {
    ...
    try {
      ...
      lauf.monitorFehler.push({ zeitpunkt, meldung: fehler instanceof Error ? fehler.message : String(fehler) });
    }
  }

  function starte(monsterArten, optionen = {}) {
    if (lauf) throw new Error('Es laeuft bereits ein ueberwachter Block-7-Schattenlauf.');
    const schatten = holeSchattenApi();
    if (schatten.status()?.laeuft) throw new Error('Vor dem ueberwachten Lauf darf kein anderer Block-7-Schattenlauf laufen.');
    ...
    const mindestAbdeckungProzent = prozent('mindestAbdeckungProzent', optionen.mindestAbdeckungProzent, STANDARD_MINDEST_ABDECKUNG_PROZENT);
    ...
    holeKonsole().ausgeben({
      ...
      sicherheit: 'Read-only; keine Adventure-Land-Spielaktion wird ausgefuehrt.'
    }, 'Block 7 Schatten mit Sampling-Qualitaet gestartet');
    return status();
  }
  ...
  holeKonsole().ausgeben({
    version: VERSION,
    start10Minuten: 'V4Block7SchattenQualitaet.starte(["goo"])',
    status: 'V4Block7SchattenQualitaet.status()',
    ergebnis: 'V4Block7SchattenQualitaet.kompaktErgebnis()',
    stop: 'V4Block7SchattenQualitaet.stoppe("Grund")',
    performanceTrick: 'performance_trick() wird vor jedem Lauf verpflichtend aktiviert.',
    erwarteteSchritte: 601,
  ...
  }, 'Block-7-Schattenlauf-Samplingqualitaet bereit');
})();